...
import { 
  ...
  Timer, Bomb, Maximize2, Sliders, ChevronUp, ChevronDown, Plus, Minus, Eye, MessageSquare, Clock, BarChart3, Settings
} from 'lucide-react';
...
const calculateAccurateEquity = (heroHand, community) => {
    if (!heroHand || community.length < 3) return 0;
    const sorted = [...heroHand, ...community].sort((a, b) => VM[b.value] - VM[a.value]);
    const highCard = VM[sorted[0].value];
    return Math.min(99, Math.max(5, (highCard / 14) * 80 + (Math.random() * 10)));
};
    ...
    return (
        <div style={{ left: `${displayPos.x}%`, top: `${displayPos.y}%` }} className={`absolute -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-20 transition-all duration-500 ${player.isFolded ? 'opacity-30 grayscale scale-95' : 'opacity-100'}`}>
            ...
            <div style={{ transform: `translateY(${visuals.badgeY}px)` }}
                ...
                {isDealer && ( 
                  <div className="absolute -top-1.5 -right-1.5 flex items-center justify-center z-[110]">
                    <div className="w-3.5 h-3.5 bg-red-600 rounded-full border-2 border-white shadow-[0_0_12px_rgba(220,38,38,0.9)] animate-pulse" />
                  </div> 
                )}
                ...
                {isActiveTurn && timeRemaining > 0 && (
                    <div className="absolute -top-2 w-full px-2 h-1.5 z-[60]">
                        <div className="w-full h-full bg-black/40 rounded-full overflow-hidden shadow-inner">
                            <div className="h-full bg-cyan-400 transition-all duration-1000 linear" style={{ width: `${(timeRemaining / TURN_LIMIT) * 100}%` }} />
                        </div>
                    </div>
                )}
                ...
                <div className="flex flex-col items-center gap-0.5 w-full">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-[10px] md:text-[12px] font-black text-white/90 uppercase tracking-tight truncate max-w-[80px]">{String(player.name || "Anon")}</span>
                    </div>
            ...
            {player.hand && Array.isArray(player.hand) && !player.isFolded && (
                <div className="relative z-10 flex items-center justify-center w-[12vw] h-[6vw] mt-4 overflow-visible">
                    {player.hand.map((c, ci) => {
                        ...
                        const offset = ci - mid;
                        const fanRotation = offset * (player.hand.length > 2 ? 22 : 35);
                        const fanTranslation = offset * (player.hand.length > 2 ? 2.5 : 4.5);
                        return (
                          <div key={c.id || ci} 
...
const App = () => {
  ...
  const heroWinProb = useMemo(() => {
    ...
    return calculateAccurateEquity(heroPlayerObj.hand, community);
  ...
  const isBrokeStatus = useMemo(() => {
    return heroPlayerObj && Number(heroPlayerObj.chips) <= 0 && phase !== PHASES.SHOWDOWN && phase !== PHASES.IDLE;
  }, [heroPlayerObj, phase]);
  ...
  useEffect(() => {
    const handleRoomUpdate = (d) => {
        ...
        if (d.phase === PHASES.SHOWDOWN) {
            setPotTransferring(true); setShowdownWinners(d.showdownWinners || null);
    ...
    return () => { 
      ...
      socket.off('profilesUpdate'); socket.off('initialDataResponse'); 
  ...
  if (currentView === VIEWS.ADMIN) return (
    <div className="h-screen bg-[#06080c] flex flex-col md:flex-row text-white uppercase font-black overflow-hidden">
        <aside className="w-full md:w-64 border-r border-white/10 p-8 flex flex-col gap-4 bg-black/20 shrink-0">
            <h2 className="text-[#fbbf24] flex items-center gap-2 mb-4"><ShieldCheck size={20}/> ADMIN</h2>
            <button onClick={()=>setAdminTab(ADMIN_TABS.PLAYERS)} className={`p-4 rounded-xl text-xs font-black ${adminTab === ADMIN_TABS.PLAYERS ? 'bg-[#fbbf24] text-black' : 'bg-white/5'}`}>PLAYERS</button>
            <button onClick={()=>setAdminTab(ADMIN_TABS.TABLES)} className={`p-4 rounded-xl text-xs font-black ${adminTab === ADMIN_TABS.TABLES ? 'bg-[#fbbf24] text-black' : 'bg-white/5'}`}>TABLES</button>
            <button onClick={handleNuclear} className={`p-4 rounded-xl flex items-center justify-center gap-2 border-2 mt-4 ${nuclearConfirm ? 'bg-red-600 border-white text-white' : 'bg-white/5 text-red-500'}`}>{nuclearConfirm ? <Bomb size={20}/> : <ShieldAlert size={20}/>}<span>{nuclearConfirm ? 'CONFIRM' : 'NUCLEAR'}</span></button>
            <button onClick={()=>{setCurrentView(VIEWS.LOGIN); setUserProfile(null);}} className="mt-auto p-4 text-white/20 hover:text-white text-xs flex items-center justify-center gap-2 font-black"><ArrowLeft size={16}/> LOGOUT</button>
        </aside>
            ...
            {adminTab === ADMIN_TABS.PLAYERS ? (
                <div className="flex flex-col gap-8">
                    ...
                    <div className="bg-white/5 p-6 rounded-2xl grid grid-cols-1 md:grid-cols-3 gap-4 border border-white/10">
                        <input value={newPlayer.name} onChange={e=>setNewPlayer({...newPlayer, name: e.target.value})} placeholder="NAME" className="bg-black/40 p-4 rounded-xl border border-white/10 outline-none uppercase text-white"/>
                        <input value={newPlayer.password} onChange={e=>setNewPlayer({...newPlayer, password: e.target.value})} placeholder="PASS" className="bg-black/40 p-4 rounded-xl border border-white/10 outline-none uppercase text-white"/>
                        <button onClick={handleCreatePlayer} className="bg-[#fbbf24] text-black rounded-xl font-black p-4">CREATE</button>
                    </div>
                    <div className="bg-white/5 rounded-2xl overflow-hidden border border-white/10 mt-6">
                        {allProfiles.map(p => (
                            <div key={p.uid} className="flex justify-between p-4 border-b border-white/5 items-center hover:bg-white/5">
                                <span className="text-sm font-black">{String(p.name)} <span className="text-white/20 ml-2">[{String(p.password)}]</span></span>
                                <div className="flex gap-4 items-center"><span className="text-emerald-400 font-mono text-lg">${Number(p.chips || 0).toLocaleString()}</span><button onClick={()=>{const n = prompt("NEW WALLET", p.chips); if(n) socket.emit('adminEditChips', {uid: p.uid, chips: Number(n)})}} className="text-cyan-400"><Edit3 size={18}/></button><button onClick={()=>socket.emit('adminDeletePlayer', p.uid)} className="text-red-500"><Trash2 size={18}/></button></div>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-8">
                    ...
                    <div className="bg-white/5 p-6 rounded-2xl grid grid-cols-1 md:grid-cols-2 gap-4 border border-white/10">
                        <input value={newTable.name} onChange={e=>setNewTable({...newTable, name: e.target.value})} placeholder="ARENA NAME" className="bg-black/40 p-4 rounded-xl border border-white/10 outline-none uppercase text-white"/>
                        <button onClick={handleSpawnArena} className="bg-emerald-600 text-white rounded-xl font-black p-4">SPAWN ARENA</button>
                    </div>
                    <div className="grid grid-cols-1 gap-4 mt-6">
                        {activeTables.map(t => (
                            <div key={t.id} className="bg-white/5 p-4 rounded-2xl flex justify-between items-center border border-white/10">
                              ...
                              <button onClick={()=>socket.emit('adminDeleteRoom', t.id)} className="bg-red-950/40 px-4 py-2 rounded-xl text-red-500 font-black text-xs">TERMINATE</button>
                            </div>
                        ))}
                    </div>
                </div>
    ...
    <div className="h-screen bg-[#06080c] flex flex-col text-white font-black uppercase overflow-hidden">
        {selectedTableForJoin && (
            <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-md px-6">
              <div className="w-full max-w-[400px] p-12 bg-slate-900 border border-[#fbbf24]/30 rounded-[2vw] shadow-2xl flex flex-col gap-10">
                <h3 className="text-3xl text-center text-[#fbbf24] underline underline-offset-8 uppercase font-black">{String(selectedTableForJoin.name)}</h3>
                <div className="space-y-6 font-black text-center uppercase">
                  <div className="flex justify-between items-center text-[10px] text-white/40 tracking-widest font-black"><span>BUY-IN AMOUNT</span><span className="text-emerald-400 text-2xl font-mono">${buyInAmount.toLocaleString()}</span></div>
                ...
                <div className="flex gap-4"><button onClick={()=>setSelectedTableForJoin(null)} className="flex-1 p-5 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 transition-all font-black uppercase">BACK</button><button onClick={joinRoom} className="flex-2 p-5 bg-emerald-600 rounded-2xl shadow-lg hover:scale-105 active:scale-95 transition-all text-sm tracking-widest font-black uppercase">SIT DOWN</button></div>
              </div>
            </div>
        )}
        <header className="h-20 border-b border-white/10 flex items-center justify-between px-12 bg-black/40 backdrop-blur-md shadow-xl z-50 shrink-0">
          <h2 className="tracking-[0.4em] text-xl flex items-center gap-4 font-black"><LayoutGrid className="text-[#fbbf24]"/> LOBBY</h2>
          <div className="flex items-center gap-10 font-black">
            <div className="flex flex-col items-end"><span className="text-[10px] text-white/40 uppercase italic">ID: {String(userProfile?.name || "??")}</span><span className="text-emerald-400 font-mono text-2xl tracking-tighter">${Number(userProfile?.chips || 0).toLocaleString()}</span></div>
        ...
        <main className="flex-1 p-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 overflow-y-auto bg-gradient-to-br from-transparent to-white/5 font-black uppercase">
            {activeTables.length === 0 ? (<div className="col-span-full flex flex-col items-center justify-center p-20 text-white/20 gap-4 uppercase font-black"><ShieldAlert size={64} /><span className="text-xl tracking-[0.5em]">NO ACTIVE ARENAS</span></div>) : (activeTables.map((t) => (<div key={t.id} className="p-8 bg-white/5 border border-white/5 rounded-3xl flex flex-col gap-6 shadow-2xl hover:border-[#fbbf24]/20 transition-all group relative overflow-hidden font-black"><h3 className="text-2xl tracking-widest text-white group-hover:text-[#fbbf24] transition-colors uppercase font-black">{String(t.name)}</h3><div className="bg-black/60 p-6 rounded-2xl flex justify-between items-center border border-white/5 shadow-inner uppercase font-black"><div className="flex flex-col font-black"><span className="text-[8px] text-white/40 tracking-widest">STAKES</span><span className="text-[#fbbf24] text-xl font-black">${t.sb}/${t.bb}</span></div><div className="flex flex-col items-end font-black"><span className="text-[8px] text-white/40 tracking-widest">SEATS</span><span className="text-white/80 font-mono text-base font-black">{t.players?.filter(p=>p).length || 0}/10</span></div></div><button onClick={()=>setSelectedTableForJoin(t)} className="relative z-20 w-full p-8 bg-emerald-600 rounded-2xl tracking-widest shadow-xl hover:scale-[1.02] active:scale-95 transition-all font-black uppercase">ENTER ARENA</button></div>)))}
        </main>
    </div>
  );
    ...
    <div className="h-screen bg-[#06080c] text-white flex flex-col overflow-hidden relative font-black uppercase tracking-tighter">
      ...
      {showVisualControls && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
            <div className="w-full max-w-[500px] bg-slate-900 border border-white/10 rounded-3xl p-8 flex flex-col gap-6 shadow-2xl max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between border-b border-white/10 pb-4">
                    <h3 className="text-xl text-[#fbbf24] flex items-center gap-2"><Sliders size={20}/> DISPLAY SETTINGS</h3>
                    <button onClick={() => setShowVisualControls(false)} className="text-white/40 hover:text-white"><X/></button>
                </div>
                
                {/* Hero Cards */}
                <div className="space-y-4">
                    <h4 className="text-[10px] tracking-widest text-emerald-400">HERO HOLE CARDS</h4>
                    <div className="grid grid-cols-2 gap-4">
                        <div className="flex flex-col gap-1">
                            <label className="text-[9px] text-white/40">SIZE SCALE ({visuals.heroCardScale.toFixed(1)})</label>
                            <input type="range" min="1.0" max="4.0" step="0.1" value={visuals.heroCardScale} onChange={(e) => setVisuals({...visuals, heroCardScale: Number(e.target.value)})} className="accent-emerald-500" />
                        </div>
                        <div className="flex flex-col gap-1">
                            <label className="text-[9px] text-white/40">Y POSITION ({visuals.heroCardY}px)</label>
                            <input type="range" min="-100" max="150" step="1" value={visuals.heroCardY} onChange={(e) => setVisuals({...visuals, heroCardY: Number(e.target.value)})} className="accent-emerald-500" />
                        </div>
                    </div>
                </div>
                ...
                {/* Opponent Cards */}
                <div className="space-y-4">
                    <h4 className="text-[10px] tracking-widest text-indigo-400">OPPONENT HOLE CARDS</h4>
                    <div className="grid grid-cols-2 gap-4">
                        <div className="flex flex-col gap-1">
                            <label className="text-[9px] text-white/40">SIZE SCALE ({visuals.oppCardScale.toFixed(1)})</label>
                            <input type="range" min="0.5" max="2.0" step="0.1" value={visuals.oppCardScale} onChange={(e) => setVisuals({...visuals, oppCardScale: Number(e.target.value)})} className="accent-indigo-500" />
                        </div>
                        <div className="flex flex-col gap-1">
                            <label className="text-[9px] text-white/40">Y POSITION ({visuals.oppCardY}px)</label>
                            <input type="range" min="-100" max="150" step="1" value={visuals.oppCardY} onChange={(e) => setVisuals({...visuals, oppCardY: Number(e.target.value)})} className="accent-indigo-500" />
                        </div>
                    </div>
                </div>
                ...
                {/* Community Cards */}
                <div className="space-y-4">
                    <h4 className="text-[10px] tracking-widest text-cyan-400">COMMUNITY CARDS</h4>
                    <div className="grid grid-cols-2 gap-4">
                        <div className="flex flex-col gap-1">
                            <label className="text-[9px] text-white/40">SIZE SCALE ({visuals.commCardScale.toFixed(1)})</label>
                            <input type="range" min="1.0" max="3.0" step="0.1" value={visuals.commCardScale} onChange={(e) => setVisuals({...visuals, commCardScale: Number(e.target.value)})} className="accent-cyan-500" />
                        </div>
                        <div className="flex flex-col gap-1">
                            <label className="text-[9px] text-white/40">Y OFFSET ({visuals.commCardY}px)</label>
                            <input type="range" min="-100" max="100" step="1" value={visuals.commCardY} onChange={(e) => setVisuals({...visuals, commCardY: Number(e.target.value)})} className="accent-cyan-500" />
                        </div>
                    </div>
                </div>
                ...
                {/* Bet Chips */}
                <div className="space-y-4">
                    <h4 className="text-[10px] tracking-widest text-amber-400">BET LABELS</h4>
                    <div className="grid grid-cols-2 gap-4">
                        <div className="flex flex-col gap-1">
                            <label className="text-[9px] text-white/40">SIZE SCALE ({visuals.betScale.toFixed(1)})</label>
                            <input type="range" min="0.5" max="2.0" step="0.1" value={visuals.betScale} onChange={(e) => setVisuals({...visuals, betScale: Number(e.target.value)})} className="accent-amber-500" />
                        </div>
                        <div className="flex flex-col gap-1">
                            <label className="text-[9px] text-white/40">Y OFFSET ({visuals.betY}px)</label>
                            <input type="range" min="-200" max="200" step="1" value={visuals.betY} onChange={(e) => setVisuals({...visuals, betY: Number(e.target.value)})} className="accent-amber-500" />
                        </div>
                    </div>
                </div>
                ...
                {/* Badge Pos */}
                <div className="space-y-4">
                    <h4 className="text-[10px] tracking-widest text-white/60">PLAYER BADGE</h4>
                    <div className="flex flex-col gap-1">
                        <label className="text-[9px] text-white/40">Y OFFSET ({visuals.badgeY}px)</label>
                        <input type="range" min="-150" max="200" step="1" value={visuals.badgeY} onChange={(e) => setVisuals({...visuals, badgeY: Number(e.target.value)})} className="w-full accent-white" />
                    </div>
                </div>

                <button onClick={() => setShowVisualControls(false)} className="w-full p-4 bg-emerald-600 rounded-xl font-black mt-2">CLOSE</button>
            </div>
        </div>
      )}
      ...
      {isBrokeStatus && (
          <div className="fixed inset-0 z-[5000] flex items-center justify-center bg-black/95 backdrop-blur-2xl p-6">
            <div className="w-full max-w-[400px] p-10 bg-slate-900 border-2 border-red-500 rounded-3xl text-center shadow-[0_0_100px_rgba(239,68,68,0.4)] font-black">
              <AlertTriangle size={80} className="text-red-500 animate-pulse mb-6 mx-auto" />
              <h2 className="text-3xl font-black mb-2">BUSTED!</h2>
              <p className="text-white/40 mb-6 text-[10px] tracking-widest">WALLET: ${userProfile?.chips?.toLocaleString() || '0'}</p>
              {(userProfile?.chips || 0) >= 1000 ? (<button onClick={() => socket.emit('playerRebuy', { roomId: currentRoomId, uid: userProfile.uid, amount: 1000 })} className="w-full p-6 bg-emerald-600 text-white rounded-2xl shadow-xl animate-bounce font-black uppercase">REBUY $1,000</button>) : (<div className="p-6 bg-white/5 rounded-2xl border border-white/10 text-white/40 text-xs font-black uppercase">INSUFFICIENT WALLET</div>)}
              <button onClick={() => {socket.emit('leaveRoom', { uid: userProfile.uid }); setCurrentView(VIEWS.LOBBY);}} className="mt-4 text-white/20 hover:text-white text-xs uppercase underline">EXIT ARENA</button>
            </div>
          </div>
      )}

      <header style={{ height: `${headerHeight}px` }} className="bg-[#0a0a0a] border-b border-white/10 flex items-center justify-between px-8 z-[80] shadow-2xl backdrop-blur-md shrink-0 font-black">
        <div className="flex items-center gap-4">
            <div className="bg-white/5 px-3 py-1.5 rounded-xl border border-white/5 shadow-inner truncate font-black uppercase flex items-center gap-2">
              <span className="text-[#fbbf24] text-[10px] tracking-widest">ARENA:</span>
              <span className="text-white text-xs">{String(activeVariant?.name || "Hold'em")}</span>
            </div>
            <div className="bg-white/5 px-2 py-1 rounded border border-white/5 text-[9px] text-white/20 font-mono tracking-widest">{VERSION}</div>
        </div>
        
        <div className="flex items-center gap-4">
          <div className="bg-white/5 border border-white/10 px-4 py-1.5 rounded-xl hidden md:flex items-center gap-4 shadow-inner font-black uppercase">
            <span className="text-white/40 text-[9px] tracking-widest uppercase">On my deal:</span>
            <select value={pendingVariantId} onChange={(e) => { setPendingVariantId(e.target.value); socket.emit('updatePlayerSettings', {uid: userProfile.uid, pendingVariant: e.target.value}); }} className="bg-transparent text-[#fbbf24] outline-none text-xs cursor-pointer font-black">{Object.entries(VARIANTS).map(([k,v])=><option key={k} value={k} className="bg-slate-900 font-black">{v.name}</option>)}</select>
          </div>
          <div className="hidden sm:flex items-center gap-2 bg-white/5 border border-white/10 px-3 py-1.5 rounded-xl mr-2 font-mono text-[11px] shadow-inner"><TrendingUp size={14} className="text-cyan-400" /><span className="text-white/40 uppercase text-[9px] tracking-tighter shrink-0">Prob:</span><span className="text-[#fbbf24] font-black text-right">{Math.round(heroWinProb || 0)}%</span></div>
          <div className="flex gap-2 uppercase items-center">
              <button onClick={() => setShowVisualControls(true)} className="text-cyan-400 p-2 bg-white/5 border border-white/10 rounded-xl hover:bg-cyan-400/20 font-black" title="Layout Settings"><Settings size={20}/></button>
              <button onClick={addBot} className="text-indigo-400 p-2 bg-white/5 border border-white/10 rounded-xl hover:bg-indigo-400/20 font-black" title="Add Bot"><Bot size={20}/></button>
      ...
      <main className="flex-1 flex flex-col items-center justify-center relative bg-gradient-to-b from-emerald-950/20 to-transparent overflow-hidden px-2 py-2 font-black uppercase">
        <div style={{ transform: `scale(${tableZoom})`, maxHeight: `calc(100vh - ${headerHeight + footerHeight + 10}px)` }} className="relative w-full max-w-[1400px] aspect-[21/10] flex items-center justify-center h-full origin-center font-black">
            ...
            <div className="absolute inset-0 pointer-events-none z-20 font-black uppercase">
              {(players || []).map((p, i) => { if (!p) return null; const rIdx = (i - (heroIdx !== -1 ? heroIdx : 0) + TOTAL_SEATS) % TOTAL_SEATS; return (<Seat key={i} player={p} displayPos={DISPLAY_POSITIONS[rIdx]} phase={phase} winning5Ids={winning5Ids} isActiveTurn={activeIdx === i} isDealer={dealerIdx === i} isHero={i === heroIdx} relativeIdx={rIdx} seatIdx={i} timeRemaining={timeRemaining} visuals={visuals}/>); })}
            </div>
            <div className="absolute top-[48%] left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-30 pointer-events-none w-full h-full justify-center">
              ...
              {['HOLDEM', 'OMAHA', 'PINEAPPLE', 'HILOW', 'MUFLIS', 'REDSBLACKS'].includes(activeVariant?.id) && (
                <div className="flex gap-2 md:gap-4 mt-6 md:mt-12 font-black uppercase transition-transform" 
                     style={{ transform: `scale(${visuals.commCardScale}) translateY(${visuals.commCardY}px)` }}>
                  {(community || []).map((c, j) => (
                    <div key={c.id || j} className={`w-[6vw] md:w-[3vw] h-[9vw] md:h-[5vw] rounded-[4px] border bg-white flex flex-col items-center justify-center text-black font-black transition-all duration-300 ${winning5Ids?.includes(c.id) ? 'ring-4 ring-yellow-400 scale-110 z-30 shadow-[0_0_40px_rgba(251,191,36,0.6)]' : 'border-white/20 shadow-2xl'}`}>
                      <span className="text-[14px] md:text-[0.9vw] font-black">{String(c.value)}</span>
      ...
      <footer style={{ height: `${footerHeight}px` }} className="bg-black/95 backdrop-blur-3xl border-t border-white/10 flex flex-col z-[100] shadow-[0_-10px_40px_rgba(0,0,0,0.5)] shrink-0 font-black uppercase overflow-hidden">
        <div className="flex-1 flex flex-col justify-center px-10 relative bg-white/5 shadow-inner py-3 font-black uppercase">
          {activeIdx === heroIdx && phase !== PHASES.IDLE && heroPlayerObj ? (
            <div className="flex flex-col gap-5 animate-in slide-in-from-bottom duration-500 items-center w-full font-black uppercase">
                <div className="absolute top-2 right-4 animate-in slide-in-from-right duration-500"><div className="flex flex-col items-end"><span className="text-[7px] text-white/40 tracking-[0.2em] font-black uppercase">Current Hand</span><span className="text-[14px] text-purple-400 font-black uppercase">{String(heroPlayerObj.strength || "High Card")}</span></div></div>
                {Number(heroPlayerObj.chips) > 0 ? (<>
                        <div className="flex gap-2 w-full max-w-[600px] font-black uppercase">
                            <button onClick={()=>handleAction('RAISE', highestBet + Math.floor(potAmount * 0.5))} className="flex-1 py-2 bg-white/5 border border-white/10 rounded-xl text-[12px] hover:bg-white/20 transition-all font-black uppercase">1/2 POT</button>
                            <button onClick={()=>handleAction('RAISE', highestBet + potAmount)} className="flex-1 py-2 bg-white/5 border border-white/10 rounded-xl text-[12px] hover:bg-white/20 transition-all font-black uppercase">POT</button>
                            <button onClick={handleAllIn} className="flex-1 py-2 bg-red-900/30 border border-red-500/50 rounded-xl text-[12px] text-red-500 hover:bg-red-600 hover:text-white transition-all font-black uppercase">ALL-IN (${(Number(heroPlayerObj.chips) + Number(heroPlayerObj.currentBet)).toLocaleString()})</button>
                        </div>
                        <div className="flex gap-6 w-full items-center justify-center font-black">
                            <button onClick={()=>handleAction('FOLD')} className="w-32 h-16 bg-red-950/60 border-2 border-red-500/50 rounded-2xl tracking-[0.2em] hover:brightness-125 transition-all font-black text-xs shadow-xl uppercase">FOLD</button>
                            <button onClick={()=>handleAction('CALL')} className="flex-1 max-w-[360px] h-16 bg-indigo-900/60 border-2 border-indigo-400/50 rounded-2xl text-xl tracking-[0.3em] hover:brightness-125 font-black shadow-xl uppercase">
                                {highestBet > heroPlayerObj.currentBet ? (highestBet - heroPlayerObj.currentBet >= heroPlayerObj.chips ? `CALL ALL-IN $${heroPlayerObj.chips.toLocaleString()}` : `CALL $${(highestBet - heroPlayerObj.currentBet).toLocaleString()}`) : 'CHECK'}
                            </button>
                            <div className="flex gap-2 items-center bg-black/60 border border-white/10 p-2 rounded-2xl shadow-inner min-w-[320px] font-black uppercase">
                                <div className="flex items-center bg-black/40 px-5 rounded-xl border border-white/5 h-14 font-black uppercase"><span className="text-[#fbbf24] text-xl font-mono mr-1">$</span><input type="number" value={raiseInput} onChange={(e) => setRaiseInput(Math.min(Number(heroPlayerObj.chips) + Number(heroPlayerObj.currentBet), Math.max(minRaiseAllowed, Number(e.target.value))))} className="w-28 bg-transparent text-center font-mono text-2xl text-[#fbbf24] outline-none font-black" /></div>
                                <button onClick={()=>handleAction('RAISE', raiseInput)} className="flex-1 h-14 bg-emerald-600/60 border border-emerald-400/50 rounded-xl flex items-center justify-center hover:brightness-125 font-black uppercase text-xl shadow-xl"><Zap size={20} className="mr-2 text-emerald-400"/> RAISE</button>
                            </div>
                        </div></>
                ) : ( <div className="flex flex-col items-center gap-2 animate-pulse"><ShieldCheck size={48} className="text-cyan-400 mb-2" /><span className="text-4xl font-black text-white tracking-tighter uppercase">ALL-IN POSITION</span></div> )}
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center h-full relative font-black uppercase">
                {showdownWinners && showdownWinners.length > 0 ? (
                    <div className="flex flex-col items-center gap-3 w-full h-full justify-center">
                        <div className="flex items-center gap-2 text-yellow-400 animate-pulse font-black tracking-[0.3em] text-xs uppercase"><Trophy size={16} /> SHOWDOWN WINNERS</div>
                        <div className="flex flex-wrap gap-6 items-center justify-center animate-in fade-in zoom-in duration-700 w-full overflow-y-auto px-4">
                            {showdownWinners.map((winner, idx) => (
                                <div key={idx} className="flex items-center gap-6 bg-black/60 p-4 rounded-[2rem] border border-yellow-500/30 shadow-[0_0_30px_rgba(251,191,36,0.2)] min-w-[320px] animate-showdown-card-pop" style={{ animationDelay: `${idx * 0.1}s` }}><div className="flex flex-col items-center shrink-0"><div className="text-[#fbbf24] font-black text-2xl drop-shadow-lg uppercase">{String(winner.name)}</div><div className="text-emerald-400 font-mono text-xl font-black">+${(winner.amount || 0).toLocaleString()}</div><div className="bg-yellow-600/20 text-yellow-400 text-[8px] tracking-widest uppercase mt-1 px-2 py-0.5 rounded border border-yellow-500/20">{String(winner.rank)}</div></div><div className="flex gap-1 items-center justify-center">{(winner.hand || []).map((c, ci) => (<div key={ci} className="w-12 h-18 bg-white rounded-lg flex flex-col items-center justify-center text-black shadow-xl transform" style={{ animation: `card-flip 0.6s cubic-bezier(0.4, 0, 0.2, 1) forwards`, animationDelay: `${0.2 + ci * 0.1}s`, opacity: 0 }}><span className="text-[14px] font-black absolute top-1 left-1.5 leading-none">{String(c.value)}</span><span className={`text-2xl ${c.suit === '♥' || c.suit === '♦' ? 'text-red-600' : 'text-black'}`}>{String(c.suit)}</span></div>))}</div></div>
                            ))}
                        </div>
                    </div>
                ) : ( <div className="flex flex-col items-center gap-4 animate-in fade-in duration-500 font-black uppercase">{phase === PHASES.IDLE ? (<div className="flex flex-col items-center gap-3"><div className="p-4 bg-white/5 rounded-full animate-pulse"><Target size={36} className="text-white/20"/></div><span className="text-white/40 tracking-[0.4em] text-lg font-black italic uppercase">ARENA IDLE</span></div>) : (<div className="flex flex-col items-center gap-2"><div className="flex items-center gap-2 text-cyan-400 animate-pulse mb-1"><MessageSquare size={16} /><span className="text-[xs] font-black tracking-[0.2em] uppercase">WAITING ON</span></div><span className="text-4xl font-black text-white tracking-tighter drop-shadow-lg uppercase">{String(players[activeIdx]?.name || "OPPONENT")}</span><div className="flex gap-1.5 mt-2">{[0, 1, 2].map(i => <div key={i} className="w-2 h-2 bg-cyan-400/40 rounded-full animate-bounce" style={{ animationDelay: `${i * 0.15}s` }} />)}</div></div>)}</div> )}
            </div>
          )}
        </div>
        <div className="h-[35%] w-full border-t border-white/10 p-2 flex flex-col overflow-hidden text-[12px] font-mono tracking-widest font-black uppercase bg-black/20">
            <div className="text-white/40 mb-1 flex items-center justify-between border-b border-white/5 pb-1 px-1 uppercase"><div className="flex items-center gap-1.5"><Eye size={12} className="text-[#fbbf24]"/> INTELLIGENCE</div><div className="flex items-center gap-1 text-emerald-500 animate-pulse text-[10px] font-black"><div className="w-1 h-1 bg-emerald-500 rounded-full" /> LIVE</div></div>
            <div className="flex-1 space-y-1 overflow-y-auto scrollbar-hide font-black p-0.5 uppercase">
                {(logs || []).map(l => (
                    <div key={l.id} className="animate-in slide-in-from-left duration-200 flex items-center gap-2 border-l-2 border-white/10 pl-2 py-1.5 hover:bg-white/5 transition-colors border-b border-white/5 uppercase">
                        <span className="text-white/20 text-[9px] font-black shrink-0 w-10 uppercase">{String(l.time)}</span> 
                        <div className="flex flex-wrap items-center gap-x-2 gap-y-1 font-black leading-none overflow-hidden uppercase">
                            <span className={`font-black uppercase text-[11px] px-1.5 py-0.5 rounded-sm shrink-0 ${
                                ...
                                l.type === 'join' ? 'bg-blue-500/20 text-blue-400' :
                                'bg-yellow-500/20 text-[#fbbf24]'
                            }`}>{String(l.name)}</span>
                            <span className="text-white/60 lowercase tracking-tight text-[11px] font-black uppercase">{String(l.action)}</span>
      ...
      <style>{`
          ...
          @keyframes deal-card { 
            0% { top: 40%; left: 50%; transform: translate(-50%, -50%) scale(0.2) rotate(720deg); opacity: 0; } 
            100% { opacity: 1; } 
          }
