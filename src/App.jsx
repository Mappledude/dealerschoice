...
const App = () => {
  ...
  const handleJoinRoom = () => {
      ...
      // Manual seeding for optimistic Seating
      setPlayers(prev => {
          const next = [...INITIAL_PLAYERS];
          next[0] = { ...userProfile, chips: buyInAmount, isSeated: true, isHero: true, hand: [], currentBet: 0, isFolded: false, isWinner: false };
          return next;
      });
        ...
        <main className="flex-1 flex flex-col p-12 overflow-y-auto relative z-10">
            ...
            {adminTab === ADMIN_TABS.TABLES && (
                <div className="flex flex-col gap-8 animate-in fade-in">
                    <div className="flex items-center justify-between border-b border-white/10 pb-6"><h2 className="text-2xl font-black uppercase tracking-widest text-white">Room Control</h2><button onClick={handleNuclearReset} className="p-4 px-8 bg-red-600 border border-red-500 rounded-2xl font-black uppercase text-xs hover:bg-red-600 hover:text-white transition-all">Nuclear Reset</button></div>
        ...
        <div className="relative w-full max-w-[1600px] aspect-[21/10] mx-auto flex items-center justify-center">
            ...
            <div className={`absolute top-[43%] left-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center z-30 pointer-events-none`}>
              <div className={`absolute left-1/2 -translate-x-1/2 transition-all duration-[800ms]`} style={{ top: potTransferring ? `${winnerPos.y - 43}vh` : '-4vw', left: potTransferring ? `${winnerPos.x - 50}vw` : '50%', transform: `translate(-50%, -50%) ${potTransferring ? 'scale(0.3)' : 'scale(1)'}`, opacity: potTransferring ? 0 : 1 }}>
                <div className="text-[4vw] font-black text-yellow-400 font-mono tracking-tighter leading-none">${Number(currentPotOnTable)}</div>
              </div>
                  ...
                  {userSeat && !userSeat.isFolded && phase !== PHASES.IDLE && (
                    <div className="relative flex items-center justify-center w-full h-full scale-[1.5]">
                      {(userSeat.hand || []).map((c, ci) => {
                        ...
                        const shouldHighlightHero = isWinnerHero && phase === PHASES.SHOWDOWN && (winning5Ids || []).includes(c.id);
