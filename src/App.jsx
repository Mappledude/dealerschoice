...
const SOCKET_URL =
  window.location.hostname === "localhost" ? "http://localhost:10000" : RENDER_URL;

/**
 * IMPORTANT: Keep a single socket instance across HMR / re-mounts to avoid:
 * - duplicate connections
 * - duplicate event handlers
 */
const getSocket = () => {
  if (!globalThis.__DEALERS_CHOICE_SOCKET__) {
    globalThis.__DEALERS_CHOICE_SOCKET__ = io(SOCKET_URL, {
      transports: ["websocket", "polling"],
      autoConnect: true,
    });
  }
  return globalThis.__DEALERS_CHOICE_SOCKET__;
};
...
const Seat = React.memo(function Seat({
  ...
  return (
    ...
    >
      {(isHero || isShowdown) &&
        !player.isFolded &&
        player.winProbability !== undefined &&
        phase !== PHASES.IDLE && (
          <div className="absolute top-[-50px] left-1/2 -translate-x-1/2 z-[300] flex flex-col items-center gap-1 animate-in fade-in zoom-in duration-300">
            <div className="bg-slate-900/80 backdrop-blur-xl border border-cyan-500/50 px-2 py-0.5 rounded-full flex items-center gap-1.5 shadow-[0_0_15px_rgba(34,211,238,0.3)]">
              ...
              <span className="text-[9px] font-black text-white font-mono">
                {Math.round(player.winProbability)}%
              </span>
            </div>
      ...
      {player.lastAction && !isActiveTurn && !isCollectingBets && (
        <div className="absolute top-[-30px] animate-bounce-short z-[200]">
          <span
            className={`text-[8px] font-black px-2 py-0.5 rounded shadow-lg uppercase border border-white/20 ${
      ...
      {player.currentBet > 0 && (
        <div
          className={`absolute z-[100] transition-all duration-700 ${
            isCollectingBets
              ? "animate-fling-to-pot opacity-0 scale-0"
              : "animate-bet-splash opacity-100"
          }`}
          style={{
      ...
      <div
        ...
        className={`relative z-50 flex flex-col items-center p-1.5 rounded-2xl border-2 bg-slate-900/95 backdrop-blur-md transition-all duration-300 min-w-[100px] md:min-w-[150px] shadow-2xl ${
          isActiveTurn
            ? "border-cyan-400 ring-4 ring-cyan-400/40 scale-105 shadow-[0_0_20px_rgba(34,211,238,0.3)]"
            : "border-white/10"
        } ${player.isWinner && isShowdown ? "border-yellow-400 animate-pulse-glow" : ""}`}
      >
            ...
            <div className="w-full h-full bg-black/40 rounded-full overflow-hidden shadow-inner">
              <div
                ...
                style={{
                  width: `${Math.max(
                    0,
                    Math.min(100, (timeRemaining / (turnTimerSeconds || 30)) * 100)
                  )}%`,
                }}
              />
            </div>
        ...
        <div className="flex flex-col items-center gap-0.5 w-full">
          <span className="text-[10px] md:text-[12px] font-black text-white/90 uppercase tracking-tight truncate w-full text-center px-2">
            {String(player.name || "Anon")}
          </span>
          <span
            className={`text-[11px] md:text-[14px] font-mono font-black ${
              player.chips === 0 ? "text-red-500 animate-pulse" : "text-emerald-400"
            }`}
          >
            ${Number(player.chips || 0).toLocaleString()}
          </span>
          {isActiveTurn && (
            <span className="text-[7px] text-cyan-400 font-black animate-pulse tracking-widest mt-0.5">
              THINKING...
            </span>
          )}
          {(highAward || lowAward) && (
            <span className="text-[8px] text-yellow-400 font-black tracking-widest mt-0.5">
              {highAward ? "HIGH" : ""} {lowAward ? "LOW" : ""}
            </span>
          )}
        </div>
      </div>
        ...
        <div className="relative z-10 flex items-center justify-center w-[12vw] h-[6vw] mt-4 overflow-visible">
          {player.hand.map((c, ci) => {
            ...
            const currentRotation =
              offset * (player.hand.length > 2 ? holeCardRotation * 0.6 : holeCardRotation);

            return (
              <div
                key={c.id || ci}
                className={`w-[5.5vw] md:w-[3vw] h-[8vw] md:h-[5vw] rounded-[4px] flex flex-col items-start p-[2px] border shadow-xl absolute transition-all duration-300 ${
                  isShowdown || isHero ? "bg-white text-black" : "bg-slate-800"
                } ${
                  isShowdown &&
                  player.isWinner &&
                  (winning5Ids || []).includes(c.id)
                    ? "ring-2 ring-yellow-400 scale-110 z-30 shadow-[0_0_20px_#fbbf24]"
                    : "border-white/20"
                }`}
                style={{
                  transform: `translateX(${
                    offset * (player.hand.length > 2 ? 1.4 : 2.5)
                  }vw) rotate(${currentRotation}deg) scale(${1.5 * currentCardScale})`,
...
const App = () => {
  ...
  // Room bb (avoid hardcoding 20 when server provides)
  const [roomBB, setRoomBB] = useState(20);

  // ---- Refs to avoid stale closures inside socket handlers ----
  const phaseRef = useRef(phase);
  const potAmountRef = useRef(potAmount);
  const userProfileRef = useRef(userProfile);
  const currentRoomIdRef = useRef(currentRoomId);
  const raiseInputRef = useRef(raiseInput);
  const highestBetRef = useRef(highestBet);
  const roomBBRef = useRef(roomBB);
  const turnTimerSecondsRef = useRef(turnTimerSeconds);

  useEffect(() => {
    phaseRef.current = phase;
  }, [phase]);
  useEffect(() => {
    potAmountRef.current = potAmount;
  }, [potAmount]);
  useEffect(() => {
  ...
  useEffect(() => {
    raiseInputRef.current = raiseInput;
  }, [raiseInput]);
  useEffect(() => {
    highestBetRef.current = highestBet;
  }, [highestBet]);
  useEffect(() => {
    roomBBRef.current = roomBB;
  }, [roomBB]);
  useEffect(() => {
    turnTimerSecondsRef.current = turnTimerSeconds;
  }, [turnTimerSeconds]);
  ...
  const minRaiseAllowed = useMemo(() => {
    const bb = Number(roomBB || 20);
    return Math.max(highestBet + bb, highestBet * 2);
  }, [highestBet, roomBB]);

  // Stable action emitter (reads latest roomId + raiseInput via refs)
  const handleAction = useCallback(
    (type, amt = 0) => {
      ...
      const useAmt =
        type === "RAISE" ? Number(amt || raiseInputRef.current || 0) : 0;

      socket.emit("playerAction", { roomId, type, amount: useAmt });
    },
    [socket]
  );
  ...
  const handleLogin = useCallback(() => {
    ...
    if (pass === "pass") {
      ...
      socket.emit("playerLogin", { password: pass });
    }
  }, [passwordInput, socket]);
  ...
  }, [selectedTableForJoin, userProfile, pendingVariantId, buyInAmount, socket]);
  ...
  }, [nuclearConfirm, socket]);

  const addBot = useCallback(() => {
    const roomId = currentRoomIdRef.current;
    if (!roomId) return;
    socket.emit("adminAddBot", { roomId });
  }, [socket]);

  // ---- Socket listeners: bind once, always read latest refs ----
  useEffect(() => {
    // Clear any existing handlers to prevent duplicates (HMR / re-mount)
    ...
    socket.on("roomUpdate", (d) => {
      ...
      // Pull bb / timer if server sends them (safe fallback)
      const nextBB = Number(d.bb ?? d?.table?.bb ?? d?.settings?.bb ?? d?.room?.bb ?? roomBBRef.current ?? 20);
      if (Number.isFinite(nextBB)) setRoomBB(nextBB);

      const nextTurnTimer = Number(d.turnTimerSeconds ?? d?.settings?.turnTimerSeconds ?? turnTimerSecondsRef.current ?? 30);
      if (Number.isFinite(nextTurnTimer) && nextTurnTimer > 0) setTurnTimerSeconds(nextTurnTimer);

      const prevPhase = phaseRef.current;
      const prevPot = potAmountRef.current;

      const phaseChanged =
        d.phase !== prevPhase &&
        prevPhase !== PHASES.IDLE &&
        d.phase !== PHASES.IDLE;

      const currentPotValue = Number(d.potData?.[0]?.amount || 0);
      const potIncreased = currentPotValue > Number(prevPot || 0);
      ...
      if (phaseChanged) {
        ...
        setTimeout(() => {
          ...
          if (potIncreased) {
            ...
            setTimeout(() => setPotAnimating(false), 600);
          }
        }, 1200);
      } else if (potIncreased && d.phase === prevPhase) {
        setPotAnimating(true);
        setTimeout(() => setPotAnimating(false), 600);
      }
      ...
      if (sVariant) {
        ...
        resolvedVariant =
          VARIANTS[vId] || { id: vId, name: typeof sVariant === "string" ? vId : sVariant.name };
      }
      ...
      // Auto-bump raiseInput to min raise when it's hero's turn
      const up = userProfileRef.current;
      if (d.activeIdx !== -1 && d.players?.[d.activeIdx]?.uid && d.players?.[d.activeIdx]?.uid === up?.uid) {
        const bb = nextBB || 20;
    ...
    return () => {
      // Cleanup all handlers for this component instance
      socket.off("roomUpdate");
      socket.off("lobbyUpdate");
      socket.off("profilesUpdate");
      socket.off("initialDataResponse");
    ...
    return (
      <div className="h-screen bg-[#06080c] flex items-center justify-center p-6 text-white font-black uppercase tracking-tighter">
        <div className="w-full max-w-[400px] p-8 md:p-12 bg-black/60 border border-white/10 rounded-3xl backdrop-blur-3xl shadow-2xl flex flex-col items-center gap-8 font-black">
          <div className="p-5 bg-white/5 rounded-full ring-1 ring-white/10 shadow-inner">
          ...
          <div className="w-full space-y-4">
            ...
            <input
              ...
              placeholder="••••••••"
              className="w-full bg-white/5 border border-white/10 p-5 rounded-2xl text-center tracking-[0.5em] text-[#fbbf24] outline-none text-xl font-black uppercase"
            />
          </div>
          <button
            onClick={handleLogin}
            className="w-full p-6 bg-[#fbbf24] text-black rounded-2xl hover:scale-[1.02] font-black text-lg transition-transform uppercase"
          >
            SIT AT TABLE
          </button>
        </div>
    ...
    return (
      <div className="h-screen bg-[#06080c] flex flex-col md:flex-row text-white font-black uppercase overflow-hidden font-black">
        <aside className="w-full md:w-64 border-b md:border-b-0 md:border-r border-white/10 p-4 md:p-8 flex flex-row md:flex-col gap-4 bg-black/20 shrink-0 font-black">
          <h2 className="text-[#fbbf24] tracking-widest hidden md:flex items-center gap-2 mb-4 font-black">
            <ShieldCheck size={20} /> ADMIN
          </h2>
          <button
            onClick={() => setAdminTab(ADMIN_TABS.PLAYERS)}
            className={`flex-1 md:flex-none p-3 md:p-4 rounded-xl text-xs md:text-sm transition-all font-black ${
          ...
          <button
            onClick={() => setAdminTab(ADMIN_TABS.TABLES)}
            className={`flex-1 md:flex-none p-3 md:p-4 rounded-xl text-xs md:text-sm transition-all font-black ${
          ...
          <button
            ...
            className={`hidden md:flex mt-auto p-4 rounded-xl items-center justify-center gap-2 border-2 transition-all font-black ${
              nuclearConfirm
          ...
          <button
            ...
            className="p-3 md:p-4 text-white/20 hover:text-white text-xs flex items-center gap-2 font-black"
          >
            <ArrowLeft size={16} />
          </button>
        </aside>
        ...
        <main className="flex-1 p-6 md:p-12 overflow-y-auto bg-black/40 font-black uppercase">
          {adminTab === ADMIN_TABS.PLAYERS ? (
            <div className="flex flex-col gap-8 animate-in fade-in">
              <h3 className="text-xl md:text-2xl tracking-widest border-l-4 border-[#fbbf24] pl-4 font-black">
                PLAYER REGISTRY
              </h3>
              <div className="bg-white/5 p-4 md:p-6 rounded-2xl grid grid-cols-1 md:grid-cols-3 gap-4 border border-white/10 shadow-xl font-black">
                <input
                  ...
                  placeholder="NAME"
                  className="bg-black/40 p-4 rounded-xl border border-white/10 outline-none focus:border-[#fbbf24] font-black uppercase"
                />
                <input
                ...
                <button
                  onClick={() =>
                    socket.emit("adminCreatePlayer", {
                      ...newPlayer,
                      uid: Math.random().toString(36).slice(2),
                    })
                  }
                  className="bg-[#fbbf24] text-black rounded-xl font-black p-4 transition-all"
                >
                  CREATE
                </button>
              </div>
              ...
              <div className="bg-white/5 rounded-2xl overflow-hidden border border-white/10 font-black">
                {(allProfiles || []).map((p) => (
                  <div
                    key={p.uid}
                    className="flex justify-between p-4 md:p-6 border-b border-white/5 hover:bg-white/5 transition-all font-black"
                  >
                    <span className="uppercase">
                      {String(p.name)}{" "}
                      <span className="text-white/20 ml-2">[{String(p.password)}]</span>
                    </span>
                    <div className="flex gap-4 items-center font-black">
                      <span className="text-emerald-400 font-mono text-sm md:text-lg tracking-tighter">
                        ${Number(p.chips || 0).toLocaleString()}
                      </span>
                      <button
                        onClick={() => {
                          const n = prompt("NEW WALLET", p.chips);
                          if (n) socket.emit("adminEditChips", { uid: p.uid, chips: Number(n) });
                        }}
                      >
                        <Edit3 size={18} className="text-cyan-400" />
                      </button>
                      <button onClick={() => socket.emit("adminDeletePlayer", p.uid)}>
            ...
            <div className="flex flex-col gap-8 animate-in fade-in font-black uppercase">
              <h3 className="text-xl md:text-2xl tracking-widest border-l-4 border-emerald-500 pl-4 font-black">
                ARENA CONTROL
              </h3>
              <div className="bg-white/5 p-4 md:p-6 rounded-2xl grid grid-cols-1 md:grid-cols-2 gap-4 border border-white/10 shadow-xl font-black">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 font-black">
                  <div className="space-y-1">
                    <span className="text-[9px] text-white/40 font-black">SB</span>
                    <input
                      value={newTable.sb}
                      type="number"
                      className="w-full bg-black/40 p-3 rounded-lg border border-white/10 font-black"
                      onChange={(e) => setNewTable({ ...newTable, sb: Number(e.target.value) })}
                    />
                  </div>
                  <div className="space-y-1">
                    <span className="text-[9px] text-white/40 font-black">BB</span>
                    <input
                      value={newTable.bb}
                      type="number"
                      className="w-full bg-black/40 p-3 rounded-lg border border-white/10 font-black"
                      onChange={(e) => setNewTable({ ...newTable, bb: Number(e.target.value) })}
                    />
                  </div>
                  <div className="space-y-1">
                    <span className="text-[9px] text-white/40 font-black">MIN</span>
                    <input
                      value={newTable.minBuy}
                      type="number"
                      className="w-full bg-black/40 p-3 rounded-lg border border-white/10 font-black"
                      onChange={(e) => setNewTable({ ...newTable, minBuy: Number(e.target.value) })}
                    />
                  </div>
                  <div className="space-y-1">
                    <span className="text-[9px] text-white/40 font-black">MAX</span>
                    <input
                      value={newTable.maxBuy}
                      type="number"
                      className="w-full bg-black/40 p-3 rounded-lg border border-white/10 font-black"
                      onChange={(e) => setNewTable({ ...newTable, maxBuy: Number(e.target.value) })}
                    />
                  </div>
                </div>
                <button
                  onClick={handleSpawnArena}
                  className="bg-emerald-600 rounded-xl font-black p-4 uppercase transition-all"
                >
                  SPAWN ARENA
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-black">
                {(activeTables || []).map((t) => (
                  <div
                    key={t.id}
                    className="bg-white/5 p-4 md:p-6 rounded-2xl flex justify-between items-center border border-white/10 hover:border-emerald-500/50 transition-all shadow-lg font-black uppercase"
                  >
                    <div>
                      <h4 className="text-[#fbbf24] text-base md:text-lg font-black truncate">
                        {String(t.name)}
                      </h4>
                      <p className="text-[10px] text-white/40 tracking-widest">
                        ${t.sb}/${t.bb} | {t.players?.filter(Boolean).length || 0}/10 SEATED
                      </p>
                    </div>
                    <button
                      onClick={() => socket.emit("adminDeleteRoom", t.id)}
                      className="bg-red-950/40 p-2 md:p-3 rounded-xl text-red-500 hover:bg-red-500 transition-all font-black"
                    >
                      TERMINATE
                    </button>
                  </div>
    ...
    return (
      <div className="h-screen bg-[#06080c] flex flex-col text-white font-black uppercase overflow-hidden">
        {selectedTableForJoin && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-md animate-in fade-in duration-300 px-6">
            <div className="w-full max-w-[400px] p-8 md:p-12 bg-slate-900 border border-[#fbbf24]/30 rounded-[2vw] shadow-2xl flex flex-col gap-10">
              <h3 className="text-2xl md:text-3xl text-center tracking-widest text-[#fbbf24] underline underline-offset-8 uppercase font-black">
                {String(selectedTableForJoin.name)}
              </h3>
              <div className="space-y-6 text-center uppercase">
                <div className="flex justify-between items-center text-[10px] text-white/40 tracking-widest font-black">
                  <span>BUY-IN AMOUNT</span>
                  <span className="text-emerald-400 text-xl md:text-2xl font-mono">
                    ${buyInAmount.toLocaleString()}
                  </span>
                </div>
                <input
                  type="range"
                  min={selectedTableForJoin.minBuy || 400}
                  max={selectedTableForJoin.maxBuy || 2000}
                  step={100}
                  value={buyInAmount}
                  onChange={(e) => setBuyInAmount(Number(e.target.value))}
                  className="w-full h-3 bg-white/10 rounded-lg appearance-none cursor-pointer accent-[#fbbf24]"
                />
              </div>
              <div className="flex gap-4">
                <button
                  onClick={() => setSelectedTableForJoin(null)}
                  className="flex-1 p-5 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 transition-all font-black uppercase"
                >
                  BACK
                </button>
                <button
                  onClick={joinRoom}
                  className="flex-2 p-5 bg-emerald-600 rounded-2xl shadow-lg hover:scale-105 active:scale-95 transition-all text-sm tracking-widest font-black uppercase"
                >
                  SIT DOWN
                </button>
              </div>
        ...
        <header className="h-20 border-b border-white/10 flex items-center justify-between px-6 md:px-12 bg-black/40 backdrop-blur-md shadow-xl z-50 shrink-0">
          <h2 className="tracking-widest md:tracking-[0.4em] text-sm md:text-xl flex items-center gap-4 font-black">
            <LayoutGrid className="text-[#fbbf24]" /> LOBBY
          </h2>
          <div className="flex items-center gap-6 md:gap-10">
            <div className="flex flex-col items-end uppercase">
              <span className="text-[8px] md:text-[10px] text-white/40 uppercase italic">
                ID: {String(userProfile?.name || "??")}
              </span>
              <span className="text-emerald-400 font-mono text-base md:text-2xl tracking-tighter font-black">
                ${Number(userProfile?.chips || 0).toLocaleString()}
              </span>
            </div>
            <button
              onClick={() => {
        ...
        <main className="flex-1 p-6 md:p-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10 overflow-y-auto bg-gradient-to-br from-transparent to-white/5">
          {(activeTables || []).map((t) => (
            <div
              key={t.id}
              className="p-8 bg-white/5 border border-white/5 rounded-3xl flex flex-col gap-6 shadow-2xl hover:border-[#fbbf24]/20 transition-all group relative overflow-hidden"
            >
              <h3 className="text-xl md:text-2xl tracking-widest text-white group-hover:text-[#fbbf24] transition-colors uppercase font-black">
                {String(t.name)}
              </h3>
              <div className="bg-black/60 p-4 md:p-6 rounded-2xl flex justify-between items-center border border-white/5 shadow-inner uppercase font-black">
                <div className="flex flex-col uppercase">
                  <span className="text-[8px] text-white/40 tracking-widest font-black">STAKES</span>
                  <span className="text-[#fbbf24] text-lg md:text-xl font-black">
                    ${t.sb}/${t.bb}
                  </span>
                </div>
                <div className="flex flex-col items-end">
                  <span className="text-[8px] text-white/40 tracking-widest font-black">SEATS</span>
                  <span className="text-white/80 font-mono text-sm md:text-base font-black">
                    {t.players?.filter((p) => p).length || 0}/10
                  </span>
                </div>
              </div>
  ...
  return (
    <div className="h-screen bg-[#06080c] text-white flex flex-col overflow-hidden relative font-black uppercase tracking-tighter">
        ...
        <div className="fixed inset-0 z-[5000] flex items-center justify-center bg-black/95 backdrop-blur-2xl p-6 font-black">
          <div className="w-full max-w-[400px] p-10 bg-slate-900 border-2 border-red-500 rounded-3xl text-center shadow-[0_0_100px_rgba(239,68,68,0.4)] font-black uppercase">
            ...
            <button
              ...
              className="w-full p-6 bg-emerald-600 text-white rounded-2xl shadow-xl animate-bounce font-black"
            >
              REBUY $1,000
            </button>
          </div>
      ...
      <header
        ...
        className="bg-[#0a0a0a] border-b border-white/10 flex items-center justify-between px-4 md:px-8 z-[80] shadow-2xl backdrop-blur-md shrink-0 font-black uppercase"
      >
        <div className="flex items-center gap-2">
          <div className="bg-white/5 px-3 py-1.5 rounded-xl border border-white/5 shadow-inner truncate font-black uppercase">
            <span className="text-[#fbbf24] text-[8px] md:text-[10px] tracking-widest font-black uppercase italic text-opacity-70">
              This Hand:
            </span>
            <span className="text-white ml-2 text-[10px] md:text-xs font-black">
              {activeVariant?.name || "Hold'em"}
            </span>
          </div>
          <button
            onClick={() => setShowLayoutControls(!showLayoutControls)}
            className={`p-2 rounded-lg transition-all font-black uppercase ${
              showLayoutControls
          ...
          <div className="absolute top-16 left-4 bg-black/95 border border-white/10 p-6 rounded-2xl shadow-2xl z-[1000] flex flex-col gap-5 min-w-[280px] max-h-[80vh] overflow-y-auto scrollbar-hide animate-in slide-in-from-top-4 backdrop-blur-xl font-black">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full accent-[#fbbf24] h-1 bg-white/10 rounded-full appearance-none"
                />
              </div>
              <div className="space-y-1">
            ...
            <div className="space-y-1">
              <span className="text-[8px] text-white/40 uppercase font-black">
                TABLE ZOOM ({Math.round(tableZoom * 100)}%)
              </span>
              <input
                type="range"
                min="0.5"
            ...
            <div className="space-y-1">
              <span className="text-[8px] text-white/40 uppercase text-emerald-400 font-black">
                HERO CARD SIZE ({Math.round(heroCardScale * 100)}%)
              </span>
              <input
                type="range"
                min="0.5"
                ...
                step="0.1"
                value={heroCardScale}
                onChange={(e) => setHeroCardScale(Number(e.target.value))}
                className="w-full accent-emerald-500 h-1 bg-white/10 rounded-full appearance-none"
              />
            </div>

            <div className="space-y-1">
              <span className="text-[8px] text-white/40 uppercase text-blue-400 font-black">
                COMMUNITY CARD SIZE ({Math.round(communityCardScale * 100)}%)
              </span>
              <input
                type="range"
                min="0.5"
                max="4.0"
                step="0.1"
                value={communityCardScale}
                onChange={(e) => setCommunityCardScale(Number(e.target.value))}
                className="w-full accent-blue-500 h-1 bg-white/10 rounded-full appearance-none"
              />
            </div>

            <div className="space-y-1">
              <span className="text-[8px] text-white/40 uppercase text-orange-400 font-black">
                PLAYER BADGE OFFSET ({playerBadgeOffset}px)
              </span>
              <input
                type="range"
                min="-200"
                max="200"
                step="1"
                value={playerBadgeOffset}
                onChange={(e) => setPlayerBadgeOffset(Number(e.target.value))}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <span className="text-[8px] text-white/40 uppercase text-purple-400 font-black">
                  STRENGTH Y ({handStrengthYOffset}px)
                </span>
                <input
                  type="range"
                  min="-200"
                  max="200"
                  step="1"
                  value={handStrengthYOffset}
                  onChange={(e) => setHandStrengthYOffset(Number(e.target.value))}
                  className="w-full accent-purple-500 h-1 bg-white/10 rounded-full appearance-none"
                />
              </div>
              <div className="space-y-1">
                <span className="text-[8px] text-white/40 uppercase text-purple-400 font-black">
                  STRENGTH X ({handStrengthXOffset}px)
                </span>
                <input
                  type="range"
                  min="-200"
                  max="200"
                  step="1"
                  value={handStrengthXOffset}
                  onChange={(e) => setHandStrengthXOffset(Number(e.target.value))}
                  className="w-full accent-purple-500 h-1 bg-white/10 rounded-full appearance-none"
                />
              </div>
            </div>
            ...
            <div className="space-y-1">
              <span className="text-[8px] text-white/40 uppercase text-indigo-400 font-black">
                HOLE CARD ROTATION ({holeCardRotation}°)
              </span>
              <input
                type="range"
                min="0"
                max="45"
                step="1"
                value={holeCardRotation}
                onChange={(e) => setHoleCardRotation(Number(e.target.value))}
            ...
            <button
              ...
              className="bg-[#fbbf24] text-black font-black py-2 rounded-lg text-[10px] tracking-widest uppercase"
            >
              CLOSE
            </button>
          </div>
        ...
        <div className="bg-white/5 border border-white/10 px-4 py-1.5 rounded-xl flex items-center gap-4 shadow-inner font-black uppercase">
          <span className="hidden sm:inline text-white/40 text-[9px] tracking-widest uppercase font-black">
            On my deal:
          </span>
          <select
            ...
            onChange={(e) => {
              ...
              setPendingVariantId(v);
              if (userProfile?.uid) socket.emit("updatePlayerSettings", { uid: userProfile.uid, pendingVariant: v });
            }}
            className="bg-transparent text-[#fbbf24] outline-none text-xs cursor-pointer font-black"
          >
            {Object.entries(VARIANTS).map(([k, v]) => (
              <option key={k} value={k} className="bg-slate-900 font-black">
                {v.name}
              </option>
            ))}
          </select>
        ...
        <div className="flex gap-2 font-black uppercase items-center">
          ...
          <button
            onClick={addBot}
            className="text-indigo-400 p-2 bg-white/5 border border-white/10 rounded-xl hover:bg-indigo-400/20 font-black"
            title="Bot"
          >
            <Bot size={20} />
          </button>
          <button
            ...
            className="text-red-500 p-2 bg-white/5 border border-white/10 rounded-xl hover:bg-red-500/20 font-black"
          >
            <LogOut size={20} />
          </button>
        </div>
      ...
      <main className="flex-1 flex flex-col items-center justify-center relative bg-gradient-to-b from-emerald-950/20 to-transparent overflow-hidden px-2 py-2 font-black uppercase">
        <div
          style={{
            ...
            maxHeight: `calc(100vh - ${headerHeight + footerHeight + 10}px)`,
          }}
          className="relative w-full max-w-[1400px] aspect-[21/10] flex items-center justify-center h-full transition-transform duration-300 ease-out origin-center font-black"
        >
          ...
          <div className="absolute top-[48%] left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-30 pointer-events-none w-full h-full justify-center">
            {!potTransferring && (
              <div className={`flex flex-col items-center transition-all duration-300 ${potAnimating ? "scale-110" : "scale-100"}`}>
                <div
                  className={`text-[6vw] md:text-[5vw] font-black text-yellow-400 font-mono tracking-tighter drop-shadow-[0_4px_20px_rgba(0,0,0,0.8)] ${
                    potAnimating ? "animate-pot-pulse" : ""
                  }`}
                >
            ...
            {["HOLDEM", "OMAHA", "PINEAPPLE", "HILOW", "MUFLIS", "REDSBLACKS"].includes(activeVariant?.id) && (
              <div
                className="flex gap-2 md:gap-4 mt-6 md:mt-12 font-black uppercase transition-transform duration-300"
                style={{ transform: `scale(${communityCardScale})` }}
              >
                {(community || []).map((c, j) => (
                  <div
                    key={c.id || j}
                    className={`w-[6vw] md:w-[3vw] h-[9vw] md:h-[5vw] rounded-[4px] border bg-white flex flex-col items-center justify-center text-black font-black transition-all duration-300 ${
                      winning5Ids?.includes(c.id)
                        ? "ring-4 ring-yellow-400 scale-110 z-30 shadow-[0_0_40px_rgba(251,191,36,0.6)]"
                        : "border-white/20 shadow-2xl"
                    }`}
                  >
                    <span className="text-[14px] md:text-[0.9vw] font-black">{String(c.value)}</span>
                    <span
                      className={`text-[18px] md:text-[2.2vw] font-black ${
                        c.suit === "♥" || c.suit === "♦" ? "text-red-600" : "text-black"
                      }`}
                    >
                      {String(c.suit)}
      ...
      <footer
        ...
        className="bg-black/95 backdrop-blur-3xl border-t border-white/10 flex z-[100] shadow-[0_-10px_40px_rgba(0,0,0,0.5)] shrink-0 font-black uppercase overflow-hidden"
      >
        <div className="flex w-[35%] border-r border-white/10 p-2 flex-col overflow-hidden text-[12px] font-mono tracking-widest font-black uppercase">
          <div className="text-white/40 mb-1 flex items-center justify-between border-b border-white/5 pb-1 px-1 uppercase">
            <div className="flex items-center gap-1.5">
              <Eye size={12} className="text-[#fbbf24]" /> INTELLIGENCE
            </div>
            <div className="flex items-center gap-1 text-emerald-500 animate-pulse text-[10px]">
          ...
          <div className="flex-1 space-y-1 overflow-y-auto scrollbar-hide font-black p-0.5">
            {(logs || []).map((l) => (
              <div
                key={l.id}
                className="animate-in slide-in-from-left duration-200 flex items-center gap-2 border-l-2 border-white/10 pl-2 py-0.5 hover:bg-white/5 transition-colors border-b border-white/5"
              >
                ...
                <div className="flex items-center gap-x-1.5 font-black leading-none overflow-hidden">
                  <span
                    className={`font-black uppercase text-[11px] px-1.5 py-0.5 rounded-sm shrink-0 ${
                      l.type === "win"
                  ...
                  <span className="text-white/60 lowercase tracking-tight text-[11px] font-black truncate">
                    {String(l.action)}
                  </span>
                  ...
                  {l.type === "win" && l.cards && (
                    <div className="flex gap-0.5 ml-1 shrink-0 scale-90 origin-left">
                      {l.cards.map((c, ci) => (
                        <div
                          key={ci}
                          className={`flex items-center px-1 rounded-sm text-[8px] font-bold ${
                            c.suit === "♥" || c.suit === "♦"
          ...
          {activeIdx === heroIdx && phase !== PHASES.SHOWDOWN && phase !== PHASES.IDLE && heroPlayerObj ? (
            <div className="flex flex-col gap-3 md:gap-5 animate-in slide-in-from-bottom duration-500 items-center w-full font-black uppercase">
              <div className="absolute top-2 right-4 animate-in slide-in-from-right duration-500">
                <div className="flex flex-col items-end">
                  <span className="text-[7px] text-white/40 tracking-[0.2em] font-black uppercase italic">
                    Current Hand
                  </span>
              ...
              <div className="flex gap-2 w-full max-w-[600px] font-black uppercase">
                <button
                  onClick={() => handleAction("RAISE", highestBet + Math.floor(potAmount * 0.5))}
                  className="flex-1 py-2 bg-white/5 border border-white/10 rounded-xl text-[10px] md:text-[12px] hover:bg-white/20 transition-all font-black"
                >
                  1/2 POT
                </button>
                <button
                  onClick={() => handleAction("RAISE", highestBet + potAmount)}
                  className="flex-1 py-2 bg-white/5 border border-white/10 rounded-xl text-[10px] md:text-[12px] hover:bg-white/20 transition-all font-black"
                >
                  POT
                </button>
                <button
                  onClick={() => handleAction("RAISE", heroPlayerObj.chips + heroPlayerObj.currentBet)}
                  className="flex-1 py-2 bg-red-900/30 border border-red-500/50 rounded-xl text-[10px] md:text-[12px] text-red-500 hover:bg-red-600 hover:text-white transition-all font-black"
                >
                  ALL-IN
                </button>
              </div>
              ...
              <div className="flex gap-3 md:gap-6 w-full items-center justify-center font-black">
                <button
                  ...
                  className="w-16 md:w-32 h-14 md:h-16 bg-red-950/60 border-2 border-red-500/50 rounded-2xl tracking-[0.2em] hover:brightness-125 transition-all font-black text-xs shadow-xl"
                >
                  FOLD
                </button>
                ...
                <button
                  ...
                  className="flex-1 max-w-[360px] h-14 md:h-16 bg-indigo-900/60 border-2 border-indigo-400/50 rounded-2xl text-sm md:text-xl tracking-[0.3em] hover:brightness-125 font-black shadow-xl shadow-indigo-900/20"
                >
                  {highestBet > heroPlayerObj.currentBet
                    ? `CALL $${(highestBet - heroPlayerObj.currentBet).toLocaleString()}`
                    : "CHECK"}
                </button>
                ...
                <div className="flex gap-2 items-center bg-black/60 border border-white/10 p-1 md:p-2 rounded-2xl shadow-inner min-w-[120px] md:min-w-[320px] font-black uppercase">
                  <div className="flex items-center bg-black/40 px-3 md:px-5 rounded-xl border border-white/5 h-12 md:h-14">
                      ...
                      onChange={(e) =>
                        setRaiseInput(
                          Math.min(
                            ...
                            Math.max(minRaiseAllowed, Number(e.target.value))
                          )
                        )
                      }
                      className="w-10 md:w-28 bg-transparent text-center font-mono text-sm md:text-2xl text-[#fbbf24] outline-none font-black"
                    />
                  </div>
                  <button
                    onClick={() => handleAction("RAISE", raiseInput)}
                    className="flex-1 h-12 md:h-14 bg-emerald-600/60 border border-emerald-400/50 rounded-xl flex items-center justify-center hover:brightness-125 font-black uppercase text-xs md:text-xl shadow-xl shadow-emerald-900/20"
                  >
            ...
            <div className="flex flex-col items-center justify-center h-full relative font-black uppercase">
              {showdownWinners && showdownWinners.length > 0 ? (
                <div className="flex flex-col items-center gap-3 w-full h-full justify-center relative">
                  <div className="flex flex-wrap gap-4 items-center justify-center w-full overflow-y-auto px-4 py-2">
                    {showdownWinners.map((winner, idx) => (
                      <div
                        key={idx}
                        className="flex flex-col items-center gap-2 bg-black/60 p-4 rounded-[2rem] border-2 border-yellow-500/40 shadow-[0_0_50px_rgba(251,191,36,0.3)] min-w-[300px] animate-showdown-card-pop relative overflow-hidden"
                      ...
                      >
                        ...
                        <div className="flex flex-col items-center z-10 mb-1">
                          <div className="text-[#fbbf24] font-black text-3xl md:text-5xl tracking-tighter mb-1 drop-shadow-[0_4px_8px_rgba(0,0,0,0.8)] uppercase animate-pulse">
                            {winner.name}
                          </div>
                          <div className="flex items-center gap-3">
                            <div className="text-emerald-400 font-mono text-xl md:text-3xl font-black">
                              +${(winner.amount || 0).toLocaleString()}
                            </div>
                            <div className="text-white/80 text-[10px] md:text-xs tracking-[0.2em] uppercase font-black px-3 py-0.5 bg-yellow-600/20 rounded-full border border-yellow-500/30 shadow-inner">
                              {winner.rank}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex gap-1 z-10">
                          {(winner.hand || []).map((c, ci) => (
                            <div
                              key={ci}
                              className="w-12 md:w-16 h-18 md:h-24 bg-white rounded-lg flex flex-col items-center justify-center text-black shadow-[0_8px_16px_rgba(0,0,0,0.5)] transform font-black transition-all hover:scale-110 active:scale-125 cursor-default relative"
                              style={{
                                animation: `card-flip 0.7s cubic-bezier(0.4, 0, 0.2, 1) forwards`,
                                animationDelay: `${0.4 + ci * 0.15}s`,
                                opacity: 0,
                              }}
                            >
                              <span className="text-[12px] md:text-xl font-black absolute top-1 left-1.5 leading-none">
                                {String(c.value)}
                              </span>
                              <span
                              ...
                              <span className="text-[12px] md:text-xl font-black absolute bottom-1 right-1.5 rotate-180 leading-none">
                                {String(c.value)}
                              </span>
                            </div>
                          ))}
                  ...
                  {phase === PHASES.IDLE ? (
                    <div className="flex flex-col items-center gap-3">
                      <div className="p-4 bg-white/5 rounded-full animate-pulse">
                        <Target size={36} className="text-white/20" />
                      </div>
                      <span className="text-white/40 tracking-[0.4em] text-xs md:text-lg font-black italic">
                        ARENA IDLE
                      </span>
                    </div>
                  ) : (
                    <div className="flex flex-col items-center gap-2">
                      <div className="flex items-center gap-2 text-cyan-400 animate-pulse mb-1">
                        ...
                        <span className="text-[10px] md:text-[xs] font-black tracking-[0.2em]">
                          WAITING ON
                        </span>
                      </div>
                      <span className="text-2xl md:text-4xl font-black text-white tracking-tighter drop-shadow-lg">
                        {players[activeIdx]?.name || "OPPONENT"}
                      </span>
                      <div className="flex gap-1.5 mt-2">
                        {[0, 1, 2].map((i) => (
                          <div
                            key={i}
                            className="w-2 h-2 bg-cyan-400/40 rounded-full animate-bounce"
      ...
      <style>{`
        ...
        .bg-glimmer {
          background: linear-gradient(135deg, #fff 0%, #fff 40%, #fbbf24 50%, #fff 60%, #fff 100%);
