...
const VARIANT_COLORS = {
  HOLDEM: '#22d3ee', OMAHA: '#a855f7', PINEAPPLE: '#eab308',
  MUFLIS: '#39FF14', HILOW: '#ff007f', REDSBLACKS: '#ff0000'
};
...
const VARIANTS = { 
  ...
  OMAHA: { id: 'OMAHA', name: 'Omaha', rules: ["Each player gets 4 hole cards.", "You MUST use EXACTLY 2 hole cards and 3 community cards."] }, 
  ...
  HILOW: { id: 'HILOW', name: 'Hi-Low Split', rules: ["Pot is split 50/50 between the High hand and the Low hand."] }, 
  REDSBLACKS: { id: 'REDSBLACKS', name: 'Reds & Blacks', rules: ["4 hole cards dealt.", "Dynamic wildcards based on suit color."] }
};
...
const DashTimer = ({ timeRemaining }) => {
  ...
  return (
    <div className="w-24 md:w-32 h-1.5 bg-white/10 rounded-full mt-1 overflow-hidden">
      <div className="h-full bg-current transition-all duration-1000 linear" style={{ width: `${percentage}%`, color, boxShadow: `0 0 10px ${color}` }} />
    </div>
  );
};

const Seat = ({ player, displayPos, phase, isActiveTurn, isDealer, visuals, formatRank, isHero }) => {
    if (!player) return null;
    const isFolded = player.isFolded;
    const shouldShowCards = isHero || phase === PHASES.SHOWDOWN;
    ...
    return (
        <div style={{ left: `${displayPos.x}%`, top: `${displayPos.y}%` }} className={`absolute -translate-x-1/2 -translate-y-1/2 flex flex-col items-center transition-all duration-500 ${isHero ? 'z-[100]' : 'z-20'}`}>
            {player.hand && player.hand.length > 0 && (
                <div className={`flex relative -mb-8 z-30 h-[10.5vw] lg:h-[8vh] ${isFolded ? 'opacity-30 grayscale scale-90' : ''}`}>
                    {player.hand.map((c, ci) => {
                        const offset = ci - (player.hand.length - 1) / 2;
                        return (
                          <div key={ci} className={`w-[7.5vw] lg:w-[5.5vh] h-[10.5vw] lg:h-[8vh] rounded-lg border absolute transition-all duration-300 shadow-2xl ${shouldShowCards ? 'bg-white' : 'card-back-diamond border-white/20'}`} 
                               style={{ transform: `translateX(${offset * (isHero ? visuals.heroCardSpread : 2.5)}vh)` }}>
                               {shouldShowCards && (
                                   <div className={`flex flex-col items-start p-1 ${c.suit === '♥' || c.suit === '♦' ? 'text-red-600' : 'text-slate-900'}`}>
                                       <span className="text-[10px] font-black leading-tight">{String(c.value)}</span>
                                       <span className="text-[14px] leading-tight">{String(c.suit)}</span>
                                   </div>
                               )}
                          </div>
                        );
            ...
            <div className={`relative z-[90] flex flex-col items-center p-2 lg:p-3 rounded-xl border transition-all duration-300 min-w-[120px] lg:min-w-[14vh] backdrop-blur-xl ${isActiveTurn ? 'border-white ring-4 ring-white/20 bg-slate-800 shadow-[0_0_40px_rgba(255,255,255,0.2)]' : 'border-white/10 bg-black/80'}`}>
                {isDealer && (
                    <div className="absolute bottom-[2px] left-1/2 -translate-x-1/2 z-[110] pointer-events-none">
                      <div className="w-2.5 h-2.5 bg-red-600 rounded-full border border-white shadow-[0_0_8px_red]" />
                    </div>
                )}
                <div className={`flex flex-col items-center w-full transition-all ${isFolded ? 'opacity-40 grayscale' : ''}`}>
                    <span className="text-[12px] lg:text-[1.6vh] font-black text-white uppercase truncate max-w-[80px]">{String(player.name)}</span>
                    <span className="text-[14px] lg:text-[2.2vh] font-mono font-black text-emerald-400">${Number(player.chips).toLocaleString()}</span>
                    {(phase === PHASES.SHOWDOWN || isHero) && player.strength && <span className="text-[8px] text-cyan-400 font-bold uppercase">{formatRank(String(player.strength))}</span>}
                </div>
                {isActiveTurn && <DashTimer timeRemaining={24} />}
            </div>
            {player.currentBet > 0 && (
                <div className="mt-2 bg-black/80 border border-yellow-500/50 px-2 py-0.5 rounded-full text-[10px] text-yellow-400 font-black font-mono shadow-xl animate-in zoom-in duration-300">
...
const App = () => {
  ...
  const visuals = { heroCardScale: 2.0, heroCardY: -54, heroCardSpread: 3.0, tableZoom: 0.9, footerHeight: 250, holeCardFan: 35 };
  ...
  const handleAction = useCallback((type, amt = 0) => {
    const finalAmount = amt !== 0 ? Number(amt) : Number(raiseInput);
    if (currentRoomId && !isNaN(finalAmount)) {
      socket.emit('playerAction', { roomId: currentRoomId, type, amount: type === 'RAISE' ? finalAmount : 0 });
    }
  ...
  const copyActivityToClipboard = () => {
    let logTextExport = logs.map(l => `[${String(l.name)}] ${String(l.action)}`).join('\n');
  ...
  const formatRank = (rank) => {
    ...
    let formatted = String(rank).toUpperCase();
    if (activeVariant.id === 'REDSBLACKS') {
        formatted = formatted.replace(/\(NATURAL\)/g, "🍀").replace(/\(JOKER\)/g, "🃏");
    } else {
        formatted = formatted.replace(/\(NATURAL\)/g, "").replace(/\(JOKER\)/g, "");
    }
    return formatted;
  ...
  useEffect(() => {
    socket.on('roomUpdate', d => {
        ...
        if (d.phase !== phaseRef.current) { if (d.phase === PHASES.PRE_FLOP) { setHandAttention(true); setTimeout(()=>setHandAttention(false), 3000); } phaseRef.current = d.phase; }
    });
    ...
    return () => { socket.off('roomUpdate'); socket.off('initialDataResponse'); socket.off('log'); socket.off('loginSuccess'); socket.off('profilesUpdate'); socket.off('lobbyUpdate'); };
  }, []);
            ...
            {handHistory.map(hand => (
                <div key={hand.id} className="border border-white/10 rounded-xl overflow-hidden bg-black/40">
                    <button onClick={() => { const n = new Set(expandedHands); if(n.has(hand.id)) n.delete(hand.id); else n.add(hand.id); setExpandedHands(n); }} className="w-full p-3 flex flex-col text-left transition-all hover:bg-white/5">
                        <div className="flex justify-between w-full uppercase"><span className="text-[9px] text-indigo-400 tracking-widest">{String(hand.variant)} HAND</span><ChevronRightIcon size={12} className={expandedHands.has(hand.id) ? 'rotate-90' : ''} /></div>
                        <div className="text-[11px] text-white/90 uppercase">{hand.winner ? <><span className={getNeonNameColor(hand.winner)}>{String(hand.winner)}</span> WON ${String(hand.amount)}</> : <span className="text-white/40 italic">In Progress...</span>}</div>
                    </button>
                    {expandedHands.has(hand.id) && (
                        <div className="px-3 pb-3 border-t border-white/5 bg-black/20 space-y-1 pt-2">
                            {hand.events.map((ev, i) => (<div key={i} className={`text-[9px] border-l-2 pl-2 ${ev.type==='win'?'border-emerald-500':ev.type==='fold'?'border-red-500':'border-indigo-500/30'}`}><span className={getNeonNameColor(ev.name)}>{String(ev.name)}</span>: <span className="text-white/80 uppercase">{String(ev.action)}</span></div>))}
                        </div>
                    )}
                </div>
            ))}
  ...
  if (currentView === VIEWS.ADMIN) return (
    <div className="h-screen bg-[#06080c] flex flex-col md:flex-row text-white uppercase font-black overflow-hidden pt-[env(safe-area-inset-top)]">
        <aside className="w-full md:w-64 border-b md:border-r border-white/10 p-8 flex flex-col gap-4 bg-black/20">
            <h2 className="text-[#fbbf24] flex items-center gap-2 mb-4 font-black uppercase"><ShieldCheck size={20}/> ADMIN</h2>
            ...
            <button onClick={()=>{ if(!nuclearConfirm){ setNuclearConfirm(true); setTimeout(()=>setNuclearConfirm(false),3000); return; } socket.emit('adminNuclearReset'); setNuclearConfirm(false); }} className={`p-3 rounded-xl border-2 transition-all uppercase ${nuclearConfirm ? 'bg-red-600 border-white text-white' : 'bg-white/5 text-red-500 border-red-500/20'}`}><Bomb size={14}/> {nuclearConfirm ? 'CONFIRM' : 'NUCLEAR'}</button>
            <button onClick={()=>{setCurrentView(VIEWS.LOBBY); socket.emit('getInitialData');}} className="p-3 rounded-xl bg-cyan-600 text-black uppercase">BACK TO LOBBY</button>
        </aside>
        <main className="flex-1 p-12 overflow-y-auto">
            {adminTab === ADMIN_TABS.PLAYERS ? (
                <div className="flex flex-col gap-8">
                    <h3 className="text-xl border-l-4 border-[#fbbf24] pl-4 uppercase">Player Registry</h3>
                    <div className="bg-white/5 p-6 rounded-2xl grid grid-cols-1 md:grid-cols-3 gap-4 border border-white/10">
                        ...
                        <button onClick={() => { socket.emit('adminCreatePlayer', { ...newPlayer }); setNewPlayer({ name: '', chips: 10000, password: '' }); }} className="bg-[#fbbf24] text-black rounded-xl p-3 font-black">CREATE PLAYER</button>
                    </div>
                    <div className="bg-white/5 rounded-2xl overflow-hidden border border-white/10">
                        {allProfiles.map(p => (
                            <div key={p.uid} className="flex justify-between p-4 border-b border-white/5 items-center hover:bg-white/5">
                                <span className="font-black">{String(p.name)}</span>
                                <div className="flex gap-4 items-center">
                                    ...
                                    <button onClick={()=>{const n = prompt("NEW PASSWORD", p.password); if(n!==null) socket.emit('adminUpdatePlayer', {uid: p.uid, password: n})}} className="text-amber-400"><Key size={16}/></button>
                ...
                <div className="flex flex-col gap-8">
                    <h3 className="text-xl border-l-4 border-emerald-500 pl-4 uppercase">Arena Control</h3>
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        {activeTables.map(t => (
                            <div key={t.id} className="bg-white/5 p-4 rounded-2xl flex justify-between items-center border border-white/10">
                              <div><h4 className="text-[#fbbf24] font-black">{String(t.name)}</h4><p className="text-[10px] text-white/40 tracking-widest uppercase">${t.sb}/${t.bb}</p></div>
                              <button onClick={()=>socket.emit('adminDeleteRoom', t.id)} className="bg-red-950/40 px-3 py-2 rounded-xl text-red-500 text-xs uppercase font-black">TERMINATE</button>
                            </div>
                        ))}
                    </div>
                </div>
        ...
        {selectedTableForJoin && (
            <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-xl px-6">
              <div className="w-full max-w-[400px] p-8 bg-slate-900 border border-emerald-500/30 rounded-3xl shadow-[0_0_50px_rgba(16,185,129,0.2)] flex flex-col gap-10">
                <h3 className="text-3xl text-center text-emerald-400 font-black uppercase">{String(selectedTableForJoin.name)}</h3>
                <div className="space-y-4 font-black text-center uppercase">
                  ...
                  <input type="range" min={selectedTableForJoin.minBuy || 5} max={Math.min(selectedTableForJoin.maxBuy || 10000, userProfile?.chips || 10000)} step={1} value={buyInAmount} onChange={(e) => setBuyInAmount(Number(e.target.value))} className="w-full h-2 bg-white/10 rounded-lg appearance-none cursor-pointer accent-emerald-500" />
                </div>
                <div className="flex gap-4"><button onClick={()=>setSelectedTableForJoin(null)} className="flex-1 p-4 bg-white/5 border border-white/10 rounded-xl font-black text-xs uppercase">CANCEL</button><button onClick={() => { socket.emit('joinRoom', { roomId: selectedTableForJoin.id, profile: userProfile, buyIn: buyInAmount }, (res) => { if(res.status==='ok'){ setCurrentRoomId(selectedTableForJoin.id); setCurrentView(VIEWS.GAME); setSelectedTableForJoin(null); } }); }} className="flex-2 p-4 bg-emerald-600 rounded-xl shadow-lg text-xs font-black uppercase">CONFIRM SEAT</button></div>
              </div>
            </div>
        )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
                {activeTables.map((t) => (
                    <div key={t.id} className="group relative bg-slate-900/40 border border-white/5 rounded-3xl flex flex-col p-8 shadow-2xl transition-all hover:border-emerald-500/30 hover:bg-slate-900/60 font-black overflow-hidden text-left">
                      <h3 className="text-3xl text-white font-black tracking-tight mb-4 uppercase truncate">{String(t.name)}</h3>
                      <div className="flex flex-col gap-4 mb-6">
                        <div className="flex justify-between items-end border-b border-white/5 pb-2"><div className="flex flex-col"><span className="text-[8px] text-white/30 tracking-widest uppercase">STAKES</span><span className="text-emerald-400 text-xl lg:text-2xl font-mono leading-none">${t.sb}/${t.bb}</span></div><div className="flex flex-col items-end"><span className="text-[8px] text-white/30 tracking-widest uppercase">BUY-IN</span><span className="text-white/80 text-sm lg:text-lg font-mono leading-none">${t.minBuy}-${t.maxBuy}</span></div></div>
                        <div className="flex flex-col gap-2">
                          ...
                          <div className="flex flex-wrap gap-1.5 min-h-[40px] p-2 bg-black/40 rounded-xl border border-white/5">{(t.players || []).filter(p=>p).map((p, idx) => (<span key={idx} className="bg-white/5 border border-white/10 px-2 py-0.5 rounded text-[8px] text-white/80 font-black flex items-center gap-1 uppercase">{p.isBot && <Bot size={8} className="text-indigo-400" />}{String(p.name).toUpperCase()}</span>))}</div>
                        </div>
                      </div>
                      <button onClick={()=>{ setSelectedTableForJoin(t); setBuyInAmount(t.maxBuy); }} className="w-full py-6 bg-emerald-600 rounded-2xl text-xs font-black shadow-lg transition-all active:scale-95 group-hover:bg-emerald-500 uppercase">JOIN ARENA <ChevronRight size={14}/></button>
                    </div>
                ))}
            </div>
        </main>
    ...
    <div style={{ height: 'calc(var(--vh, 1vh) * 100)' }} className="bg-[#1c1917] text-white flex flex-col overflow-hidden relative font-black uppercase tracking-tighter select-none">
      ...
      <div className="flex-1 flex flex-row overflow-hidden relative">
        ...
        <main className="flex-1 flex flex-col items-center justify-center relative floor-executive-parquet overflow-hidden font-black uppercase text-center">
            {heroPlayerObj && !heroPlayerObj.isFolded && phase !== PHASES.IDLE && (
              <><div className="absolute top-6 left-6 z-[90] flex flex-col items-start pointer-events-none"><span className="text-[8px] md:text-[10px] text-white/30 tracking-[0.3em] font-normal mb-1 uppercase">Low Strength</span><span className="text-white text-[14px] lg:text-[2.5vh] font-normal tracking-tighter">{(phase === PHASES.PRE_FLOP || !heroPlayerObj?.lowStrength) ? "-" : formatRank(String(heroPlayerObj.lowStrength))}</span><span className="text-white text-[11px] lg:text-[1.5vh] font-normal mt-1">{Math.round(heroPlayerObj?.lowWinProbability || 0)}% WIN PROB</span></div><div className="absolute top-6 right-6 z-[90] flex flex-col items-end pointer-events-none"><span className="text-[8px] md:text-[10px] text-white/30 tracking-[0.3em] font-normal mb-1 uppercase">Strength</span><span className="text-white text-[14px] lg:text-[2.5vh] font-normal tracking-tighter">{(phase === PHASES.PRE_FLOP || !heroPlayerObj?.strength) ? "-" : formatRank(String(heroPlayerObj.strength))}</span><span className="text-white text-[11px] lg:text-[1.5vh] font-normal mt-1">{Math.round(heroPlayerObj?.winProbability || 0)}% WIN PROB</span></div></>
            )}
            <div style={{ transform: isMobile ? `scale(${visuals.tableZoom * 0.9})` : `scale(${Math.min(visuals.tableZoom, 1.1)})` }} className="relative w-full max-w-[1400px] aspect-[15/10] lg:aspect-[16/9] flex items-center justify-center h-full origin-center">
                ...
                <div className="absolute inset-0 pointer-events-none z-20">{players.map((p, i) => <Seat key={i} player={p} displayPos={DISPLAY_POSITIONS[i]} phase={phase} isActiveTurn={activeIdx === i} isDealer={dealerIdx === i} isHero={i === heroIdx} visuals={visuals} formatRank={formatRank} winning5Ids={winning5Ids} />)}</div>
      ...
      {showRebuyModal && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/90 backdrop-blur-xl px-6">
          <div className="w-full max-w-[400px] p-8 bg-slate-900 border border-indigo-500/30 rounded-3xl flex flex-col gap-10">
            ...
            <div className="flex gap-4"><button onClick={()=>setShowRebuyModal(false)} className="flex-1 p-4 bg-white/5 border border-white/10 rounded-xl font-black text-xs uppercase">CANCEL</button><button onClick={()=>{ if(currentRoomId && userProfile) socket.emit('playerRebuy', { roomId: currentRoomId, uid: userProfile.uid, amount: Number(rebuyAmount) }); setShowRebuyModal(false); }} className="flex-2 p-4 bg-indigo-600 rounded-xl shadow-lg transition-all text-xs font-black uppercase">INJECT FUNDS</button></div>
          </div>
        </div>
      )}
      {showRulesModal && (
