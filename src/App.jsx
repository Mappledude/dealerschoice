  ...
  if (currentView === VIEWS.ADMIN) return (
    <div className="h-screen bg-[#06080c] flex flex-col md:flex-row text-white font-black uppercase overflow-hidden">
        <aside className="w-full md:w-64 border-b md:border-b-0 md:border-r border-white/10 p-4 md:p-8 flex flex-row md:flex-col gap-4 bg-black/20 shrink-0">
            ...
            <button onClick={()=>setAdminTab(ADMIN_TABS.PLAYERS)} className={`flex-1 md:flex-none p-3 md:p-4 rounded-xl text-xs md:text-sm transition-all font-black ${adminTab === ADMIN_TABS.PLAYERS ? 'bg-[#fbbf24] text-black' : 'text-white/40'}`}>PLAYERS</button>
            <button onClick={()=>setAdminTab(ADMIN_TABS.TABLES)} className={`flex-1 md:flex-none p-3 md:p-4 rounded-xl text-xs md:text-sm transition-all font-black ${adminTab === ADMIN_TABS.TABLES ? 'bg-[#fbbf24] text-black' : 'text-white/40'}`}>TABLES</button>
            ...
            {adminTab === ADMIN_TABS.PLAYERS ? (
                <div className="flex flex-col gap-8 animate-in fade-in">
                    ...
                    <div className="bg-white/5 p-4 md:p-6 rounded-2xl grid grid-cols-1 md:grid-cols-3 gap-4 border border-white/10 shadow-xl">
                        ...
                        <button onClick={()=>socket.emit('adminCreatePlayer', {...newPlayer, uid: Math.random().toString(36).slice(2)})} className="bg-[#fbbf24] text-black rounded-xl font-black p-4">CREATE</button>
                    </div>
                ...
                <div className="flex flex-col gap-8 animate-in fade-in">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {(activeTables || []).map(t => (
                            <div key={t.id} className="bg-white/5 p-4 md:p-6 rounded-2xl flex justify-between items-center border border-white/10 shadow-lg font-black uppercase">
        ...
        {selectedTableForJoin && (
            <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-md animate-in fade-in duration-300 px-6">
                <div className="w-full max-w-[400px] p-8 md:p-12 bg-slate-900 border border-[#fbbf24]/30 rounded-[2vw] shadow-2xl flex flex-col gap-10">
                    <h3 className="text-2xl md:text-3xl text-center tracking-widest text-[#fbbf24] underline underline-offset-8 font-black">{String(selectedTableForJoin.name)}</h3>
        ...
        <main className="flex-1 p-6 md:p-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10 overflow-y-auto bg-gradient-to-br from-transparent to-white/5 font-black uppercase">
            {(activeTables || []).map((t) => (
                <div key={t.id} className="p-8 bg-white/5 border border-white/5 rounded-3xl flex flex-col gap-6 shadow-2xl hover:border-[#fbbf24]/20 transition-all group relative overflow-hidden font-black">
                    <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-20 transition-opacity"><LayoutGrid size={64}/></div>
                    ...
                    <div className="bg-black/60 p-4 md:p-6 rounded-2xl flex justify-between items-center border border-white/5 shadow-inner">
                        ...
                        <div className="flex flex-col items-end"><span className="text-[8px] text-white/40 tracking-widest">SEATS</span><span className="text-white/80 font-mono text-sm md:text-base">{t.players?.filter(p=>p).length}/10</span></div>
                    </div>
                    <button onClick={()=>setSelectedTableForJoin(t)} className="w-full p-6 md:p-8 bg-emerald-600 rounded-2xl tracking-widest shadow-xl hover:scale-[1.02] transition-all">ENTER ARENA</button>
                </div>
            ))}
        </main>
    </div>
    ...
    <div className="h-screen bg-[#06080c] text-white flex flex-col overflow-hidden relative font-black uppercase tracking-tighter">
      {isBrokeStatus && (
          <div className="fixed inset-0 z-[5000] flex items-center justify-center bg-black/95 backdrop-blur-2xl animate-in zoom-in-95 duration-300 p-6 font-black uppercase">
                <div className="w-full max-w-[400px] p-10 md:p-14 bg-slate-900 border-2 border-red-500 rounded-3xl text-center shadow-[0_0_100px_rgba(239,68,68,0.4)]">
                    ...
                    <h2 className="text-3xl font-black tracking-widest text-white mb-2">BUSTED!</h2>
                    <p className="text-white/40 text-[10px] mb-8 tracking-widest">REBUY OR BE REMOVED FROM THE SEAT.</p>
                    <button onClick={() => socket.emit('adminAddChips', { roomId: currentRoomId, uid: userProfile.uid, chips: 1000 })} className="w-full p-6 bg-emerald-600 hover:bg-emerald-500 text-white rounded-2xl shadow-xl tracking-[0.2em] animate-bounce">REBUY $1,000</button>
                    <button onClick={() => {setCurrentView(VIEWS.LOBBY); setCurrentRoomId(null);}} className="w-full mt-4 p-4 text-white/20 hover:text-white transition-all text-xs tracking-widest">EXIT</button>
                </div>
          </div>
      )}
      ...
      <header style={{ height: `${headerHeight}px` }} className="bg-[#0a0a0a] border-b border-white/10 flex items-center justify-between px-4 md:px-8 z-[80] shadow-2xl backdrop-blur-md shrink-0 font-black uppercase">
        <div className="flex items-center gap-2">
            ...
            <button onClick={() => setShowLayoutControls(!showLayoutControls)} className={`p-2 rounded-lg transition-all ${showLayoutControls ? 'bg-[#fbbf24] text-black shadow-[0_0_15px_#fbbf24]' : 'bg-white/5 text-white/40'}`}><Sliders size={18}/></button>
        </div>

        {showLayoutControls && (
            <div className="absolute top-16 left-4 bg-black/95 border border-white/10 p-6 rounded-2xl shadow-2xl z-[1000] flex flex-col gap-5 min-w-[240px] animate-in slide-in-from-top-4 backdrop-blur-xl">
        ...
        <div className="bg-white/5 border border-white/10 px-4 py-1.5 rounded-xl flex items-center gap-4 shadow-inner">
            <span className="hidden sm:inline text-white/40 text-[9px] tracking-widest uppercase font-black">DEALER:</span>
            ...
            <div className="absolute top-[43%] left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-30 pointer-events-none w-full h-full justify-center">
              {!potTransferring && (
                <div className={`flex flex-col items-center transition-all duration-300 ${potAnimating ? 'scale-125' : 'scale-100'}`}>
                    ...
                    <div className="px-4 py-1 bg-black/40 rounded-full border border-white/5"><span className="text-[10px] md:text-xs text-[#fbbf24] tracking-widest font-black uppercase">POT</span></div>
                </div>
              )}
              <div className="flex gap-2 md:gap-4 scale-[1.1] md:scale-[1.8] mt-6 md:mt-12">
                  {(community || []).map((c, j) => (
      ...
      <footer style={{ height: `${footerHeight}px` }} className="bg-black/90 backdrop-blur-3xl border-t border-white/10 flex z-[100] shadow-[0_-20px_50px_rgba(0,0,0,0.8)] shrink-0 font-black uppercase overflow-hidden">
        <div className="hidden md:flex w-1/4 border-r border-white/10 p-6 flex flex-col overflow-hidden text-[10px] font-mono tracking-widest font-black">
            ...
            <div className="flex-1 space-y-2 overflow-y-auto scrollbar-hide">
                {logs.map(l => (
                    <div key={l.id} className="animate-in slide-in-from-left duration-300 flex items-start gap-2 border-l-2 border-white/5 pl-2 py-0.5 font-black">
        ...
        <div className="flex-1 flex flex-col justify-center px-4 md:px-10 relative bg-white/5 shadow-inner py-4 font-black">
          {activeIdx === heroIdx && phase !== PHASES.SHOWDOWN && phase !== PHASES.IDLE && heroPlayerObj ? (
            <div className="flex flex-col gap-4 animate-in slide-in-from-bottom duration-500 items-center w-full">
                <div className="flex gap-2 w-full max-w-[600px] font-black uppercase">
                    ...
                    <button onClick={()=>handleAction('RAISE', heroPlayerObj.chips + heroPlayerObj.currentBet)} className="flex-1 py-3 bg-red-900/20 border border-red-500/50 rounded-2xl text-[9px] md:text-[10px] text-red-500 hover:bg-red-600 transition-all font-black uppercase">ALL-IN</button>
                </div>
                <div className="flex gap-2 md:gap-4 w-full items-center justify-center font-black">
                    <button onClick={()=>handleAction('FOLD')} className="w-20 md:w-28 h-16 md:h-20 bg-red-950/60 border-2 border-red-500/50 rounded-3xl tracking-widest hover:brightness-125 transition-all font-black text-xs uppercase shadow-lg shadow-red-900/20">FOLD</button>
                    <button onClick={()=>handleAction('CALL')} className="flex-1 max-w-[320px] h-16 md:h-20 bg-blue-900/60 border-2 border-blue-400/50 rounded-3xl text-lg md:text-2xl tracking-widest hover:brightness-125 font-black uppercase shadow-lg shadow-blue-900/20">{highestBet > heroPlayerObj.currentBet ? `CALL $${highestBet - heroPlayerObj.currentBet}` : 'CHECK'}</button>
                    <div className="flex gap-1 items-center bg-black/60 border border-white/10 p-1 md:p-2 rounded-3xl shadow-inner min-w-[140px] md:min-w-[280px]">
                        <div className="flex items-center bg-black/40 px-2 md:px-4 rounded-2xl border border-white/5 h-12 md:h-14 font-black">
                            <span className="text-[#fbbf24] text-xs font-mono mr-1 uppercase font-black">$</span>
                            <input type="number" value={raiseInput} onChange={(e) => setRaiseInput(Math.min(heroPlayerObj.chips + heroPlayerObj.currentBet, Math.max(highestBet + 20, Number(e.target.value))))} className="w-12 md:w-24 bg-transparent text-center font-mono text-base md:text-2xl text-[#fbbf24] outline-none font-black" />
                        </div>
                        <button onClick={()=>handleAction('RAISE', raiseInput)} className="flex-1 h-12 md:h-14 bg-emerald-700/60 border border-emerald-400/50 rounded-2xl flex items-center justify-center hover:brightness-125 font-black uppercase text-xs md:text-xl shadow-lg shadow-emerald-900/20"><Zap size={20} className="md:mr-2 text-emerald-400"/></button>
                    </div>
                </div>
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center h-full relative font-black">
                {showdownWinners && showdownWinners.length > 0 ? (
                    <div className="flex items-center gap-6 md:gap-14 animate-in fade-in zoom-in-95 duration-500 w-full h-full justify-center font-black">
                        <div className="flex flex-col items-center font-black">
                            <div className="p-5 bg-yellow-500/10 rounded-full border-2 border-yellow-500/20 animate-bounce mb-2 shadow-[0_0_40px_rgba(251,191,36,0.1)]"><Trophy size={48} className="text-[#fbbf24]" /></div>
                            <div className="text-center font-black uppercase">
                                <h4 className="text-[#fbbf24] text-base md:text-xl font-black uppercase truncate max-w-[120px] md:max-w-none">{showdownWinners[0].name || "Anon"} SCOOPS!</h4>
                                <p className="text-white/60 text-[8px] md:text-xs font-black tracking-widest">{showdownWinners[0].rank || "Hand"}</p>
                            </div>
                        </div>
                        <div className="flex flex-col gap-4 font-black">
                             <div className="flex gap-2 p-4 bg-black/50 rounded-3xl border-2 border-yellow-500/20 relative shadow-2xl font-black">
                                  <div className="absolute -top-4 -right-4 bg-emerald-500 text-black px-4 py-1.5 rounded-full font-black text-lg md:text-2xl shadow-xl animate-pulse ring-4 ring-black">+${(showdownWinners[0].amount || 0).toLocaleString()}</div>
                                  {(showdownWinners[0].hand || []).map((c, ci) => (
                                     <div key={ci} className="w-[11vw] md:w-[4.5vw] h-[15vw] md:h-[6.5vw] bg-white rounded-xl flex flex-col items-center justify-center text-black shadow-2xl ring-2 ring-yellow-400/50 transform hover:scale-110 transition-all duration-300 font-black bg-glimmer">
                                         <span className="text-[3.5vw] md:text-[1.2vw] font-black">{String(c.value)}</span>
                                         <span className={`text-[6vw] md:text-[2.2vw] font-black ${c.suit === '♥' || c.suit === '♦' ? 'text-red-600' : 'text-black'}`}>{String(c.suit)}</span>
                                     </div>
                                  ))}
                             </div>
                    ...
                    <div className="flex flex-col items-center gap-5 animate-pulse font-black uppercase">
                        <div className="p-6 bg-white/5 rounded-full border border-white/5 shadow-inner"><Target size={48} className="text-white/20"/></div>
                        <span className="text-white/40 tracking-[0.5em] text-xs md:text-xl font-black italic">{phase === PHASES.IDLE ? "ARENA IDLE" : "WAITING FOR MOVE"}</span>
                    </div>
                )}
            </div>
          )}
