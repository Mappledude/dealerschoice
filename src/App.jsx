...
const Seat = ({ 
  ...
  relativeIdx, visuals, bigBlind, showdownWinners
    ...
    return (
        <div style={{ left: `${displayPos.x}%`, top: `${displayPos.y}%` }} className={`absolute -translate-x-1/2 -translate-y-1/2 flex flex-col items-center transition-all duration-500 ${isHero ? 'z-[100]' : 'z-20'} ${isFoldedBool ? 'opacity-30 grayscale scale-90' : 'opacity-100'} ${player.waitingForNextHand ? 'opacity-50' : ''}`}>
            ...
            {player.hand && Array.isArray(player.hand) && !isFoldedBool && !player.waitingForNextHand && (
                <div className={`absolute flex items-center justify-center w-[15vw] lg:w-[12vh] h-[8vw] lg:h-[8vh] pointer-events-none ${cardZIndex}`} style={{ transform: isMobile ? `translate(${cardInwardX}vw, ${cardInwardY}vw)` : `translate(${cardInwardX * 0.4}vh, calc(${cardInwardY * 0.4}vh - ${isHero ? '70px' : '0px'}))` }}>
                    {player.hand.map((c, ci) => {
                        ...
                        // Highlight only if the card is part of the currently displayed winner's 5-card hand
                        const isWinningCard = (winning5Ids || []).includes(c.id);

                        return (
                          <div key={`${c.id || ci}-${ci}`} className={`w-[7.5vw] lg:w-[5.5vh] h-[10.5vw] lg:h-[8vh] rounded-lg flex flex-col items-start justify-start p-1 border absolute transition-all duration-300 shadow-2xl ${shouldRevealCards ? 'bg-white' : 'bg-slate-900 border-white/20'}`} style={{ transform: isMobile ? `translateX(${offset * cardSpacing}vw) rotate(${rotation}deg) scale(${scale})` : `translateX(${offset * cardSpacing}vh) rotate(${rotation}deg) scale(${scale})`, transformOrigin: 'bottom center', zIndex: 100 + ci }}>
                              ...
                              {phase === PHASES.SHOWDOWN && player.isWinner && isWinningCard && !isMuckWin && (<div className="absolute inset-0 ring-4 ring-yellow-400 rounded-lg animate-pulse" />)}
                          </div>
                        );
                    })}
                </div>
            ...
            <div className={`relative z-[90] flex flex-col items-center p-2 lg:p-3 rounded-xl border transition-all duration-300 min-w-[120px] lg:min-w-[14vh] overflow-hidden backdrop-blur-xl scale-[0.85] ${isActiveTurn ? 'border-white ring-4 ring-white/20 bg-slate-800 shadow-[0_0_40px_rgba(255,255,255,0.2)]' : 'border-white/10 bg-black/80'} ${player.isWinner && phase === PHASES.SHOWDOWN ? 'border-yellow-400 ring-2 ring-yellow-400/50' : ''}`}>
                ...
                <div className="flex flex-col items-center w-full relative z-10 py-1 overflow-hidden">
                    ...
                    <div className="w-full h-[24px] lg:h-[3.5vh] relative flex items-center justify-center">
                        <div className={`absolute inset-0 flex items-center justify-center transition-all duration-300 ${ghostAction && !isFoldedBool ? 'opacity-0 scale-90 pointer-events-none' : 'opacity-100 scale-100'}`}>
...
const App = () => {
  ...
  const handHistory = useMemo(() => {
    const hands = []; let currentHand = null;
    ([...logs].reverse()).forEach(log => {
        if (String(log.action).includes("IS DEALING") || String(log.action).includes("PRE_FLOP DEALT")) {
            ...
            currentHand = { id: log.handId || `hand-${log.timestamp}-${Math.random()}`, winner: null, rank: null, amount: null, events: [], variant: String(log.action).split('DEALING ')[1] || "Poker" };
        }
        if (currentHand) {
  ...
  const copyActivityToClipboard = () => {
    ...
    handHistory.forEach(hand => {
      ...
      hand.events.forEach(ev => { text += `[${new Date(ev.timestamp).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit', second:'2-digit'})}] ${ev.name}: ${ev.action}\n`; });
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide space-y-3 pr-1 font-black">
            {handHistory.length > 0 ? handHistory.map((hand) => (
                <div key={hand.id} className="border border-white/5 rounded-xl overflow-hidden bg-white/5">
                    <button onClick={() => { const n = new Set(expandedHands); if (n.has(hand.id)) n.delete(hand.id); else n.add(hand.id); setExpandedHands(n); }} className="w-full p-3 flex flex-col items-start gap-1 transition-all hover:bg-white/5">
                    ...
                    {expandedHands.has(hand.id) && (<div className="px-3 pb-3 border-t border-white/5 bg-black/40 space-y-1 pt-2">{hand.events.map((ev, i) => (<div key={ev.uniqueKey || `ev-${i}`} className={`text-[9px] md:text-[10px] leading-tight py-1 border-l-2 pl-2 ${ev.type === 'win' ? 'border-emerald-500 bg-emerald-500/5' : ev.type === 'fold' ? 'border-red-500 bg-red-500/5' : 'border-indigo-500 bg-indigo-500/5'}`}><span className="text-white/30 font-mono mr-2 uppercase">[{new Date(ev.timestamp).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit', second:'2-digit'})}]</span> <span className={getNeonNameColor(ev.name)}>{String(ev.name)}</span>: <span className="text-white/90 uppercase">{String(ev.action)}</span></div>))}</div>)}
                </div>
  ...
  useEffect(() => {
    const handleRoomUpdate = (d) => {
        ...
        if (isPhaseTransition && [PHASES.FLOP, PHASES.TURN, PHASES.RIVER].includes(d.phase)) {
            ...
            setLogs(prev => [{ name: "SYSTEM", action: `${d.phase} DEALT [ ${cardString} ]`, type: 'phase', handId: currentHandId.current, timestamp: Date.now(), uniqueKey: `board-${Date.now()}` }, ...prev]);
            ...
            if (d.phase === PHASES.FLOP) {
                const vId = d.activeVariant?.id || 'HOLDEM';
                setTimeout(() => { setAnnouncement({ text: VARIANTS[vId]?.name || "Poker", color: VARIANT_COLORS[vId] || '#fff' }); setTimeout(() => setAnnouncement(null), 1500); }, 3000); 
            }
        }
        phaseRef.current = d.phase;
        setPhase(d.phase); setCommunity(d.community || []); setPotAmount(d.potAmount || d.potData?.[0]?.amount || 0); setActiveIdx(d.activeIdx ?? -1); setHighestBet(d.highestBet || 0); if (d.bb) setBigBlind(d.bb); if (d.minRaiseAmount !== undefined) setMinRaiseAmount(d.minRaiseAmount); setDealerIdx(d.dealerIdx ?? -1); setTimeRemaining(d.timeRemaining || 0);
        if (d.activeVariant) { const vId = typeof d.activeVariant === 'string' ? d.activeVariant : d.activeVariant.id; setActiveVariant(VARIANTS[vId] || { id: vId, name: d.activeVariant.name || vId, rules: [] }); }
        
        if (d.phase === PHASES.SHOWDOWN && !showdownWinners) {
            setPotTransferring(true); setCurrentShowdownIdx(0); const rawWinners = d.showdownWinners || []; setShowdownWinners(rawWinners); setWinning5Ids(rawWinners[0]?.winning5Ids || d.winning5Ids || []);
    ...
    socket.on('profilesUpdate', (list) => { setAllProfiles(list); setUserProfile(prev => { if (!prev) return null; const updated = list.find(p => p.uid === prev.uid); return updated ? { ...prev, chips: updated.chips } : prev; }); });
                ...
                return (
                    <div key={`winner-disp-${winner.name}-${currentShowdownIdx}`} className="flex flex-col items-center justify-start w-full gap-2 animate-in fade-in slide-in-from-bottom-2 duration-500">
                        <div className={`flex items-center gap-3 bg-white/5 px-5 py-1 rounded-full border border-white/10 max-w-full overflow-hidden shadow-2xl`}>
                            ...
                            <div className="text-sm md:text-xl font-black tracking-tighter flex items-center gap-2 leading-none whitespace-nowrap"><span className={getNeonNameColor(winner.name)}>{String(winner.name).toUpperCase()}</span>{isMuckWin ? (<span className="text-white ml-2">SCOOPED THE POT</span>) : (<><span className="text-white/40">WON TOTAL</span><span className="text-emerald-400 font-mono ml-2">+${Number(winner.amount).toLocaleString(undefined, {minimumFractionDigits: 2})}</span></>)}</div>
                        </div>
                        {!isMuckWin && (<><div className="text-[10px] md:text-sm font-black text-white/60 tracking-widest uppercase">HOLDING <span className={themeColor}>{String(formatRank(winner.rank))}</span></div><div className="flex gap-1 justify-center mt-1">{(winner.hand || []).map((c, ci) => (<div key={`winner-card-${ci}`} className={`w-10 md:w-16 h-13 md:h-20 bg-white rounded flex flex-col items-start justify-start p-1 text-black shadow-2xl border-t-2 border-x-2 ${cardBorder} relative overflow-hidden`}><span className={`text-[11px] md:text-sm font-black leading-tight ${c.suit === '♥' || c.suit === '♦' ? 'text-red-600' : 'text-black'}`}>{String(c.value)}</span><span className={`text-[13px] md:text-xl leading-none ${c.suit === '♥' || c.suit === '♦' ? 'text-red-600' : 'text-black'}`}>{String(c.suit)}</span><div className="absolute bottom-0 w-full h-1/2 bg-gradient-to-t from-black/40 to-transparent" /></div>))}</div></>)}
                    </div>
                );
            })()
          ) : (
