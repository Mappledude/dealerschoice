...
const Seat = ({ player, displayPos, phase, isActiveTurn, isDealer, visuals, formatRank, isHero }) => {
    ...
    return (
        <div style={{ left: `${displayPos.x}%`, top: `${displayPos.y}%` }} className={`absolute -translate-x-1/2 -translate-y-1/2 flex flex-col items-center transition-all duration-500 ${isHero ? 'z-[100]' : 'z-20'}`}>
            ...
            <div className={`relative z-[90] flex flex-col items-center p-2 lg:p-3 rounded-xl border transition-all duration-300 min-w-[120px] lg:min-w-[14vh] backdrop-blur-xl ${isActiveTurn ? 'border-white ring-4 ring-white/20 bg-slate-800 shadow-[0_0_20px_rgba(255,255,255,0.2)]' : 'border-white/10 bg-black/80'}`}>
                {isDealer && (
                ...
                <div className={`flex flex-col items-center w-full transition-all ${isFolded ? 'opacity-40 grayscale' : ''}`}>
                    ...
                    {(phase === PHASES.SHOWDOWN || isHero) && player.strength && <span className="text-[8px] text-cyan-400 font-bold uppercase">{formatRank(player.strength)}</span>}
                </div>
  ...
  useEffect(() => {
    socket.on('roomUpdate', d => {
        ...
        if (d.phase !== phaseRef.current) {
            if (d.phase === PHASES.PRE_FLOP) { setHandAttention(true); setTimeout(()=>setHandAttention(false), 3000); }
