...
const Seat = ({ player, displayPos, phase, winning5Ids, isCollectingBets, isActiveTurn, strengthLabel, potTransferring, timeRemaining, isHero, cardScale }) => {
    if (!player || !displayPos) return null;
    const isShowdown = phase === PHASES.SHOWDOWN;
    const highAward = player.hiLowAwards?.high;
    const lowAward = player.hiLowAwards?.low;
        ...
        <div style={{ left: `${displayPos.x}%`, top: `${displayPos.y}%` }} className={`absolute -translate-x-1/2 -translate-y-1/2 flex flex-col-reverse items-center z-20 transition-all duration-500 ${player.isFolded ? 'opacity-20 grayscale scale-90' : 'opacity-100'}`}>
            {isShowdown && potTransferring && (
                <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-16 flex flex-col gap-1 items-center z-[500]">
                    {highAward && <span className="bg-emerald-600 text-white text-[8px] md:text-[10px] px-2 py-0.5 rounded-full font-black animate-bounce shadow-lg whitespace-nowrap uppercase">HIGH (+${highAward})</span>}
                    {lowAward && <span className="bg-orange-600 text-white text-[8px] md:text-[10px] px-2 py-0.5 rounded-full font-black animate-bounce shadow-lg whitespace-nowrap uppercase">LOW (+${lowAward})</span>}
                </div>
            )}
...
const App = () => {
  ...
  // 2. DERIVED STATE (DEFINED EARLY TO PREVENT REFERENCE ERRORS)
                    ...
                    <div className="bg-white/5 rounded-2xl overflow-hidden border border-white/10">
                        {(allProfiles || []).map(p => (
                            <div key={p.uid} className="flex justify-between p-4 md:p-6 border-b border-white/5 hover:bg-white/5 transition-all font-black uppercase">
                                <span>{String(p.name)} <span className="text-white/20 ml-2 text-xs">[{String(p.password)}]</span></span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {(activeTables || []).map(t => (
                            <div key={t.id} className="bg-white/5 p-4 md:p-6 rounded-2xl flex justify-between items-center border border-white/10 hover:border-emerald-500/50 transition-all shadow-lg font-black uppercase">
                                ...
                                <button onClick={()=>socket.emit('adminDeleteRoom', t.id)} className="bg-red-950/40 p-2 md:p-3 rounded-xl text-red-500 hover:bg-red-500 transition-all text-xs font-black">TERMINATE</button>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-md animate-in fade-in duration-300 px-6 font-black uppercase">
                <div className="w-full max-w-[400px] p-8 md:p-12 bg-slate-900 border border-[#fbbf24]/30 rounded-[2vw] shadow-2xl flex flex-col gap-10">
                    ...
                    <div className="space-y-6 font-black text-center uppercase">
                        <div className="flex justify-between items-center text-[10px] text-white/40 tracking-widest font-black uppercase font-black"><span>BUY-IN AMOUNT</span><span className="text-emerald-400 text-xl md:text-2xl font-mono font-black">${buyInAmount.toLocaleString()}</span></div>
        ...
        <header className="h-20 border-b border-white/10 flex items-center justify-between px-6 md:px-12 bg-black/40 backdrop-blur-md shadow-xl z-50 shrink-0 font-black">
            ...
            <div className="flex items-center gap-6 md:gap-10 font-black uppercase">
                <div className="flex flex-col items-end font-black">
                    <span className="text-[8px] md:text-[10px] text-white/40 uppercase italic font-black">ID: {String(userProfile?.name || "??")}</span>
        ...
        <main className="flex-1 p-6 md:p-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10 overflow-y-auto bg-gradient-to-br from-transparent to-white/5 font-black uppercase">
            {(activeTables || []).map((t) => (
                <div key={t.id} className="p-8 bg-white/5 border border-white/5 rounded-3xl flex flex-col gap-6 shadow-2xl hover:border-[#fbbf24]/20 transition-all group relative overflow-hidden font-black uppercase">
                    ...
                    <div className="text-[10px] text-white/40 h-8 md:h-10 tracking-widest overflow-hidden font-black uppercase font-black">SEATED: {t.players?.filter(p => p).map(p => String(p.name)).join(', ') || 'NONE'}</div>
                    <div className="bg-black/60 p-4 md:p-6 rounded-2xl flex justify-between items-center border border-white/5 shadow-inner uppercase font-black">
                        <div className="flex flex-col font-black uppercase"><span className="text-[8px] text-white/40 tracking-widest font-black uppercase">STAKES</span><span className="text-[#fbbf24] text-lg md:text-xl font-black uppercase">${t.sb}/${t.bb}</span></div>
                        <div className="flex flex-col items-end font-black uppercase"><span className="text-[8px] text-white/40 tracking-widest font-black uppercase">SEATS</span><span className="text-white/80 font-mono text-sm md:text-base font-black uppercase">{t.players?.filter(p=>p).length}/10</span></div>
                    </div>
    ...
    <div className="h-screen bg-[#06080c] text-white flex flex-col overflow-hidden relative font-black uppercase tracking-tighter">
          ...
          <div className="fixed inset-0 z-[5000] flex items-center justify-center bg-black/95 backdrop-blur-2xl animate-in zoom-in-95 duration-300 p-6 font-black uppercase">
                <div className="w-full max-w-[400px] p-10 md:p-14 bg-slate-900 border-2 border-red-500 rounded-3xl text-center shadow-[0_0_100px_rgba(239,68,68,0.4)] font-black uppercase">
                    <div className="relative inline-block mb-6 font-black">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center font-mono text-3xl text-white mt-1 uppercase font-black">
                    ...
                    <button onClick={() => {setCurrentView(VIEWS.LOBBY); setCurrentRoomId(null);}} className="w-full mt-4 p-4 text-white/20 hover:text-white transition-all text-xs tracking-widest font-black uppercase font-black uppercase">EXIT</button>
                </div>
          </div>
      )}

      {/* 2. HEADER - DYNAMIC HEIGHT */}
      <header style={{ height: `${headerHeight}px` }} className="bg-[#0a0a0a] border-b border-white/10 flex items-center justify-between px-4 md:px-8 z-[80] shadow-2xl backdrop-blur-md shrink-0 font-black uppercase">
        <div className="flex items-center gap-2">
            <div className="bg-white/5 px-3 py-1.5 rounded-xl border border-white/5 shadow-inner truncate">
                <span className="text-[#fbbf24] text-[8px] md:text-[10px] tracking-widest uppercase font-black uppercase">ARENA:</span>
        ...
        {showLayoutControls && (
            <div className="absolute top-16 left-4 bg-black/95 border border-white/10 p-6 rounded-2xl shadow-2xl z-[1000] flex flex-col gap-5 min-w-[240px] animate-in slide-in-from-top-4 backdrop-blur-xl font-black uppercase">
                <div className="space-y-2 font-black">
                    <div className="flex justify-between text-[8px] text-white/40 font-black uppercase font-black"><span>HEADER</span><span>{headerHeight}PX</span></div>
                ...
                <div className="space-y-2 font-black">
                    <div className="flex justify-between text-[8px] text-white/40 font-black uppercase font-black"><span>HUD</span><span>{footerHeight}PX</span></div>
                ...
                <div className="space-y-2 font-black">
                    <div className="flex justify-between text-[8px] text-white/40 font-black uppercase font-black"><span>ZOOM</span><span>{Math.round(tableZoom * 100)}%</span></div>
                ...
                <div className="space-y-2 font-black">
                    <div className="flex justify-between text-[8px] text-white/40 font-black uppercase font-black"><span>CARD SIZE</span><span>{Math.round(cardScale * 100)}%</span></div>
                ...
                <button onClick={()=>setShowLayoutControls(false)} className="bg-[#fbbf24] text-black font-black py-2 rounded-lg text-[10px] tracking-widest uppercase font-black font-black uppercase">CLOSE</button>
            </div>
        )}
      ...
      <main className="flex-1 flex flex-col items-center justify-center relative bg-gradient-to-b from-emerald-950/20 to-transparent overflow-hidden px-2 py-2 font-black uppercase">
        <div style={{ transform: `scale(${tableZoom})`, maxHeight: `calc(100vh - ${headerHeight + footerHeight + 10}px)` }} className="relative w-full max-w-[1400px] aspect-[21/10] flex items-center justify-center h-full transition-transform duration-300 ease-out origin-center font-black">
            ...
            <div className="absolute inset-0 bg-[#0f3d2e]/40 rounded-[50%] border-[2vw] border-slate-900/60 shadow-[inset_0_0_15vw_rgba(0,0,0,0.8)] border-double font-black" />
            <div className="absolute top-[43%] left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-30 pointer-events-none w-full h-full justify-center">
              ...
              <div className="flex gap-2 md:gap-4 scale-[1.1] md:scale-[1.8] mt-6 md:mt-12 font-black">
                  {(community || []).map((c, j) => (
      ...
      <footer style={{ height: `${footerHeight}px` }} className="bg-black/90 backdrop-blur-3xl border-t border-white/10 flex z-[100] shadow-[0_-10px_40px_rgba(0,0,0,0.5)] shrink-0 font-black uppercase overflow-hidden font-black">
        <div className="hidden lg:flex w-1/4 border-r border-white/10 p-6 flex flex-col overflow-hidden text-[10px] font-mono tracking-widest font-black uppercase">
            ...
            <div className="flex-1 space-y-2 overflow-y-auto scrollbar-hide font-black uppercase">
                {(logs || []).map(l => (
                    <div key={l.id} className="animate-in slide-in-from-left duration-300 flex items-start gap-2 border-l-2 border-white/5 pl-2 py-0.5 font-black uppercase font-black uppercase">
                        <span className="text-white/20 text-[8px] font-black shrink-0 uppercase font-black uppercase">{String(l.time)}</span> 
                        <div className="flex flex-wrap gap-x-1 font-black uppercase font-black uppercase font-black uppercase"><span className={`font-black uppercase text-[9px] ${l.type === 'win' ? 'text-emerald-400' : l.type === 'variant' ? 'text-purple-400' : 'text-[#fbbf24]'}`}>{String(l.name)}</span><span className="text-white/60 lowercase tracking-normal text-[9px] font-black">{String(l.action)}</span></div>
                    </div>
                ))}
            </div>
        </div>
        ...
        <div className="flex-1 flex flex-col justify-center px-4 md:px-10 relative bg-white/5 shadow-inner py-2 font-black uppercase">
          {activeIdx === heroIdx && phase !== PHASES.SHOWDOWN && phase !== PHASES.IDLE && heroPlayerObj ? (
            <div className="flex flex-col gap-2 md:gap-4 animate-in slide-in-from-bottom duration-500 items-center w-full font-black">
                <div className="flex gap-2 w-full max-w-[600px] font-black uppercase font-black uppercase">
                    <button onClick={()=>handleAction('RAISE', highestBet + Math.floor(potAmount * 0.5))} className="flex-1 py-2 bg-white/5 border border-white/10 rounded-xl text-[9px] md:text-[10px] hover:bg-[#fbbf24] hover:text-black transition-all font-black truncate px-1">1/2 POT</button>
                    <button onClick={()=>handleAction('RAISE', highestBet + potAmount)} className="flex-1 py-2 bg-white/5 border border-white/10 rounded-xl text-[9px] md:text-[10px] hover:bg-[#fbbf24] hover:text-black transition-all font-black truncate px-1">POT</button>
                    <button onClick={()=>handleAction('RAISE', heroPlayerObj.chips + heroPlayerObj.currentBet)} className="flex-1 py-2 bg-red-900/20 border border-red-500/50 rounded-xl text-[9px] md:text-[10px] text-red-500 hover:bg-red-600 transition-all font-black uppercase font-black uppercase">ALL-IN</button>
                </div>

                <div className="flex gap-2 md:gap-4 w-full items-center justify-center font-black uppercase font-black uppercase">
                    <button onClick={()=>handleAction('FOLD')} className="w-16 md:w-28 h-12 md:h-16 bg-red-950/60 border-2 border-red-500/50 rounded-3xl tracking-widest hover:brightness-125 transition-all font-black text-xs uppercase shadow-lg shadow-red-900/20">FOLD</button>
                    <button onClick={()=>handleAction('CALL')} className="flex-1 max-w-[320px] h-12 md:h-16 bg-blue-900/60 border-2 border-blue-400/50 rounded-3xl text-sm md:text-xl tracking-widest hover:brightness-125 font-black uppercase shadow-lg shadow-blue-900/20">{highestBet > heroPlayerObj.currentBet ? `CALL $${highestBet - heroPlayerObj.currentBet}` : 'CHECK'}</button>
                    <div className="flex gap-1 md:gap-2 items-center bg-black/60 border border-white/10 p-1 md:p-2 rounded-3xl shadow-inner min-w-[100px] md:min-w-[280px] font-black uppercase">
                        <div className="flex items-center bg-black/40 px-2 md:px-4 rounded-xl border border-white/5 h-10 md:h-12 font-black uppercase">
                            ...
                            <input type="number" value={raiseInput} onChange={(e) => setRaiseInput(Math.min(heroPlayerObj.chips + heroPlayerObj.currentBet, Math.max(minRaiseAllowed, Number(e.target.value))))} className="w-8 md:w-24 bg-transparent text-center font-mono text-sm md:text-2xl text-[#fbbf24] outline-none font-black font-black" />
                        </div>
                        <button onClick={()=>handleAction('RAISE', raiseInput)} className="flex-1 h-10 md:h-12 bg-emerald-700/60 border border-emerald-400/50 rounded-2xl flex items-center justify-center hover:brightness-125 font-black uppercase text-xs md:text-xl shadow-lg shadow-emerald-900/20"><Zap size={18} className="md:mr-2 text-emerald-400"/></button>
                    </div>
                </div>
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center h-full relative font-black uppercase font-black uppercase">
                {showdownWinners && showdownWinners.length > 0 ? (
                    <div className="flex items-center gap-6 md:gap-14 animate-in fade-in zoom-in-95 duration-500 w-full h-full justify-center font-black uppercase font-black uppercase">
                        <div className="flex flex-col items-center font-black uppercase">
                            ...
                            <div className="text-center font-black uppercase font-black uppercase">
                                <h4 className="text-[#fbbf24] text-base md:text-2xl font-black uppercase truncate max-w-[120px] md:max-w-none">{showdownWinners[0].name || "Anon"} SCOOPS! font-black uppercase font-black uppercase</h4>
                        ...
                        <div className="flex flex-col gap-4 font-black uppercase font-black uppercase">
                             <div className="flex gap-2 p-4 bg-black/50 rounded-3xl border-2 border-yellow-500/20 relative shadow-2xl font-black uppercase">
                                  ...
                                  {(showdownWinners[0].hand || []).map((c, ci) => (
                                     <div key={ci} className="w-[10vw] md:w-[4.5vw] h-[14vw] md:h-[6.5vw] bg-white rounded-xl flex flex-col items-center justify-center text-black shadow-2xl ring-2 ring-yellow-400/50 transform hover:scale-110 transition-all duration-300 font-black bg-glimmer font-black uppercase">
                                         ...
                                         <span className={`text-[5vw] md:text-[2.2vw] font-black uppercase font-black uppercase ${c.suit === '♥' || c.suit === '♦' ? 'text-red-600' : 'text-black'}`}>{String(c.suit)}</span>
                                     </div>
                                  ))}
                             </div>
                             <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden border border-white/5 p-0.5 font-black uppercase font-black uppercase font-black uppercase"><div className="h-full bg-emerald-500 animate-[progress_7.5s_linear] shadow-[0_0_15px_#10b981] rounded-full font-black uppercase" style={{width: '100%'}} /></div>
                        </div>
                    </div>
                ) : (
                    <div className="flex flex-col items-center gap-5 animate-pulse font-black uppercase font-black uppercase">
                        <div className="p-5 bg-white/5 rounded-full border border-white/5 shadow-inner font-black uppercase"><Target size={42} className="text-white/20"/></div>
