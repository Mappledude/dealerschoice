    ...
    <div className="flex-1 flex flex-col h-full overflow-hidden p-4">
        <div className="flex items-center justify-between text-indigo-400 text-[10px] mb-4 border-b border-indigo-500/20 pb-2 font-black tracking-[0.2em] uppercase">
            ...
            <div className="flex items-center gap-2">
                <button onClick={handleCopyLogs} className="text-white/30 hover:text-emerald-400 transition-all" title="Copy All Logs"><Copy size={14}/></button>
                {isMobile && <button onClick={() => setIntelExpanded(false)} className="text-white/30 hover:text-white ml-1"><X size={14}/></button>}
            </div>
        </div>
        <div className="flex-1 overflow-y-auto scrollbar-hide space-y-3 pr-1 font-black">
            {handHistory && handHistory.length > 0 ? handHistory.map((hand) => (
...
const App = () => {
  ...
  const handleNuclearTriplePurge = useCallback(() => {
    ...
    socket.emit('adminNuclearReset'); setPlayers(INITIAL_PLAYERS); setAllProfiles([]); setActiveTables([]); setNuclearConfirm(false);
  }, [nuclearConfirm]);
  ...
  return (
    <div style={{ height: 'calc(var(--vh, 1vh) * 100)', ...ambientStyle }} className="text-white flex flex-col overflow-hidden relative font-black uppercase tracking-tighter select-none">
      ...
      <div className="flex-1 flex flex-row overflow-hidden relative">
        {intelExpanded && !isMobile && (<aside className="w-80 bg-black/40 border-r border-white/5 hidden lg:flex flex-col animate-in slide-in-from-left duration-300">
          <ActivityFeedContent 
            logs={logs}
        ...
        <main className="flex-1 flex flex-col items-center justify-center relative overflow-hidden font-black uppercase text-center">
            {heroPlayerObj && phase !== PHASES.IDLE && phase !== PHASES.PRE_FLOP && (
              <div className={`transition-all duration-500 ${heroPlayerObj.isFolded ? 'opacity-30 grayscale pointer-events-none' : ''}`}>
                {activeVariant?.id === 'HILOW' && (<div className="absolute top-6 left-6 z-[90] flex flex-col items-start pointer-events-none animate-in fade-in slide-in-from-left duration-700"><div className="bg-black/95 border-2 border-emerald-400 px-4 py-2 rounded-xl shadow-2xl"><span className="text-[10px] text-white/50 tracking-[0.3em] font-black mb-1 block">LOW HAND</span><span className="text-lg lg:text-[2.8vh] font-black text-white tracking-tight leading-none paper-white-standard">{formatRank(String(heroPlayerObj?.lowStrength))}</span></div><div className="mt-2 bg-black/95 border border-white/10 px-3 py-1.5 rounded-lg flex items-center gap-2"><TrendingUp size={12} className="text-emerald-400"/><span className="text-xs font-mono font-black text-emerald-400 tracking-tighter paper-white-standard">{Math.round(heroPlayerObj?.lowWinProbability || 0)}% LOW WIN PROB</span></div></div>)}
            ...
            <div style={{ transform: isMobile ? `scale(${visuals.tableZoom})` : `scale(${Math.min(visuals.tableZoom, 1.2)})` }} className="relative w-full max-w-[1400px] aspect-[15/10] lg:aspect-[16/9] flex items-center justify-center h-full origin-center">
                ...
                <div className={`absolute inset-[35px] rounded-[50%] transition-all duration-700 overflow-hidden ${(previewVariantId || activeVariant?.id) === 'MUFLIS' ? 'animate-muflis-glow' : ''} ${(previewVariantId || activeVariant?.id) === 'OMAHA' ? 'animate-omaha-swirl' : ''}`} style={{ backgroundColor: TABLE_FELT_COLOR, backgroundImage: `radial-gradient(circle at center, rgba(255,255,255,0.1) 0%, transparent 75%)`, boxShadow: `inset 0 0 100px rgba(0,0,0,0.6)` }} ><div className="absolute inset-0 opacity-[0.03] pointer-events-none" style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'><feTurbulence type='fractalNoise' baseFrequency='0.65' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)'/%3E%3C/svg%3E")` }} /></div>
            ...
            (() => {
                ...
                return (
                    <div key={`winner-disp-${winner.name}-${currentShowdownIdx}`} className="flex flex-col items-center justify-start w-full gap-2 animate-in fade-in slide-in-from-bottom-2 duration-500">
                        ...
                        {!isMuckWin && (<><div className="text-[10px] md:text-sm font-black text-white/60 tracking-widest uppercase">HOLDING <span className={themeColor}>{String(formatRank(String(winner.rank)))}</span></div><div className="flex gap-1 justify-center mt-1">{(winner.winningHandCards || winner.hand || []).slice(0,5).map((c, ci) => (
