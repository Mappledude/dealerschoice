...
const App = () => {
    ...
    ([...logs].reverse()).forEach(log => {
        if (String(log.action).includes("DEALING") || String(log.action).includes("LOCKS IN") || String(log.action).includes("PRE_FLOP DEALT")) {
            ...
            currentHand = { 
              ...
              amount: null, 
              events: [], 
              variant: String(log.action).includes("LOCKS IN") 
                ? String(log.action).split("LOCKS IN ")[1] 
                : (String(log.action).split('DEALING ')[1] || "Poker")
            };
        }
        if (currentHand) {
  ...
  useEffect(() => {
    ...
    socket.on('variantPreview', (vId) => setPreviewVariantId(vId));
  ...
  if (currentView === VIEWS.ADMIN) return (
    <div style={{ height: 'calc(var(--vh, 1vh) * 100)' }} className="bg-[#06080c] flex flex-col md:flex-row text-white uppercase font-black overflow-hidden pt-[env(safe-area-inset-top)]">
        <aside className="w-full md:w-64 border-b md:border-r border-white/10 p-4 md:p-8 flex flex-row md:flex-col gap-2 md:gap-4 bg-black/20 shrink-0">
            ...
            <button onClick={handleNuclearTriplePurge} className={`flex-1 md:flex-none p-3 rounded-xl flex items-center justify-center gap-2 border-2 transition-all uppercase ${nuclearConfirm ? 'bg-red-600 border-white text-white' : 'bg-white/5 text-red-500 border-red-500/20'}`}>
      ...
      <header className="bg-black/80 border-b border-white/5 flex items-center justify-between px-4 z-[80] shadow-2xl backdrop-blur-md shrink-0 font-black pt-[env(safe-area-inset-top)] h-[45px] md:h-[55px]">
        <div className="flex-1 flex items-center">
          <button 
            ...
            style={{ backgroundColor: VARIANT_COLORS[previewVariantId || activeVariant?.id || 'HOLDEM'] || '#1e293b' }} 
          ...
          >
            <span style={{ color: getContrastColor(VARIANT_COLORS[previewVariantId || activeVariant?.id || 'HOLDEM']) }} className="text-[8px] tracking-widest leading-none mb-0.5 uppercase font-black flex items-center gap-1 opacity-70">
              {phase === PHASES.IDLE ? 'Previewing:' : 'This Hand:'} <HelpCircle size={8}/>
            </span>
            <span style={{ color: getContrastColor(VARIANT_COLORS[previewVariantId || activeVariant?.id || 'HOLDEM']) }} className="text-xs md:text-sm font-black truncate drop-shadow-sm">
              {VARIANTS[previewVariantId || activeVariant?.id]?.name || '...'}
            </span>
          </button>
        </div>
        ...
        <div className="flex-1 flex items-center justify-end">
          <div 
            className={`border px-3 py-1 rounded-lg flex flex-col min-w-[120px] relative transition-all duration-300 group shadow-lg ${dealAttention ? 'animate-deal-trigger border-white' : 'border-white/10'}`}
            style={{ backgroundColor: VARIANT_COLORS[pendingVariantId] || '#1e293b' }}
          >
            <span style={{ color: getContrastColor(VARIANT_COLORS[pendingVariantId]) }} className="text-[8px] tracking-widest leading-none mb-0.5 uppercase font-bold opacity-70">On My Deal:</span>
            <div className="flex items-center">
              <select 
                value={pendingVariantId} 
                onChange={(e) => { 
                  setPendingVariantId(e.target.value); 
                  socket.emit('previewVariation', { roomId: currentRoomId, variantId: e.target.value });
              ...
              <ChevronDown 
                size={18} 
                className="animate-subtle-bounce ml-1 pointer-events-none" 
                style={{ color: getContrastColor(VARIANT_COLORS[pendingVariantId]) }}
              />
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-row overflow-hidden relative">
        ...
        <main className="flex-1 flex flex-col items-center justify-center relative overflow-hidden font-black uppercase text-center">
            ...
            {heroPlayerObj && phase !== PHASES.IDLE && phase !== PHASES.PRE_FLOP && (
              <div className={`transition-all duration-500 ${heroPlayerObj.isFolded ? 'opacity-30 grayscale pointer-events-none' : ''}`}>
                {activeVariant?.id === 'HILOW' && (
                  <div className="absolute top-6 left-6 z-[90] flex flex-col items-start pointer-events-none animate-in fade-in slide-in-from-left duration-700">
                      <div className="bg-black/95 border-2 border-emerald-400 px-4 py-2 rounded-xl shadow-2xl">
                        <span className="text-[10px] text-white/50 tracking-[0.3em] font-black mb-1 block">LOW HAND</span>
                        <span className="text-lg lg:text-[2.8vh] font-black text-white tracking-tight leading-none paper-white-standard">{formatRank(String(heroPlayerObj?.lowStrength))}</span>
                      </div>
                  </div>
                )}
                <div className="absolute top-6 right-6 z-[90] flex flex-col items-end pointer-events-none animate-in fade-in slide-in-from-right duration-700">
                  <div className="bg-black/95 border-2 border-cyan-400 px-4 py-2 rounded-xl shadow-2xl">
                    <span className="text-[10px] text-white/50 tracking-[0.3em] font-black mb-1 block">STRENGTH</span>
                    <span className="text-lg lg:text-[2.8vh] font-black text-white tracking-tight leading-none paper-white-standard">{formatRank(String(heroPlayerObj?.strength))}</span>
                  </div>
                  <div className="mt-2 bg-black/95 border border-white/10 px-3 py-1.5 rounded-lg flex items-center gap-2">
                    <TrendingUp size={12} className="text-yellow-400"/>
                    <span className="text-xs font-mono font-black text-yellow-400 tracking-tighter paper-white-standard">
            ...
            <div style={{ transform: isMobile ? `scale(${visuals.tableZoom})` : `scale(${Math.min(visuals.tableZoom, 1.2)})` }} className="relative w-full max-w-[1400px] aspect-[15/10] lg:aspect-[16/9] flex items-center justify-center h-full origin-center">
                ...
                <div className="absolute inset-[-100px] rounded-[50%] blur-[120px] opacity-20 pointer-events-none z-0" 
                     style={{ background: `radial-gradient(circle at center, ${underTableNeonColor}, transparent 70%)` }} 
                />
                ...
                <div 
                  className={`absolute inset-[35px] rounded-[50%] transition-all duration-700 overflow-hidden ${(previewVariantId || activeVariant?.id) === 'MUFLIS' ? 'animate-muflis-glow' : ''} ${(previewVariantId || activeVariant?.id) === 'OMAHA' ? 'animate-omaha-swirl' : ''}`} 
                  style={{ 
                ...
                <div className="absolute top-[calc(48%-50px)] left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-30 pointer-events-none w-full">
                  {!potTransferring && (
                    <div className="flex flex-col items-center mb-3 transition-all">
                      ...
                      <div className="text-[6vw] lg:text-[6vh] font-black text-white font-mono tracking-tighter leading-none drop-shadow-[0_5px_15px_rgba(0,0,0,1)] paper-white-standard">${Number(totalDisplayPot).toLocaleString(undefined, {minimumFractionDigits: 2})}</div>
                    </div>
                  )}
                  {community.length > 0 && (
         ...
         <div className="flex-1 flex flex-col items-center justify-start px-4 relative pt-6"> 
          {phase === PHASES.SHOWDOWN && showdownWinners && showdownWinners.length > 0 ? (
            (() => {
             ...
             <div className="flex flex-col gap-4 items-center w-full">
                ...
                <div className="flex flex-row gap-2 w-full max-w-[800px] items-stretch justify-center font-black h-14">
                    ...
                    <button 
                      onClick={() => { if (activeIdx === heroIdx) handleAction('CALL'); else setPreAction(preAction === 'CHECK' ? null : 'CHECK'); }} 
                      className={`flex-1 bg-white/10 border rounded-xl text-xl font-black truncate px-2 flex items-center justify-center gap-2 transition-all 
                        ${activeIdx === heroIdx ? 'border-white shadow-[0_0_30px_rgba(255,255,255,0.4)] ring-4 ring-white/20 animate-pulse' : preAction === 'CHECK' ? 'border-emerald-400 ring-2 ring-emerald-400/50' : 'border-white/5 opacity-60'}`}
                    >
                      {preAction === 'CHECK' && <Check size={20} className="text-emerald-400" />} 
                      {activeIdx === heroIdx ? (highestBet > (heroPlayerObj?.currentBet || 0) + 0.005 ? `CALL $${(highestBet - (heroPlayerObj?.currentBet || 0)).toLocaleString()}` : 'CHECK') : 'CHECK'}
                    </button>
            ...
            <div className="flex flex-col items-center gap-1 py-10">
              <span className="text-white/10 tracking-[0.8em] text-sm font-black italic animate-pulse uppercase">Arena Observation</span>
            </div>
          )}
         </div>
      </footer>
