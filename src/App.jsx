...
const VARIANTS = { 
  ...
  MUFLIS: { id: 'MUFLIS', name: 'Muflis', rules: ["Ace is the lowest card (value 1).", "The hand that would normally be weakest wins the pot."] }, 
  HILOW: { id: 'HILOW', name: 'Hi-Low Split', rules: ["Pot split 50/50 between High and Low hands.", "Must use 2 hole + 3 board cards for both halves."] }, 
  REDSBLACKS: { id: 'REDSBLACKS', name: 'Reds & Blacks', rules: ["4 hole cards dealt.", "Special wildcards based on suit color and parity."] }
};
...
const Seat = ({ 
  ...
  relativeIdx, visuals, bigBlind, showdownWinners, currentWinnerHandIds
    ...
    const cardZIndex = isHero ? 'z-[100]' : (phase === PHASES.SHOWDOWN ? 'z-[150]' : 'z-[40]');
                ...
                <div className="flex flex-col items-center w-full relative z-10 py-1 overflow-hidden">
                    ...
                    <div className="w-full h-[24px] lg:h-[3.5vh] relative flex items-center justify-center">
                        {action && !player.isFolded && !isCollectingBets ? (
                            <div className="w-full h-full flex items-center justify-center bg-black/40 rounded-lg"><span className={`text-[14px] lg:text-[2.2vh] font-black italic uppercase tracking-tight text-center px-1 drop-shadow-md whitespace-nowrap ${action.color}`}>{String(action.text)}</span></div>
                        ) : (
                            <div className="w-full h-full flex items-center justify-center"><span className={`text-[18px] lg:text-[2.8vh] font-mono font-black ${player.chips <= 0 ? 'text-red-500' : 'text-emerald-400'} leading-none tracking-tighter`}>${Number(player.chips || 0).toLocaleString(undefined, {minimumFractionDigits: 2})}</span></div>
                        )}
                    </div>
...
const App = () => {
  ...
  const visuals = useMemo(() => ({ footerHeight: isMobile ? 150 : 250, tableZoom: 0.9, holeCardFan: 35 }), [isMobile]);
  ...
  const joinRoom = useCallback(() => {
    ...
    joinLock.current = true; setIsJoining(true);
  ...
  const handHistory = useMemo(() => {
    ...
    ([...logs].reverse()).forEach(log => {
        if (String(log.action).includes("IS DEALING") || String(log.action).includes("PRE_FLOP DEALT")) {
            ...
            currentHand = { id: log.handId || `hand-${log.timestamp}`, winner: null, rank: null, amount: null, events: [], variant: String(log.action).split('DEALING ')[1] || "Poker" };
        }
        if (currentHand) {
            ...
            if (log.type === 'win') {
                ...
                else if (String(log.action).includes("SCOOPED")) { currentHand.winner = log.name; currentHand.rank = "Muck/Default"; }
            }
        }
    });
    if (currentHand) hands.push(currentHand);
  ...
  const onCreatePlayer = () => {
    ...
    socket.emit('adminCreatePlayer', { ...newPlayer, uid: 'p_' + Math.random().toString(36).slice(2, 7), chips: Number(newPlayer.chips) });
    ...
    const handleRoomUpdate = (d) => {
        ...
        if (isPhaseTransition && [PHASES.FLOP, PHASES.TURN, PHASES.RIVER].includes(d.phase)) {
            ...
            if (d.phase === PHASES.FLOP) {
                setTimeout(() => { setAnnouncement({ text: VARIANTS[d.activeVariant?.id || 'HOLDEM']?.name || "Poker", color: VARIANT_COLORS[d.activeVariant?.id || 'HOLDEM'] || '#fff' }); setTimeout(() => setAnnouncement(null), 1500); }, 3000); 
            }
        }
    ...
    <div style={{ height: 'calc(var(--vh, 1vh) * 100)' }} className="bg-[#06080c] flex flex-col md:flex-row text-white uppercase font-black overflow-hidden pt-[env(safe-area-inset-top)]">
        ...
        <main className="flex-1 p-5 md:p-12 overflow-y-auto bg-black/40">
            {adminTab === ADMIN_TABS.PLAYERS ? (
                <div className="flex flex-col gap-5 md:gap-8"><h3 className="text-lg md:text-xl border-l-4 border-[#fbbf24] pl-4 font-black">PLAYER REGISTRY</h3><div className="bg-white/5 p-4 md:p-6 rounded-2xl grid grid-cols-1 md:grid-cols-3 gap-4 border border-white/10"><input value={newPlayer.name} onChange={e=>setNewPlayer({...newPlayer, name: e.target.value})} placeholder="NAME" className="bg-black/40 p-3 rounded-xl border border-white/10 outline-none uppercase text-white text-sm"/><input value={newPlayer.password} onChange={e=>setNewPlayer({...newPlayer, password: e.target.value})} placeholder="PASS" className="bg-black/40 p-3 rounded-xl border border-white/10 outline-none uppercase text-white text-sm"/><input type="number" value={newPlayer.chips} onChange={e=>setNewPlayer({...newPlayer, chips: e.target.value})} placeholder="WALLET" className="bg-black/40 p-3 rounded-xl border border-white/10 outline-none text-white text-sm"/><button onClick={onCreatePlayer} className="bg-[#fbbf24] text-black rounded-xl font-black p-3 text-sm lg:col-span-3">CREATE PLAYER</button></div><div className="bg-white/5 rounded-2xl overflow-hidden border border-white/10">{allProfiles.length > 0 ? allProfiles.map(p => (<div key={p.uid} className="flex justify-between p-3 md:p-4 border-b border-white/5 items-center hover:bg-white/5"><span className="text-[10px] md:text-sm font-black truncate max-w-[100px]">{String(p.name)}</span><div className="flex gap-2 md:gap-4 items-center"><span className="text-emerald-400 font-mono text-xs md:text-lg">${Number(p.chips || 0).toLocaleString()}</span><button onClick={()=>{const n = prompt("NEW WALLET", String(p.chips || 0)); if(n !== null && n !== "") socket.emit('adminUpdatePlayer', {uid: p.uid, chips: Number(n)})}} className="text-cyan-400"><Edit3 size={14}/></button><button onClick={()=>{const n = prompt("NEW PASSWORD", String(p.password || "")); if(n !== null && n !== "") socket.emit('adminUpdatePlayer', {uid: p.uid, password: n})}} className="text-amber-400"><Key size={14}/></button><button onClick={()=>socket.emit('adminDeletePlayer', p.uid)} className="text-red-500"><Trash2 size={14}/></button></div></div>)) : <div className="p-10 text-center text-white/20">NO PLAYERS REGISTERED</div>}</div></div>
            ) : (
                <div className="flex flex-col gap-5 md:gap-8"><h3 className="text-lg md:text-xl border-l-4 border-emerald-500 pl-4 font-black">ARENA CONTROL</h3><div className="bg-white/5 p-4 md:p-6 rounded-2xl grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 border border-white/10"><input value={newTable.name} onChange={e=>setNewTable({...newTable, name: e.target.value})} placeholder="ARENA NAME" className="bg-black/40 p-3 rounded-xl border border-white/10 outline-none uppercase text-white text-sm"/><div className="flex gap-2"><input type="number" step="0.05" value={newTable.sb} onChange={e=>setNewTable({...newTable, sb: Number(e.target.value)})} placeholder="SB" className="bg-black/40 p-3 rounded-xl border border-white/10 outline-none text-white text-sm w-1/2"/><input type="number" step="0.05" value={newTable.bb} onChange={e=>setNewTable({...newTable, bb: Number(e.target.value)})} placeholder="BB" className="bg-black/40 p-3 rounded-xl border border-white/10 outline-none text-white text-sm w-1/2"/></div><div className="flex gap-2"><input type="number" value={newTable.minBuy} onChange={e=>setNewTable({...newTable, minBuy: Number(e.target.value)})} placeholder="MIN BUY" className="bg-black/40 p-3 rounded-xl border border-white/10 outline-none text-white text-sm w-1/2"/><input type="number" value={newTable.maxBuy} onChange={e=>setNewTable({...newTable, maxBuy: Number(e.target.value)})} placeholder="MAX BUY" className="bg-black/40 p-3 rounded-xl border border-white/10 outline-none text-white text-sm w-1/2"/></div><button onClick={onSpawnArena} className="bg-emerald-600 text-white rounded-xl font-black p-3 text-sm lg:col-span-3">SPAWN ARENA</button></div><div className="grid grid-cols-1 gap-2.5 md:gap-4">{activeTables.length > 0 ? activeTables.map(t => (<div key={t.id} className="bg-white/5 p-3 rounded-2xl flex justify-between items-center border border-white/10"><div><h4 className="text-[#fbbf24] font-black text-xs md:text-base">{String(t.name)}</h4><p className="text-[8px] text-white/40 tracking-widest uppercase">${t.sb}/${t.bb} • Buy-in: ${t.minBuy}-${t.maxBuy}</p></div><button onClick={()=>socket.emit('adminDeleteRoom', t.id)} className="bg-red-950/40 px-2 py-1.5 rounded-xl text-red-500 font-black text-[8px]">TERMINATE</button></div>)) : <div className="p-10 text-center text-white/20">NO ACTIVE ARENAS</div>}</div></div>
            )}
        </main>
    </div>
  );
            ...
            <div style={{ transform: isMobile ? `scale(${visuals.tableZoom})` : `scale(${Math.min(visuals.tableZoom, 1.2)})` }} className="relative w-full max-w-[1400px] aspect-[15/10] lg:aspect-[16/9] flex items-center justify-center h-full origin-center">
                ...
                <div className="absolute top-[calc(48%-50px)] left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-30 pointer-events-none w-full">{!potTransferring && (<div className="flex flex-col items-center mb-6"><span className="text-white/20 text-[10px] tracking-[0.5em] mb-1 uppercase font-bold">Total Pot:</span><div className="text-[6vw] lg:text-[6vh] font-black text-white font-mono tracking-tighter leading-none drop-shadow-[0_0_30px_rgba(255,255,255,0.2)]">${Number(totalDisplayPot).toLocaleString(undefined, {minimumFractionDigits: 2})}</div></div>)}{community.length > 0 && (<div className="flex gap-2 md:gap-4 mt-4 transition-transform" style={{ transform: isMobile ? `scale(${visuals.commCardScale})` : `scale(${visuals.commCardScale * 0.8})` }}>
                    {(community || []).map((c, j) => { 
                        const isWinnerCard = (currentWinnerHandIds || []).includes(c.id); 
                        const isRedSuit = c.suit === '♥' || c.suit === '♦';
                        return (<div key={`comm-${c.id || j}-${j}`} className={`w-[8vw] lg:w-[6vh] h-[11vw] lg:h-[9vh] rounded-xl border-2 bg-white flex flex-col items-start justify-start p-1.5 text-black font-black transition-all duration-500 animate-in slide-in-from-bottom-4 ${isWinnerCard ? 'ring-4 ring-yellow-400 scale-110 shadow-[0_0_30px_#fbbf24]' : 'border-white/10 opacity-100'} ${phase === PHASES.SHOWDOWN && !isWinnerCard ? 'opacity-20 grayscale' : ''}`}><span className={`text-[12px] lg:text-[1.6vh] font-black leading-tight ${isRedSuit ? 'text-red-600' : 'text-slate-900'}`}>{String(c.value)}</span><span className={`text-[14px] lg:text-[2.2vh] font-black leading-tight ${isRedSuit ? 'text-red-600' : 'text-slate-900'}`}>{String(c.suit)}</span></div>); 
                    })}
                </div>)}</div>
          ...
          {phase === PHASES.SHOWDOWN && showdownWinners && (showdownWinners || []).length > 0 ? (
            (() => {
                ...
                return (<div key={`winner-disp-${winner.name}-${currentShowdownIdx}`} className="flex flex-col items-center justify-start w-full gap-2 animate-in fade-in slide-in-from-bottom-2 duration-500"><div className={`flex items-center gap-3 ${isLowWin ? "bg-emerald-400/10" : (isHiLo ? "bg-amber-400/10" : "bg-white/5")} px-5 py-1 rounded-full border ${isLowWin ? "border-emerald-400/30" : (isHiLo ? "border-amber-400/30" : "border-white/10")} max-w-full overflow-hidden shadow-2xl`}><Trophy size={14} className={themeColor + " animate-bounce shrink-0"} /><div className="text-sm md:text-xl font-black tracking-tighter flex items-center gap-2 leading-none whitespace-nowrap"><span className={getNeonNameColor(winner.name)}>{String(winner.name || "").toUpperCase()}</span>{isMuckWin ? (<span className="text-white ml-2">SCOOPED THE POT</span>) : (<><span className="text-white/40">WON TOTAL</span><span className="text-emerald-400 font-mono ml-2">+${Number(winner.amount || 0).toLocaleString(undefined, {minimumFractionDigits: 2})}</span></>)}</div></div>{!isMuckWin && (<><div className="text-[10px] md:text-sm font-black text-white/60 tracking-widest uppercase">HOLDING <span className={themeColor}>{formatRank(String(winner.rank || ""))}</span></div><div className="flex gap-1 justify-center mt-1">{(winner.hand || []).map((c, ci) => {
                    const isRedSuit = c.suit === '♥' || c.suit === '♦';
                    return (<div key={`winner-card-${ci}`} className={`w-10 md:w-16 h-13 md:h-20 bg-white rounded flex flex-col items-start justify-start p-1 text-black shadow-2xl border-t-2 border-x-2 ${cardBorder} relative overflow-hidden`}><span className={`text-[11px] md:text-sm font-black leading-tight ${isRedSuit ? 'text-red-600' : 'text-black'}`}>{String(c.value)}</span><span className={`text-[13px] md:text-xl leading-none ${isRedSuit ? 'text-red-600' : 'text-black'}`}>{String(c.suit)}</span><div className="absolute bottom-0 w-full h-1/2 bg-gradient-to-t from-black/40 to-transparent" /></div>);
                })}</div></>)}</div>);
            })()
          ) : (
