...
const App = () => {
  ...
  useEffect(() => {
    ...
    setSocket(newSocket);
    newSocket.on('connect', () => setLocalId(newSocket.id));
  ...
  const handleAction = (type, amt = 0) => {
    if (socket) socket.emit('playerAction', { type, amount: amt });
  };
  ...
  return (
    <div className="h-screen bg-[#06080c] text-white font-sans flex flex-col overflow-hidden relative">
      ...
      <main className="flex-1 flex items-center justify-center relative pt-16 pb-36 px-4">
        <div className="relative w-full max-w-[1600px] aspect-[21/10] mx-auto flex items-center justify-center">
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex flex-col items-center z-50">
              ...
              {userSeat && phase !== PHASES.IDLE && !isShowdown && (
              ...
              <div className={`flex items-center gap-4 px-10 py-2 rounded-full border-2 bg-black shadow-2xl ${userSeat?.isWinner && isShowdown ? 'border-yellow-400 scale-110' : 'border-white/10'}`}>
      ...
      <footer className="fixed bottom-0 left-0 right-0 h-[200px] bg-black/40 backdrop-blur-3xl border-t border-white/10 z-[6000] flex pointer-events-none">
        <div className="flex-1 p-6 border-r border-white/10 pointer-events-auto overflow-hidden">
          <div className="text-slate-400 uppercase font-black text-sm mb-4 border-b border-white/10 pb-2">INTELLIGENCE FEED</div>
        ...
        <div className="flex-1 py-6 px-10 pointer-events-auto flex flex-col justify-center items-center relative">
          {isHeroTurn ? (
            <div className="w-full h-full flex flex-col justify-between animate-in fade-in slide-in-from-bottom-2">
              <div className="flex gap-4 justify-center">
                <button onClick={() => handleAction('RAISE', Math.floor(currentPotOnTable * 0.5 + highestBet))} className="w-24 h-10 bg-white/5 border border-white/10 rounded-full text-[10px] font-black">1/2 POT</button>
                <button onClick={() => handleAction('RAISE', currentPotOnTable + highestBet)} className="w-24 h-10 bg-white/5 border border-white/10 rounded-full text-[10px] font-black text-yellow-400">POT</button>
              </div>
              <div className="flex gap-8 justify-center mt-4">
                <button onClick={() => handleAction('FOLD')} className="w-32 h-12 bg-red-950/40 border border-red-500/50 rounded-full font-black text-xs text-red-400">FOLD</button>
                <button onClick={() => handleAction('CALL')} className="w-48 h-12 bg-blue-950/40 border border-blue-500/50 rounded-full font-black text-sm text-blue-400">{highestBet > (userSeat?.currentBet || 0) ? 'CALL' : 'CHECK'}</button>
                <button onClick={() => handleAction('RAISE', highestBet + 40)} className="w-32 h-12 bg-emerald-950/40 border border-emerald-500/50 rounded-full font-black text-xs text-emerald-400">RAISE</button>
              </div>
            </div>
          ) : (
            <div className="text-center opacity-80">
              ...
              <div className="font-black text-[1.5vw] tracking-widest text-[#fbbf24] uppercase">
                {activeIdx !== -1 ? `${players[activeIdx]?.name.toUpperCase()}'S TURN` : "WAITING"}
              </div>
            </div>
          )}
        </div>
