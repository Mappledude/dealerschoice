...
const VARIANTS = { 
  ...
  MUFLIS: { id: 'MUFLIS', name: 'Muflis', rules: ["Worst hand wins the pot.", "Ace is the lowest card (value 1).", "The 'best' hand is the one that would normally be the weakest."] }, 
  HILOW: { id: 'HILOW', name: 'Hi-Low Split', rules: ["Pot is split 50/50 between the High hand and the Low hand.", "4 hole cards dealt.", "Must use 2 hole + 3 board cards for both halves."] }, 
  REDSBLACKS: { id: 'REDSBLACKS', name: 'Reds & Blacks', rules: ["4 hole cards dealt.", "Special Joker mechanic: If your hand contains color combinations, you may play with enhanced strength."] }
};
...
const DashTimer = ({ timeRemaining }) => {
  ...
  return (
    <div className="w-24 md:w-32 h-1.5 bg-white/10 rounded-full relative mt-1 overflow-hidden">
      <div className="absolute inset-0 overflow-hidden transition-all duration-1000 linear" style={{ width: `${percentage}%`, backgroundColor: color, boxShadow: `0 0 10px ${color}` }} />
    </div>
  );
};
...
const Seat = ({ 
  ...
  visuals, bigBlind, showdownWinners, formatRank, isSpotlighted
}) => {
    ...
    const currentAction = useMemo(() => {
        ...
        if (phase === PHASES.PRE_FLOP && currentBetNum > 0 && !lastActionStr) {
            if (currentBetNum === bigBlind) return { text: `BB $${currentBetNum}`, color: "text-indigo-400" };
            return { text: `SB $${currentBetNum}`, color: "text-purple-400" };
        }
        if (!lastActionStr) return null;
        switch (lastActionStr) {
            case 'RAISE': return { text: `RAISE $${currentBetNum}`, color: "text-orange-500" };
            case 'CALL': return { text: `CALL $${currentBetNum}`, color: "text-emerald-400" };
    ...
    return (
        <div style={{ left: `${displayPos.x}%`, top: `${displayPos.y}%` }} className={`absolute -translate-x-1/2 -translate-y-1/2 flex flex-col items-center transition-all duration-700 ${isHero ? 'z-[100]' : 'z-20'} ${player.waitingForNextHand ? 'opacity-50' : ''} ${isSpotlighted ? 'scale-[1.25] z-[400]' : ''}`}>
            ...
            {player.hand && Array.isArray(player.hand) && !player.waitingForNextHand && (
                <div 
                  className={`flex items-center justify-center w-[15vw] lg:w-[12vh] h-[8vw] lg:h-[8vh] pointer-events-none transition-all duration-500 ${isHero ? 'z-[200] absolute' : 'z-[80] relative -mb-[5.25vw] lg:-mb-[4vh]'} ${isFoldedBool ? 'opacity-30 grayscale scale-90' : 'opacity-100'}`} 
            ...
            <div className="relative flex flex-col items-center">
                <div className={`relative z-[90] flex flex-col items-center p-2 lg:p-3 rounded-xl border transition-all duration-300 min-w-[120px] lg:min-w-[14vh] overflow-hidden backdrop-blur-3xl scale-[0.85] ${isActiveTurn ? 'border-white ring-4 ring-white/20 bg-slate-800 shadow-[0_0_40px_rgba(255,255,255,0.2)]' : 'border-white/10 bg-black/60'} ${(player.isWinner && phase === PHASES.SHOWDOWN) || isSpotlighted ? 'border-yellow-400 ring-4 ring-yellow-400/50' : ''}`}>
                    {isDealer && (<div className="absolute top-2 right-2 z-[110] pointer-events-none"><div className="w-2.5 h-2.5 bg-red-600 rounded-full border border-red-900 shadow-[0_0_8px_rgba(239,68,68,0.8)]" /></div>)}
                    <div className={`flex flex-col items-center w-full transition-all duration-500 ${isFoldedBool ? 'opacity-40 grayscale' : 'opacity-100'}`}>
                        <div className="flex flex-col items-center gap-0.5 mb-1">
                          <div className="flex items-center gap-1 opacity-60">
                            {player.isBot && <Bot size={10} className="text-indigo-400" />}
                            <span className="text-[12px] lg:text-[1.6vh] font-black text-white uppercase tracking-wider truncate max-w-[80px]">{String(player.name)}</span>
                          </div>
                        </div>
                        <div className="w-full h-[24px] lg:h-[3.5vh] relative flex items-center justify-center overflow-hidden">
                            <div className={`absolute inset-0 flex items-center justify-center transition-all duration-300 ${ghostAction ? 'opacity-0 -translate-y-4' : 'opacity-100 translate-y-0'}`}>
                                <span className={`text-[18px] lg:text-[2.8vh] font-mono font-black ${player.chips <= 0 ? 'text-red-500' : 'text-emerald-400'}`}>
                            ...
                            {ghostAction && (
                                <div className={`absolute inset-0 flex items-center justify-center transition-all duration-300 animate-action-status-in`}>
                                    <span className={`text-[14px] lg:text-[2.4vh] font-black italic uppercase tracking-tighter ${ghostAction.color}`}>{String(ghostAction.text)}</span>
                                </div>
                            )}
                        </div>
    ...
    <div className="w-full max-w-[800px] bg-black/40 border border-white/10 rounded-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col h-full">
      ...
      <div className={`flex-1 overflow-y-auto scrollbar-hide p-2 ${isHiLo && aggregated.length >= 2 ? 'grid grid-cols-2 gap-2' : 'space-y-1.5'}`}>
        {aggregated.map((player, idx) => (
          <div key={`ledger-${player.uid || idx}`} style={{ animationDelay: `${idx * 150}ms` }} className={`bg-white/5 rounded-xl p-1.5 flex flex-col gap-1 animate-in fade-in slide-in-from-left-2 duration-300 ${isHiLo ? 'border border-indigo-500/20' : ''}`}>
...
const App = () => {
  ...
  if (currentView === VIEWS.ADMIN) return (
    <div style={{ height: 'calc(var(--vh, 1vh) * 100)' }} className="bg-[#06080c] flex flex-col md:flex-row text-white font-black overflow-hidden">
        <aside className="w-full md:w-64 border-b md:border-r border-white/10 p-4 flex flex-row md:flex-col gap-2 bg-black/20 shrink-0">
            <button onClick={()=>setAdminTab(ADMIN_TABS.PLAYERS)} className={`flex-1 md:flex-none p-3 rounded-xl ${adminTab === ADMIN_TABS.PLAYERS ? 'bg-[#fbbf24] text-black' : 'bg-white/5'}`}>PLAYERS</button>
            <button onClick={()=>setAdminTab(ADMIN_TABS.TABLES)} className={`flex-1 md:flex-none p-3 rounded-xl ${adminTab === ADMIN_TABS.TABLES ? 'bg-[#fbbf24] text-black' : 'bg-white/5'}`}>TABLES</button>
            <button onClick={()=>{setCurrentView(VIEWS.LOBBY); socket.emit('getInitialData');}} className="flex-1 md:flex-none p-3 rounded-xl bg-cyan-600 text-black">BACK</button>
        </aside>
            ...
            {adminTab === ADMIN_TABS.PLAYERS ? (
                <div className="flex flex-col gap-8">
                    ...
                    <div className="bg-white/5 rounded-2xl border border-white/10">
                        {allProfiles.map(p => (
                            <div key={`profile-${p.uid}`} className="flex justify-between p-4 border-b border-white/5 items-center">
                                <span className="uppercase">{String(p.name)}</span>
                                <div className="flex gap-4 items-center">
                                  <span className="text-emerald-400 font-mono">${(Number(p.chips) || 0).toLocaleString()}</span>
  ...
  if (currentView === VIEWS.LOBBY) return (
    <div style={{ height: 'calc(var(--vh, 1vh) * 100)' }} className="bg-[#000] flex flex-col text-white font-black uppercase overflow-hidden">
        ...
        <header className="h-20 border-b border-white/5 flex items-center justify-between px-6 md:px-12 bg-black/60 backdrop-blur-md">
          <h2 className="tracking-[0.5em] text-lg flex items-center gap-3"><LayoutGrid className="text-emerald-400 w-5"/> LOBBY <span className="text-[10px] text-white/20 font-mono">{VERSION}</span></h2>
          <div className="flex items-center gap-6 font-black">
            <div className="flex flex-col items-end"><span className="text-[10px] text-white/40">{String(userProfile?.name)}</span><span className="text-emerald-400 font-mono text-2xl">${(Number(userProfile?.chips) || 0).toLocaleString()}</span></div>
            <button onClick={()=>{setCurrentView(VIEWS.LOGIN); setUserProfile(null);}} className="text-white/20 hover:text-red-500"><LogOut size={20}/></button>
          </div>
        </header>
        <main className="flex-1 p-4 md:p-12 overflow-y-auto bg-gradient-to-b from-slate-900/20 to-black">
  ...
  return (
    <div style={{ height: 'calc(var(--vh, 1vh) * 100)' }} className="bg-[#06080c] text-white flex flex-col overflow-hidden relative font-black uppercase select-none">
      ...
      {showRulesModal && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/90 backdrop-blur-3xl px-6" onClick={()=>setShowRulesModal(false)}>
          <div className="w-full max-w-[500px] p-8 bg-slate-900 border border-cyan-500/30 rounded-3xl relative shadow-2xl" onClick={e=>e.stopPropagation()}>
            ...
            <div style={{ transform: isMobile ? `scale(${visuals.tableZoom})` : `scale(${Math.min(visuals.tableZoom, 1.2)})` }} className="relative w-full max-w-[1400px] aspect-[16/9] flex items-center justify-center origin-center">
                ...
                <div className="absolute inset-0 pointer-events-none z-20">
                    {(players || []).map((p, i) => { 
                        ...
                        return (<Seat key={`seat-${p.uid || i}`} player={p} displayPos={DISPLAY_POSITIONS[(i - (heroIdx !== -1 ? heroIdx : 0) + TOTAL_SEATS) % TOTAL_SEATS]} phase={phase} winning5Ids={winning5Ids} isActiveTurn={activeIdx === i} isDealer={dealerIdx === i} isHero={i === heroIdx} visuals={visuals} bigBlind={bigBlind} showdownWinners={showdownWinners} isCollectingBets={potTransferring} timeRemaining={timeRemaining} formatRank={formatRank} isSpotlighted={isThisPlayerWinningShowdown} />); 
                    })}
                </div>
                <div className="absolute top-[45%] left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-30 pointer-events-none w-full">
                  {!potTransferring && (
                    <div className="flex flex-col items-center mb-3">
                      <span className="text-white/50 text-[10px] tracking-[0.5em] mb-1">TOTAL POT</span>
                  ...
                  {community.length > 0 && (
                    <div className="flex gap-2 md:gap-4 mt-4 transition-transform" style={{ transform: isMobile ? `scale(${visuals.commCardScale})` : `scale(${visuals.commCardScale * 0.8})` }}>
                      {(community || []).map((c, j) => { 
                        const isRed = c.suit === '♥' || c.suit === '♦'; 
                        return (
                          <div key={`comm-card-${j}`} className={`w-[8vw] lg:w-[6vh] h-[11vw] lg:h-[9vh] rounded-xl border-2 bg-white flex flex-col items-start justify-start p-1.5 text-black font-black transition-all duration-500 shadow-2xl ${winning5Ids?.includes(c.id) ? 'ring-4 ring-yellow-400 scale-110 shadow-[0_0_30px_#fbbf24]' : 'border-white/10'}`}>
        ...
        <header className="bg-black/40 border-b border-white/10 flex items-center justify-between px-4 z-[80] shadow-xl backdrop-blur-3xl shrink-0 font-black h-[48px] md:h-[52px]">
          <div className="flex-1 flex items-center h-full">
            <button onClick={()=>setShowRulesModal(true)} style={{ backgroundColor: VARIANT_COLORS[activeVariant?.id || 'HOLDEM'] || '#1e293b' }} className={`border px-3 h-[36px] md:h-[42px] rounded-lg flex flex-col justify-center gap-0 border-white/10 shadow-lg ${handAttention ? 'animate-pulse ring-2 ring-white scale-105' : ''} transition-all active:scale-95`}>
              <span style={{ color: getContrastColor(VARIANT_COLORS[activeVariant?.id || 'HOLDEM']) }} className="text-[7px] md:text-[9px] uppercase opacity-70">Hand:</span>
              <span style={{ color: getContrastColor(VARIANT_COLORS[activeVariant?.id || 'HOLDEM']) }} className="text-[10px] md:text-sm truncate font-black">{String(activeVariant?.name || 'Poker')}</span>
            </button>
          </div>
          <div className="flex-1 flex items-center justify-center gap-4 h-full">
            <button onClick={() => setIntelExpanded(!intelExpanded)} className={`p-1.5 border border-white/20 rounded-lg active:scale-95 transition-all ${intelExpanded ? 'bg-indigo-600 text-white shadow-[0_0_15px_rgba(79,70,229,0.5)]' : 'text-indigo-400 bg-white/5'}`}><Eye size={16}/></button>
            <button onClick={() => setShowVisualControls(!showVisualControls)} className={`p-1.5 border border-white/20 rounded-lg active:scale-95 transition-all ${showVisualControls ? 'bg-cyan-600 text-white shadow-[0_0_15px_rgba(34,211,238,0.5)]' : 'text-cyan-400 bg-white/5'}`}><Settings size={16}/></button>
            <button onClick={() => {if (userProfile) socket.emit('leaveRoom', { uid: userProfile.uid }); setCurrentView(VIEWS.LOBBY);}} className="text-red-500 p-1.5 border border-white/20 rounded-lg active:scale-95 transition-all bg-white/5 hover:bg-red-950/20"><LogOut size={16}/></button>
          </div>
          <div className="flex-1 flex items-center justify-end h-full">
            <div style={{ background: pendingVariantId === 'RANDOM' ? RAINBOW_GRADIENT : (VARIANT_COLORS[pendingVariantId] || '#0f172a') }} className={`border px-3 h-[36px] md:h-[42px] rounded-lg flex flex-col justify-center gap-0 relative border-white/10 ${dealAttention ? 'animate-pulse ring-2 ring-white scale-105' : ''} transition-all`}>
              <span style={{ color: 'black' }} className="text-[7px] md:text-[9px] uppercase opacity-80 font-black">Deal Choice:</span>
                ...
                {heroPlayerObj && (Number(heroPlayerObj.chips) || 0) < Number(bigBlind) && (phase === PHASES.IDLE || phase === PHASES.SHOWDOWN || heroPlayerObj.isFolded || heroPlayerObj.waitingForNextHand) ? (
                    ...
                    <div className="flex flex-row gap-2 w-full max-w-[400px] items-stretch justify-center h-16 md:h-10">
                      ...
                      <div className={`flex-[1.5] flex bg-black/60 border border-white/20 rounded-xl overflow-hidden transition-all ${activeIdx !== heroIdx ? 'opacity-20 grayscale' : ''}`}>
                        <button onClick={()=> { if(activeIdx === heroIdx) handleRaiseSubmit(raiseInput); }} className={`flex-1 ${Number(raiseInput) >= Number(effectiveMaxBet) ? 'bg-amber-600 border-amber-400' : 'bg-emerald-600 border-emerald-400'} font-black text-[14px] md:text-xs active:scale-95 transition-all shadow-xl`}>
          ...
          <div className="w-full max-w-[400px] bg-black/80 border border-white/20 rounded-3xl p-8 flex flex-col gap-6 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-6 font-black">
              ...
              <div className="flex flex-col gap-4 pt-4 border-t border-white/10">
                ...
                <div className="flex flex-col gap-2"><label className="text-[10px] text-white/70 uppercase flex justify-between">Hero Card Scale <span>{(Number(visuals.heroCardScale) || 2.0).toFixed(2)}</span></label><input type="range" min="1.0" max="5.0" step="0.1" value={visuals.heroCardScale} onChange={(e) => setVisuals({...visuals, heroCardScale: Number(e.target.value)})} className="accent-emerald-400 cursor-pointer" /></div>
                <div className="flex flex-col gap-2"><label className="text-[10px] text-white/70 uppercase flex justify-between">Hero Card Y <span>{visuals.heroCardY}px</span></label><input type="range" min="-300" max="300" step="1" value={visuals.heroCardY} onChange={(e) => setVisuals({...visuals, heroCardY: Number(e.target.value)})} className="accent-indigo-400 cursor-pointer" /></div>
