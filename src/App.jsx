    ...
    <div className="h-screen bg-[#06080c] text-white flex flex-col overflow-hidden relative font-black uppercase tracking-tighter">
      ...
      <header style={{ height: `${headerHeight}px` }} className="bg-[#0a0a0a] border-b border-white/10 flex items-center justify-between px-4 md:px-8 z-[80] shadow-2xl backdrop-blur-md shrink-0 font-black uppercase">
        ...
        {showLayoutControls && (
            <div className="absolute top-16 left-4 bg-black/95 border border-white/10 p-6 rounded-2xl shadow-2xl z-[1000] flex flex-col gap-5 min-w-[280px] max-h-[80vh] overflow-y-auto scrollbar-hide animate-in slide-in-from-top-4 backdrop-blur-xl font-black">
                <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1"><span className="text-[8px] text-white/40 uppercase font-black">HEADER</span><input type="range" min="40" max="100" value={headerHeight} onChange={(e)=>setHeaderHeight(Number(e.target.value))} className="w-full accent-[#fbbf24] h-1 bg-white/10 rounded-full appearance-none"/></div>
                    <div className="space-y-1"><span className="text-[8px] text-white/40 uppercase font-black">FOOTER</span><input type="range" min="120" max="400" value={footerHeight} onChange={(e)=>setFooterHeight(Number(e.target.value))} className="w-full accent-[#fbbf24] h-1 bg-white/10 rounded-full appearance-none"/></div>
                </div>
                <div className="space-y-1"><span className="text-[8px] text-white/40 uppercase font-black">TABLE ZOOM ({Math.round(tableZoom * 100)}%)</span><input type="range" min="0.5" max="1.5" step="0.05" value={tableZoom} onChange={(e)=>setTableZoom(Number(e.target.value))} className="w-full accent-[#fbbf24] h-1 bg-white/10 rounded-full appearance-none"/></div>
                <div className="space-y-1"><span className="text-[8px] text-white/40 uppercase text-emerald-400 font-black">HERO CARD SIZE ({Math.round(heroCardScale * 100)}%)</span><input type="range" min="0.5" max="3.5" step="0.1" value={heroCardScale} onChange={(e)=>setHeroCardScale(Number(e.target.value))} className="w-full accent-emerald-500 h-1 bg-white/10 rounded-full appearance-none"/></div>
                <div className="space-y-1"><span className="text-[8px] text-white/40 uppercase text-blue-400 font-black">COMMUNITY CARD SIZE ({Math.round(communityCardScale * 100)}%)</span><input type="range" min="0.5" max="4.0" step="0.1" value={communityCardScale} onChange={(e)=>setCommunityCardScale(Number(e.target.value))} className="w-full accent-blue-500 h-1 bg-white/10 rounded-full appearance-none"/></div>
                <div className="space-y-1"><span className="text-[8px] text-white/40 uppercase text-orange-400 font-black">PLAYER BADGE OFFSET ({playerBadgeOffset}px)</span><input type="range" min="-200" max="200" step="1" value={playerBadgeOffset} onChange={(e)=>setPlayerBadgeOffset(Number(e.target.value))} className="w-full accent-orange-500 h-1 bg-white/10 rounded-full appearance-none"/></div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1"><span className="text-[8px] text-white/40 uppercase text-purple-400 font-black">STRENGTH Y ({handStrengthYOffset}px)</span><input type="range" min="-200" max="200" step="1" value={handStrengthYOffset} onChange={(e)=>setHandStrengthYOffset(Number(e.target.value))} className="w-full accent-purple-500 h-1 bg-white/10 rounded-full appearance-none"/></div>
                  <div className="space-y-1"><span className="text-[8px] text-white/40 uppercase text-purple-400 font-black">STRENGTH X ({handStrengthXOffset}px)</span><input type="range" min="-200" max="200" step="1" value={handStrengthXOffset} onChange={(e)=>setHandStrengthXOffset(Number(e.target.value))} className="w-full accent-purple-500 h-1 bg-white/10 rounded-full appearance-none"/></div>
                </div>
                <div className="space-y-1"><span className="text-[8px] text-white/40 uppercase text-indigo-400 font-black">HOLE CARD ROTATION ({holeCardRotation}°)</span><input type="range" min="0" max="45" step="1" value={holeCardRotation} onChange={(e)=>setHoleCardRotation(Number(e.target.value))} className="w-full accent-indigo-500 h-1 bg-white/10 rounded-full appearance-none"/></div>
        ...
        <div className="flex gap-2 font-black uppercase items-center">
            ...
            <button onClick={() => socket.emit('adminAddBot', { roomId: currentRoomId })} className="text-indigo-400 p-2 bg-white/5 border border-white/10 rounded-xl hover:bg-indigo-400/20 font-black" title="Bot"><Bot size={20}/></button>
      ...
      <main className="flex-1 flex flex-col items-center justify-center relative bg-gradient-to-b from-emerald-950/20 to-transparent overflow-hidden px-2 py-2 font-black uppercase tracking-tighter">
        <div style={{ transform: `scale(${tableZoom})`, maxHeight: `calc(100vh - ${headerHeight + footerHeight + 10}px)` }} className="relative w-full max-w-[1400px] aspect-[21/10] flex items-center justify-center h-full transition-transform duration-300 ease-out origin-center font-black">
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10 overflow-hidden translate-y-[20%]">
                <span className="text-[12vw] font-black text-white/5 italic tracking-tighter uppercase select-none rotate-[-12deg] whitespace-nowrap">{activeVariant?.name || "Hold'em"}</span>
            </div>
            ...
            <div className="absolute top-[48%] left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-30 pointer-events-none w-full h-full justify-center">
              ...
              {['HOLDEM', 'OMAHA', 'PINEAPPLE', 'HILOW', 'MUFLIS', 'REDSBLACKS'].includes(activeVariant?.id) && (
                <div className="flex gap-2 md:gap-4 mt-6 md:mt-12 font-black uppercase transition-transform duration-300" style={{ transform: `scale(${communityCardScale})` }}>
                    {(community || []).map((c, j) => (
                        ...
                        <div className="flex items-center gap-x-1.5 font-black leading-none overflow-hidden">
                            ...
                            {l.type === 'win' && l.cards && (
                                <div className="flex gap-0.5 ml-1 shrink-0 scale-90 origin-left">
                                    {l.cards.map((c, ci) => (
                                        <div key={ci} className={`flex items-center px-1 rounded-sm text-[8px] font-bold ${c.suit === '♥' || c.suit === '♦' ? 'text-red-600 bg-white' : 'text-slate-900 bg-white'}`}>{c.value}{c.suit}</div>
                                    ))}
                                </div>
                            )}
                        </div>
          ...
          {activeIdx === heroIdx && phase !== PHASES.SHOWDOWN && phase !== PHASES.IDLE && heroPlayerObj ? (
            <div className="flex flex-col gap-3 md:gap-5 animate-in slide-in-from-bottom duration-500 items-center w-full font-black uppercase">
                <div className="absolute top-2 right-4 animate-in slide-in-from-right duration-500">
                    <div className="flex flex-col items-end"><span className="text-[7px] text-white/40 tracking-[0.2em] font-black uppercase italic">Current Hand</span><span className="text-[12px] md:text-[14px] text-purple-400 font-black uppercase">{phase === PHASES.PRE_FLOP ? "Pre-flop" : (heroPlayerObj.strength || "High Card")}</span></div>
                </div>
                ...
                <div className="flex gap-3 md:gap-6 w-full items-center justify-center font-black">
                    ...
                    <div className="flex gap-2 items-center bg-black/60 border border-white/10 p-1 md:p-2 rounded-2xl shadow-inner min-w-[120px] md:min-w-[320px] font-black uppercase">
                        ...
                        <button onClick={()=>handleAction('RAISE', raiseInput)} className="flex-1 h-12 md:h-14 bg-emerald-600/60 border border-emerald-400/50 rounded-xl flex items-center justify-center hover:brightness-125 font-black uppercase text-xs md:text-xl shadow-xl shadow-emerald-900/20 font-black"><Zap size={20} className="md:mr-2 text-emerald-400"/> RAISE</button>
                    </div>
                </div>
            </div>
          ) : (
                ...
                {showdownWinners && showdownWinners.length > 0 ? (
                    <div className="flex flex-wrap gap-4 items-center justify-center w-full overflow-y-auto px-4 py-2">
                        {showdownWinners.map((winner, idx) => (
                            <div key={idx} className="flex flex-col items-center gap-3 bg-black/60 p-4 rounded-[1.5rem] border-2 border-yellow-500/40 shadow-[0_0_50px_rgba(251,191,36,0.3)] min-w-[280px] animate-showdown-card-pop relative overflow-hidden" style={{ animationDelay: `${idx * 0.2}s` }}>
                                ...
                                <div className="flex flex-col items-center z-10 mb-1">
                                    <div className="text-[#fbbf24] font-black text-3xl md:text-5xl tracking-tighter mb-1 drop-shadow-[0_4px_8px_rgba(0,0,0,0.8)] uppercase animate-pulse">{winner.name}</div>
                                ...
                                <div className="flex gap-1 z-10">
                                    {(winner.hand || []).map((c, ci) => (
                                        <div key={ci} className="w-11 md:w-15 h-16 md:h-22 bg-white rounded-lg flex flex-col items-center justify-center text-black shadow-[0_6px_12px_rgba(0,0,0,0.4)] transform font-black transition-all hover:scale-110 active:scale-125 cursor-default relative" style={{ animation: `card-flip 0.7s cubic-bezier(0.4, 0, 0.2, 1) forwards`, animationDelay: `${0.4 + ci * 0.15}s`, opacity: 0 }}>
