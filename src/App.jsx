  ...
  handStrengthYOffset, handStrengthXOffset, seatIdx
    ...
    return (
        <div style={{ left: `${displayPos.x}%`, top: `${displayPos.y}%` }} className={`absolute -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-20 transition-all duration-500 ${player.isFolded ? 'opacity-30 grayscale scale-95' : 'opacity-100'}`}>
            {(isHero || isShowdown || player.isBot) && !player.isFolded && phase !== PHASES.IDLE && (
            ...
            >
                ...
                <div className="flex flex-col items-center gap-0.5 w-full">
                    ...
                    {player.chips === 0 && !player.isFolded && phase !== PHASES.IDLE && phase !== PHASES.SHOWDOWN && (
                        <span className="text-[7px] text-red-500 font-black animate-pulse tracking-widest mt-0.5 uppercase">ALL-IN</span>
                    )}
                </div>
            </div>
                ...
                <div className="relative z-10 flex items-center justify-center w-[12vw] h-[6vw] mt-4 overflow-visible">
                    ...
                    {strengthLabel && !player.isFolded && (isHero || isShowdown) && phase !== PHASES.IDLE && (
                        <div className="absolute -bottom-12 z-[120] whitespace-nowrap bg-purple-600/90 backdrop-blur-md px-3 py-1 rounded-full border border-purple-400 shadow-[0_0_15px_rgba(168,85,247,0.5)] animate-in fade-in zoom-in h-7 flex items-center justify-center" 
                            style={{ transform: `translate(${handStrengthXOffset}px, ${handStrengthYOffset}px)`, bottom: '-15px' }}>
...
const App = () => {
  ...
  useEffect(() => {
    const handleRoomUpdate = (d) => {
        ...
        setPhase(d.phase); setCommunity(d.community || []); setPotAmount(d.potData?.[0]?.amount || 0);
    ...
    socket.on('loginSuccess', (p) => { 
        setUserProfile(p); setPendingVariantId(p.pendingVariant || 'HOLDEM'); setCurrentView(VIEWS.LOBBY); socket.emit('getInitialData'); 
    });
    socket.on('log', (d) => {
    ...
    socket.emit('getInitialData');
    return () => { 
      socket.off('roomUpdate', handleRoomUpdate); socket.off('lobbyUpdate', handleLobbyUpdate); 
      socket.off('profilesUpdate', handleProfilesUpdate); socket.off('initialDataResponse', handleInitialData);
    ...
    <div className="h-screen bg-[#06080c] flex flex-col md:flex-row text-white font-black uppercase overflow-hidden font-black">
        <aside className="w-full md:w-64 border-b md:border-b-0 md:border-r border-white/10 p-4 md:p-8 flex flex-row md:flex-col gap-4 bg-black/20 shrink-0 font-black">
            ...
            <div className="mt-auto flex flex-col gap-2">
                <button onClick={()=>{if(!nuclearConfirm){setNuclearConfirm(true);setTimeout(()=>setNuclearConfirm(false),3000);return;}socket.emit('adminNuclearReset');setNuclearConfirm(false);}} className={`hidden md:flex p-4 rounded-xl items-center justify-center gap-2 border-2 transition-all font-black ${nuclearConfirm ? 'bg-red-600 border-white text-white animate-pulse' : 'bg-red-950/20 border-red-500 text-red-500'}`}>
                    {nuclearConfirm ? <Bomb size={20}/> : <ShieldAlert size={20}/>}
                    <span>{nuclearConfirm ? 'CONFIRM' : 'NUCLEAR'}</span>
                </button>
                <button onClick={()=>{setCurrentView(VIEWS.LOGIN); setUserProfile(null);}} className="p-3 md:p-4 text-white/20 hover:text-white text-xs flex items-center justify-center gap-2 font-black border border-white/5 rounded-xl"><ArrowLeft size={16}/> BACK</button>
            </div>
        </aside>
            ...
            {adminTab === ADMIN_TABS.PLAYERS ? (
                <div className="flex flex-col gap-8 animate-in fade-in">
                    ...
                    <div className="bg-white/5 rounded-2xl overflow-hidden border border-white/10 font-black">
                        {allProfiles.length === 0 && <div className="p-10 text-center text-white/20">NO PROFILES LOADED</div>}
                        {(allProfiles || []).map(p => (
                ...
                <div className="flex flex-col gap-8 animate-in fade-in font-black uppercase">
                    ...
                    <div className="bg-white/5 p-4 md:p-6 rounded-2xl grid grid-cols-1 md:grid-cols-2 gap-4 border border-white/10 shadow-xl font-black">
                        ...
                        <button onClick={()=>{ if (!newTable.name) return; socket.emit('adminCreateRoom', { ...newTable, id: 'room_' + Math.random().toString(36).slice(2, 9) }); setNewTable({ name: '', sb: 10, bb: 20, minBuy: 400, maxBuy: 2000, pendingVariant: 'HOLDEM' }); }} className="bg-emerald-600 rounded-xl font-black p-4 uppercase transition-all font-black">SPAWN ARENA</button>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-black">
                        {activeTables.length === 0 && <div className="p-10 text-center text-white/20">NO ACTIVE ARENAS</div>}
                        {(activeTables || []).map(t => (
    ...
    <div className="h-screen bg-[#06080c] flex flex-col text-white font-black uppercase overflow-hidden font-black">
        ...
        <main className="flex-1 p-6 md:p-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10 overflow-y-auto bg-gradient-to-br from-transparent to-white/5 font-black uppercase">
            {activeTables.length === 0 ? (
                <div className="col-span-full flex flex-col items-center justify-center p-20 text-white/20 gap-4">
                    <ShieldAlert size={64} />
                    <span className="text-xl tracking-[0.5em]">NO ACTIVE ARENAS</span>
                </div>
            ) : (
                activeTables.map((t) => (
                    <div key={t.id} className="p-8 bg-white/5 border border-white/5 rounded-3xl flex flex-col gap-6 shadow-2xl hover:border-[#fbbf24]/20 transition-all group relative overflow-hidden font-black">
                        ...
                        <button onClick={()=>setSelectedTableForJoin(t)} className="relative z-20 w-full p-6 md:p-8 bg-emerald-600 rounded-2xl tracking-widest shadow-xl hover:scale-[1.02] active:scale-95 transition-all font-black uppercase cursor-pointer pointer-events-auto">ENTER ARENA</button>
                    </div>
                ))
            )}
        </main>
  ...
  return (
    <div className="h-screen bg-[#06080c] text-white flex flex-col overflow-hidden relative font-black uppercase tracking-tighter">
      {isBrokeStatus && (
          <div className="fixed inset-0 z-[5000] flex items-center justify-center bg-black/95 backdrop-blur-2xl p-6 font-black">
                <div className="w-full max-w-[400px] p-10 bg-slate-900 border-2 border-red-500 rounded-3xl text-center shadow-[0_0_100px_rgba(239,68,68,0.4)] font-black uppercase">
                    ...
                    <h2 className="text-3xl font-black mb-2 uppercase">BUSTED!</h2>
                    <p className="text-white/40 mb-6 text-[10px] tracking-widest uppercase">WALLET BALANCE: ${userProfile?.chips?.toLocaleString() || '0'}</p>
                    {(userProfile?.chips || 0) >= 1000 ? (
                        <button onClick={() => socket.emit('playerRebuy', { roomId: currentRoomId, uid: userProfile.uid, amount: 1000 })} className="w-full p-6 bg-emerald-600 text-white rounded-2xl shadow-xl animate-bounce font-black uppercase">REBUY $1,000</button>
                    ) : (
                        <div className="p-6 bg-white/5 rounded-2xl border border-white/10 text-white/40 text-xs font-black uppercase">INSUFFICIENT WALLET FUNDS</div>
                    )}
      ...
      <header style={{ height: `${headerHeight}px` }} className="bg-[#0a0a0a] border-b border-white/10 flex items-center justify-between px-4 md:px-8 z-[80] shadow-2xl backdrop-blur-md shrink-0 font-black uppercase">
        ...
        <div className="hidden sm:flex items-center gap-2 bg-white/5 border border-white/10 px-3 py-1.5 rounded-xl mr-2 font-mono text-[11px] shadow-inner animate-in fade-in slide-in-from-right duration-500">
            <TrendingUp size={14} className="text-cyan-400 animate-pulse" />
            <span className="text-white/40 uppercase text-[9px] font-black tracking-tighter shrink-0">Prob:</span>
            <span className="text-[#fbbf24] font-black min-w-[35px] text-right">{Math.round(heroPlayerObj?.winProbability || 0)}%</span>
        </div>
      ...
      <main className="flex-1 flex flex-col items-center justify-center relative bg-gradient-to-b from-emerald-950/20 to-transparent overflow-hidden px-2 py-2 font-black uppercase">
        <div style={{ transform: `scale(${tableZoom})`, maxHeight: `calc(100vh - ${headerHeight + footerHeight + 10}px)` }} className="relative w-full max-w-[1400px] aspect-[21/10] flex items-center justify-center h-full transition-transform duration-300 ease-out origin-center font-black">
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10 overflow-hidden translate-y-[20%]">
                <span className="text-[12vw] font-black text-white/5 italic tracking-tighter uppercase select-none rotate-[-12deg] whitespace-nowrap">{String(activeVariant?.name || "Hold'em")}</span>
            </div>
            ...
            <div className="flex flex-col gap-3 md:gap-5 animate-in slide-in-from-bottom duration-500 items-center w-full font-black uppercase">
                <div className="absolute top-2 right-4 animate-in slide-in-from-right duration-500">
                    <div className="flex flex-col items-end">
                      ...
                      <span className="text-[12px] md:text-[14px] text-purple-400 font-black uppercase">{String(heroPlayerObj.strength || "High Card")}</span>
                    </div>
                </div>
            ...
            <div className="flex flex-col items-center justify-center h-full relative font-black uppercase">
                {showdownWinners && showdownWinners.length > 0 ? (
                    <div className="flex flex-col items-center gap-3 w-full h-full justify-center">
                        <div className="flex items-center gap-2 text-yellow-400 animate-pulse font-black tracking-[0.3em] text-[10px] md:text-xs"><Trophy size={16} /> SHOWDOWN WINNERS</div>
      ...
      <style>{`
          ...
          .animate-deal-card {
            animation: deal-card 0.7s cubic-bezier(0.2, 0.8, 0.2, 1) forwards;
          }
      `}</style>
    </div>
  );
