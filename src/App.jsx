...
// --- TOP-LEVEL UTILITIES (Ensures availability across all components) ---
...
const formatRank = (rank) => {
  if (!rank || typeof rank !== 'string' || rank === "null" || rank === "No Qualifier") return "";
  ...
  const cleanLabel = rank.replace(/^(high|low|scoop): /i, "");
...
const Seat = ({ 
  ...
  relativeIdx, visuals, bigBlind, showdownWinners, formatRank
    ...
    const currentAction = useMemo(() => {
        ...
        if (isFoldedBool) return { text: "FOLDED", color: "text-red-500" };
        if (phase === PHASES.PRE_FLOP && player.currentBet > 0 && !player.lastAction) {
            if (player.currentBet === bigBlind) return { text: `BB $${player.currentBet}`, color: "text-indigo-400" };
            return { text: `SB $${player.currentBet}`, color: "text-purple-400" };
        }
        if (!player.lastAction) return null;
        switch (player.lastAction) {
            case 'RAISE': return { text: `RAISE $${player.currentBet}`, color: "text-orange-500" };
            case 'CALL': return { text: `CALL $${player.currentBet}`, color: "text-emerald-400" };
            case 'CHECK': return { text: "CHECK", color: "text-slate-400" };
            default: return null;
        }
    }, [player, isFoldedBool, phase, bigBlind]);

    useEffect(() => {
        if (currentAction) setGhostAction(currentAction);
        else if (isCollectingBets) {
            const timer = setTimeout(() => setGhostAction(null), 2000);
            return () => clearTimeout(timer);
        } else setGhostAction(null);
        ...
        <div style={{ left: `${displayPos.x}%`, top: `${displayPos.y}%` }} className={`absolute -translate-x-1/2 -translate-y-1/2 flex flex-col items-center transition-all duration-500 ${isHero ? 'z-[100]' : 'z-20'} ${player.waitingForNextHand ? 'opacity-50' : ''}`}>
            ...
            <div className="relative flex flex-col items-center">
                <div className={`relative z-[90] flex flex-col items-center p-2 lg:p-3 rounded-xl border transition-all duration-300 min-w-[120px] lg:min-w-[14vh] overflow-hidden backdrop-blur-xl scale-[0.85] ${isActiveTurn ? 'border-white ring-4 ring-white/20 bg-slate-800 shadow-[0_0_40px_rgba(255,255,255,0.2)]' : 'border-white/10 bg-black/80'} ${player.isWinner && phase === PHASES.SHOWDOWN ? 'border-yellow-400 ring-2 ring-yellow-400/50' : ''}`}>
                    ...
                    <div className={`flex flex-col items-center w-full relative z-10 py-1 transition-all duration-500 ${isFoldedBool ? 'opacity-40 grayscale' : 'opacity-100'}`}>
                        <div className="flex flex-col items-center gap-0.5 shrink-0 mb-1">
                          ...
                          {phase !== PHASES.IDLE && phase !== PHASES.PRE_FLOP && !isFoldedBool && player.strength && (
                        ...
                        <div className="w-full h-[24px] lg:h-[3.5vh] relative flex items-center justify-center overflow-hidden">
                            <div className={`absolute inset-0 flex items-center justify-center transition-all duration-300 ${ghostAction ? 'opacity-0 -translate-y-4 scale-75' : 'opacity-100 translate-y-0 scale-100'}`}>
                                <span className="text-[18px] lg:text-[2.8vh] font-mono font-black text-emerald-400 leading-none tracking-tighter paper-white-standard">
                                    ${Number(player.chips).toLocaleString(undefined, {minimumFractionDigits: 2})}
                                </span>
                            </div>
                            {ghostAction && (
                                <div className={`absolute inset-0 flex items-center justify-center transition-all duration-300 animate-action-status-in`}>
...
const App = () => {
  // --- REFS (Defined at top to prevent ReferenceError) ---
  ...
  const [newPlayer, setNewPlayer] = useState({ name: '', password: '' });
  ...
  const ambientStyle = useMemo(() => {
    ...
    return { background: `radial-gradient(circle at center, ${color}15 0%, #06080c 100%)`, transition: 'background 1.5s ease-in-out' };
  ...
  const handleLogin = useCallback(() => { 
    const pass = String(passwordInput).toLowerCase().trim();
    if (pass === 'pass') { 
        setUserProfile({ name: 'SYSTEM ADMIN', uid: 'admin_sys', role: 'admin', chips: 1000000 }); 
  ...
  const joinRoom = useCallback(() => {
    ...
    socket.emit('joinRoom', { 
      ...
      buyIn: Math.min(buyInAmount, userProfile.chips || 1000) 
    }, (res) => {
        setIsJoining(false);
  ...
  const handleNuclearTriplePurge = useCallback(() => {
    ...
    setPlayers(INITIAL_PLAYERS); setAllProfiles([]); setActiveTables([]); setNuclearConfirm(false);
  }, [nuclearConfirm]);
  ...
  const ActivityFeedContent = () => (
    <div className="flex-1 flex flex-col h-full overflow-hidden p-4">
        <div className="flex items-center justify-between text-indigo-400 text-[10px] mb-4 border-b border-indigo-500/20 pb-2 font-black tracking-[0.2em] uppercase">
            ...
            <div className="flex items-center gap-2">{isMobile && <button onClick={() => setIntelExpanded(false)} className="text-white/30 hover:text-white"><X size={14}/></button>}</div>
        </div>
        <div className="flex-1 overflow-y-auto scrollbar-hide space-y-3 pr-1 font-black">
            {handHistory.length > 0 ? handHistory.map((hand) => (
                <div key={hand.id} className="border border-white/5 rounded-xl overflow-hidden bg-white/5">
                    <button onClick={() => { 
                      const n = new Set(expandedHands); if (n.has(hand.id)) n.delete(hand.id); else n.add(hand.id); setExpandedHands(n); 
                    }} className="w-full p-3 flex flex-col items-start gap-1 transition-all hover:bg-white/5">
                        <div className="flex items-center justify-between w-full"><span className="text-[9px] text-indigo-400 font-bold tracking-widest uppercase">{String(hand.variant)} HAND</span><ChevronRightIcon size={12} className={`transition-transform text-white/40 ${expandedHands.has(hand.id) ? 'rotate-90' : ''}`} /></div>
                        <div className="text-[11px] text-white/90 text-left">{hand.winner ? (<span className="flex items-center gap-2 text-emerald-400 uppercase"><Trophy size={10} /> <span className={getNeonNameColor(hand.winner)}>{String(hand.winner)}</span> WON ${String(hand.amount)}</span>) : (<span className="text-white/40 italic uppercase">HAND IN PROGRESS...</span>)}</div>
  ...
  useEffect(() => {
    const handleRoomUpdate = (d) => {
        if (!d) return;
        setPlayers(() => { 
          const next = Array(TOTAL_SEATS).fill(null); (d.players || []).forEach((p, i) => { if (p) next[i] = { ...p, seatIdx: i }; }); return next; 
        });
        ...
        if (isPhaseTransition && [PHASES.FLOP, PHASES.TURN, PHASES.RIVER].includes(d.phase)) {
            setHandAttention(true); setTimeout(() => { setHandAttention(false); setDealAttention(true); setTimeout(() => setDealAttention(false), 1000); }, 3000);
        }
        phaseRef.current = d.phase; setPhase(d.phase); setCommunity(d.community || []); setPotAmount(d.potAmount || d.potData?.[0]?.amount || 0); 
        setActiveIdx(d.activeIdx ?? -1); setHighestBet(d.highestBet || 0); setBigBlind(d.bb || 2); 
        setMinRaiseAmount(d.minRaiseAmount !== undefined ? d.minRaiseAmount : 0); setDealerIdx(d.dealerIdx ?? -1); setTimeRemaining(d.timeRemaining || 0);
        if (d.activeVariant) setActiveVariant(VARIANTS[d.activeVariant.id] || { id: d.activeVariant.id, name: d.activeVariant.name, rules: [] });
    ...
    socket.on('variantPreview', setPreviewVariantId);
    socket.on('lobbyUpdate', setActiveTables);
    socket.on('log', (l) => setLogs(prev => [{...l, timestamp: Date.now()}, ...prev].slice(0, 100)));
    ...
    socket.on('loginSuccess', (p) => { 
        const prof = p.profile || p; setUserProfile(prof); setPendingVariantId(prof.pendingVariant || 'HOLDEM'); 
        socket.emit('getInitialData'); 
        if (prof.role === 'admin') setCurrentView(VIEWS.ADMIN);
        else if (p.activeRoomId) { setCurrentRoomId(p.activeRoomId); socket.emit('joinRoom', { roomId: p.activeRoomId, profile: prof, buyIn: 0 }, (res) => { if (res?.status === 'ok') setCurrentView(VIEWS.GAME); else setCurrentView(VIEWS.LOBBY); }); } 
        else setCurrentView(VIEWS.LOBBY); 
    });
    
    return () => { socket.off('roomUpdate'); socket.off('variantPreview'); socket.off('lobbyUpdate'); socket.off('profilesUpdate'); socket.off('initialDataResponse'); socket.off('loginSuccess'); socket.off('log'); };
  ...
  useEffect(() => {
    if (activeIdx === heroIdx && heroPlayerObj && turnInitializedRef.current !== activeIdx) {
      ...
      setRaiseInput(Math.min(min, Number(heroPlayerObj.chips || 0) + Number(heroPlayerObj.currentBet || 0)));
  ...
  if (currentView === VIEWS.ADMIN) return (
    <div style={{ height: 'calc(var(--vh, 1vh) * 100)' }} className="bg-[#06080c] flex flex-col md:flex-row text-white uppercase font-black overflow-hidden pt-[env(safe-area-inset-top)]">
        <aside className="w-full md:w-64 border-b md:border-r border-white/10 p-4 md:p-8 flex flex-row md:flex-col gap-2 md:gap-4 bg-black/20 shrink-0">
            ...
            <button onClick={handleNuclearTriplePurge} className={`flex-1 md:flex-none p-3 rounded-xl flex items-center justify-center gap-2 border-2 transition-all uppercase ${nuclearConfirm ? 'bg-red-600 border-white text-white' : 'bg-white/5 text-red-500 border-red-500/20'}`}><Bomb size={14}/> {nuclearConfirm ? 'CONFIRM' : 'NUCLEAR'}</button>
            ...
            {adminTab === ADMIN_TABS.PLAYERS ? (
                <div className="flex flex-col gap-5 md:gap-8">
                    ...
                    <div className="bg-white/5 p-4 md:p-6 rounded-2xl grid grid-cols-1 md:grid-cols-3 gap-4 border border-white/10">
                        ...
                        <button onClick={() => { if (!String(newPlayer.name).trim()) return; socket.emit('adminCreatePlayer', { name: String(newPlayer.name).trim(), password: String(newPlayer.password || '').trim(), chips: 1000, role: 'player', uid: 'p_' + Math.random().toString(36).slice(2, 7) }); setNewPlayer({ name: '', password: '' }); }} className="bg-[#fbbf24] text-black rounded-xl font-black p-3 text-sm uppercase">Create Player</button>
                    </div>
                    <div className="bg-white/5 rounded-2xl overflow-hidden border border-white/10">
                        {allProfiles.length > 0 ? allProfiles.map(p => (
                            <div key={p.uid} className="flex justify-between p-3 md:p-4 border-b border-white/5 items-center hover:bg-white/5 animate-in slide-in-from-right duration-300">
                                ...
                                <div className="flex gap-2 md:gap-4 items-center">
                                  ...
                                  <button onClick={()=>{const n = prompt("NEW WALLET", String(p.chips || 0)); if(n !== null && n !== "") socket.emit('adminUpdatePlayer', {uid: p.uid, chips: Number(n)})}} className="text-cyan-400"><Edit3 size={14}/></button>
                ...
                <div className="flex flex-col gap-5 md:gap-8">
                    ...
                    <div className="bg-white/5 p-4 md:p-6 rounded-2xl grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 border border-white/10">
                        ...
                        <div className="flex gap-2">
                          <input type="number" value={newTable.sb} onChange={e=>setNewTable({...newTable, sb: Number(e.target.value)})} placeholder="SB" className="bg-black/40 p-3 rounded-xl border border-white/10 outline-none text-white text-sm w-1/2"/>
                          <input type="number" value={newTable.bb} onChange={e=>setNewTable({...newTable, bb: Number(e.target.value)})} placeholder="BB" className="bg-black/40 p-3 rounded-xl border border-white/10 outline-none text-white text-sm w-1/2"/>
                        </div>
                        <div className="flex gap-2">
                        ...
                        <button onClick={() => { if (!String(newTable.name).trim()) return; socket.emit('adminCreateRoom', { name: String(newTable.name).trim(), id: 'room_' + Date.now().toString(36), sb: Number(newTable.sb), bb: Number(newTable.bb), minBuy: Number(newTable.minBuy), maxBuy: Number(newTable.maxBuy) }); setNewTable({ name: '', sb: 1, bb: 2, minBuy: 50, maxBuy: 100 }); }} className="bg-emerald-600 text-white rounded-xl font-black p-3 text-sm lg:col-span-3 uppercase">Spawn Arena</button>
                    </div>
                        ...
                        {activeTables.map(t => (
                            <div key={t.id} className="bg-white/5 p-3 rounded-2xl flex justify-between items-center border border-white/10">
                              ...
                              <div className="flex items-center gap-2">
                                <button onClick={() => socket.emit('adminAddBot', { roomId: t.id })} className="bg-indigo-600/40 px-2 py-1.5 rounded-xl text-indigo-400 font-black text-[8px] uppercase">Inject Bot</button>
                                <button onClick={()=>socket.emit('adminDeleteRoom', t.id)} className="bg-red-950/40 px-2 py-1.5 rounded-xl text-red-500 font-black text-[8px] uppercase">Terminate</button>
                              </div>
                            </div>
                        ))}
                    </div>
    ...
    <div style={{ height: 'calc(var(--vh, 1vh) * 100)' }} className="bg-[#000] flex flex-col text-white font-black uppercase overflow-hidden pb-[env(safe-area-inset-bottom)]">
            ...
            <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-xl px-6">
              <div className="w-full max-w-[400px] p-8 bg-slate-900 border border-emerald-500/30 rounded-3xl shadow-[0_0_50px_rgba(16,185,129,0.2)] flex flex-col gap-10">
                ...
                <div className="space-y-4 font-black text-center uppercase">
                  <div className="flex justify-between items-center text-[10px] text-white/40 tracking-[0.2em] font-black"><span>SEATING AMOUNT</span><span className="text-emerald-400 text-2xl font-mono">${Math.min(buyInAmount, userProfile?.chips || 1000).toLocaleString()}</span></div>
                  <input type="range" min={selectedTableForJoin.minBuy || 50} max={Math.min(selectedTableForJoin.maxBuy || 100, userProfile?.chips || 1000)} step={1} value={buyInAmount} onChange={(e) => setBuyInAmount(Number(e.target.value))} className="w-full h-2 bg-white/10 rounded-lg appearance-none cursor-pointer accent-emerald-500" />
                </div>
        ...
        <main className="flex-1 p-4 md:p-12 overflow-y-auto bg-gradient-to-b from-slate-900/20 to-black font-black uppercase text-center">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-7xl mx-auto">
                {activeTables.map((t) => (
                    <div key={t.id} className="group relative bg-slate-900/40 border border-white/5 rounded-2xl md:rounded-3xl flex flex-col p-6 md:p-8 shadow-2xl transition-all hover:border-emerald-500/30 hover:bg-slate-900/60 font-black uppercase text-left">
  ...
  return (
    <div style={{ height: 'calc(var(--vh, 1vh) * 100)', ...ambientStyle }} className="text-white flex flex-col overflow-hidden relative font-black uppercase tracking-tighter select-none">
      <svg style={{ visibility: 'hidden', position: 'absolute', width: 0, height: 0 }}><filter id="noiseFilter"><feTurbulence type="fractalNoise" baseFrequency="0.05 0.2" numOctaves="3" seed={noiseSeed} result="noise" /><feDisplacementMap in="SourceGraphic" in2="noise" scale="2" xChannelSelector="R" yChannelSelector="G" /></filter></svg>

      {announcement && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center pointer-events-none">
            <div className="relative"><div className="absolute inset-0 blur-[40px] opacity-50 bg-current scale-150 animate-pulse" style={{ color: announcement.color }} /><h1 className="text-[10vw] font-black uppercase italic animate-announcement-pop drop-shadow-[0_0_50px_rgba(0,0,0,1)] text-center px-10 relative z-10" style={{ color: announcement.color }}>{String(announcement.text)}</h1></div>
        </div>
      )}
        ...
        <main className="flex-1 flex flex-col items-center justify-center relative overflow-hidden font-black uppercase text-center">
            {heroPlayerObj && phase !== PHASES.IDLE && phase !== PHASES.PRE_FLOP && (
              <div className={`transition-all duration-500 ${heroPlayerObj.isFolded ? 'opacity-30 grayscale pointer-events-none' : ''}`}>
                {activeVariant?.id === 'HILOW' && (<div className="absolute top-6 left-6 z-[90] flex flex-col items-start pointer-events-none animate-in fade-in slide-in-from-left duration-700"><div className="bg-black/95 border-2 border-emerald-400 px-4 py-2 rounded-xl shadow-2xl"><span className="text-[10px] text-white/50 tracking-[0.3em] font-black mb-1 block">LOW HAND</span><span className="text-lg lg:text-[2.8vh] font-black text-white tracking-tight leading-none paper-white-standard">{formatRank(String(heroPlayerObj?.lowStrength))}</span></div></div>)}
                <div className="absolute top-6 right-6 z-[90] flex flex-col items-end pointer-events-none animate-in fade-in slide-in-from-right duration-700"><div className="bg-black/95 border-2 border-cyan-400 px-4 py-2 rounded-xl shadow-2xl"><span className="text-[10px] text-white/50 tracking-[0.3em] font-black mb-1 block">STRENGTH</span><span className="text-lg lg:text-[2.8vh] font-black text-white tracking-tight leading-none paper-white-standard">{formatRank(String(heroPlayerObj?.strength))}</span></div><div className="mt-2 bg-black/95 border border-white/10 px-3 py-1.5 rounded-lg flex items-center gap-2"><TrendingUp size={12} className="text-yellow-400"/><span className="text-xs font-mono font-black text-yellow-400 tracking-tighter paper-white-standard">{Math.round(heroPlayerObj?.winProbability || 0)}% WIN PROB</span></div></div>
              </div>
            )}
            ...
            <div style={{ transform: isMobile ? `scale(${visuals.tableZoom})` : `scale(${Math.min(visuals.tableZoom, 1.2)})` }} className="relative w-full max-w-[1400px] aspect-[15/10] lg:aspect-[16/9] flex items-center justify-center h-full origin-center">
                <div className="absolute inset-[-100px] rounded-[50%] blur-[120px] opacity-20 pointer-events-none z-0" style={{ background: `radial-gradient(circle at center, ${underTableNeonColor}, transparent 70%)` }} />
                <div className="absolute inset-[-20px] rounded-[50%] z-0"><div className="absolute inset-0 rounded-[50%] blur-[20px] opacity-40 animate-pulse" style={{ background: (previewVariantId || activeVariant?.id) === 'REDSBLACKS' ? 'conic-gradient(#ff0000 0deg 120deg, #000 120deg 180deg, #ff0000 180deg 300deg, #000 300deg 360deg)' : (previewVariantId || activeVariant?.id) === 'HILOW' ? `linear-gradient(to right, ${VARIANT_COLORS.HILOW}, ${HILOW_SECONDARY_COLOR})` : VARIANT_COLORS[previewVariantId || activeVariant?.id || 'HOLDEM'] }} /><div className="absolute inset-0 rounded-[50%] border-[24px] border-[#0a0a0a] shadow-[0_20px_50px_rgba(0,0,0,0.8),inset_0_2px_10px_rgba(255,255,255,0.1)]" /></div>
                <div className="absolute inset-0 rounded-[50%] border-[40px] border-[#1a110a] shadow-[inset_0_0_20px_rgba(0,0,0,0.5)] z-0 bg-[#2b1d12]" />
                <div className={`absolute inset-[35px] rounded-[50%] transition-all duration-700 overflow-hidden ${(previewVariantId || activeVariant?.id) === 'MUFLIS' ? 'animate-muflis-glow' : ''} ${(previewVariantId || activeVariant?.id) === 'OMAHA' ? 'animate-omaha-swirl' : ''}`} style={{ backgroundColor: TABLE_FELT_COLOR, backgroundImage: `radial-gradient(circle at center, rgba(255,255,255,0.1) 0%, transparent 75%)`, boxShadow: `inset 0 0 100px rgba(0,0,0,0.6)` }} ><div className="absolute inset-0 opacity-[0.03] pointer-events-none" style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'><feTurbulence type='fractalNoise' baseFrequency='0.65' numOctaves='3' seed={noiseSeed} result='noise' /><feDisplacementMap in="SourceGraphic" in2="noise" scale="2" xChannelSelector="R" yChannelSelector="G" /></filter></svg>")` }} /></div>
                ...
                <div className="absolute top-[calc(48%-50px)] left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-30 pointer-events-none w-full">{!potTransferring && (<div className="flex flex-col items-center mb-3 transition-all"><span className="text-white/20 text-[10px] tracking-[0.5em] mb-1 uppercase font-bold">Total Pot:</span><div className="text-[6vw] lg:text-[6vh] font-black text-white font-mono tracking-tighter leading-none drop-shadow-[0_5px_15px_rgba(0,0,0,1)] paper-white-standard">${Number(totalDisplayPot).toLocaleString(undefined, {minimumFractionDigits: 2})}</div></div>)}{community.length > 0 && (<div className="flex gap-2 md:gap-4 mt-4 transition-transform" style={{ transform: isMobile ? `scale(${visuals.commCardScale || 1.2})` : `scale(${visuals.commCardScale * 0.8 || 1.0})` }}>{(community || []).map((c, j) => { const isRed = c.suit === '♥' || c.suit === '♦'; return (<div key={`comm-${c.id || j}-${j}`} className={`w-[8vw] lg:w-[6vh] h-[11vw] lg:h-[9vh] rounded-xl border-2 bg-white flex flex-col items-start justify-start p-1.5 text-black font-black transition-all duration-500 ${winning5Ids?.includes(c.id) ? 'ring-4 ring-yellow-400 scale-110 shadow-[0_0_30px_#fbbf24]' : 'border-white/10'}`}><span className={`text-[12px] lg:text-[1.6vh] font-black leading-tight ${isRed ? 'text-red-600' : 'text-slate-900'}`}>{String(c.value)}</span><span className={`text-[14px] lg:text-[2.2vh] font-black leading-tight ${isRed ? 'text-red-600' : 'text-slate-900'}`}>{String(c.suit)}</span></div>);})}</div>)}</div>
                {activeIdx === heroIdx && heroPlayerObj && phase !== PHASES.IDLE && (
                  <div className="absolute right-4 md:right-[20px] top-[15%] bottom-[15%] w-16 md:w-20 flex flex-col items-center justify-end z-[250] pointer-events-auto">
                    <div className="flex-1 w-full relative flex items-center justify-center py-4"><input type="range" min={Math.min(minRaiseAmount || (highestBet + bigBlind), Number(heroPlayerObj.chips || 0) + Number(heroPlayerObj.currentBet || 0))} max={Number(heroPlayerObj.chips || 0) + Number(heroPlayerObj.currentBet || 0)} step={1} value={raiseInput} onChange={(e) => setRaiseInput(Number(e.target.value))} className="vertical-range appearance-none bg-white/10 w-8 md:w-10 h-full rounded-full accent-emerald-500 cursor-pointer" style={{ WebkitAppearance: 'slider-vertical', writingMode: 'bt-lr' }} /></div>
                    <div className="mt-4 bg-black/95 border-2 border-emerald-400 px-3 py-2 rounded-xl animate-in zoom-in duration-300 flex flex-col items-center min-w-[110px]"><span className="text-[8px] text-white/40 tracking-widest mb-1 font-bold uppercase text-center">Raise To</span><div className="flex items-center justify-center w-full"><span className="text-emerald-500 font-mono text-lg md:text-2xl mr-0.5">$</span><input type="text" inputMode="decimal" value={raiseInput} onFocus={(e) => e.target.select()} onChange={(e) => { const val = e.target.value.replace(/[^0-9.]/g, ''); setRaiseInput(Number(val)); }} className="bg-transparent text-emerald-400 font-mono text-xl md:text-3xl font-black text-center outline-none w-full" /></div></div>
                  </div>
                )}
            </div>
        </main>
         ...
         <div className="flex-1 flex flex-col items-center justify-start px-4 relative pt-6"> 
          {phase === PHASES.SHOWDOWN && showdownWinners && showdownWinners.length > 0 ? (
            (() => {
                ...
                const isLowWin = String(winner.rank).includes("LOW:"); const isMuckWin = winner.rank === "!";
                const themeColor = isLowWin ? "text-emerald-400" : "text-white";
                return (
                    <div key={`winner-disp-${winner.name}-${currentShowdownIdx}`} className="flex flex-col items-center justify-start w-full gap-2 animate-in fade-in slide-in-from-bottom-2 duration-500">
                        <div className={`flex items-center gap-3 bg-white/5 px-5 py-1 rounded-full border border-white/10 max-w-full overflow-hidden shadow-2xl`}><Trophy size={14} className={themeColor + " animate-bounce shrink-0"} /><div className="text-sm md:text-xl font-black tracking-tighter flex items-center gap-2 leading-none whitespace-nowrap"><span className={getNeonNameColor(winner.name)}>{String(winner.name).toUpperCase()}</span>{isMuckWin ? (<span className="text-white ml-2">SCOOPED THE POT</span>) : (<><span className="text-white/40">WON TOTAL</span><span className="text-emerald-400 font-mono ml-2">+${Number(winner.amount).toLocaleString(undefined, {minimumFractionDigits: 2})}</span></>)}</div></div>
                        {!isMuckWin && (<><div className="text-[10px] md:text-sm font-black text-white/60 tracking-widest uppercase">HOLDING <span className={themeColor}>{String(formatRank(String(winner.rank)))}</span></div><div className="flex gap-1 justify-center mt-1">{(winner.hand || []).map((c, ci) => (
                                <div key={`winner-card-${ci}`} className={`w-10 md:w-16 h-13 md:h-20 bg-white rounded flex flex-col items-start justify-start p-1 text-black shadow-2xl border-t-2 border-x-2 border-white/20 relative overflow-hidden`}>
          ...
          ) : (<div className="flex flex-col items-center gap-1 py-10"><span className="text-white/10 tracking-[0.8em] text-sm font-black italic animate-pulse uppercase">Arena Observation</span></div>)}
         </div>
      </footer>
      ...
      <style>{`
          ...
          @keyframes action-status-in { 0% { opacity: 0; transform: translateY(10px) scale(0.9); filter: blur(4px); } 50% { opacity: 1; transform: translateY(-2px) scale(1.1); filter: blur(0px); } 100% { opacity: 1; transform: translateY(0) scale(1); } }
          ...
          .animate-subtle-bounce { animation: subtle-bounce 2s infinite ease-in-out; }
          ...
          .paper-white-standard { color: #ffffff !important; text-shadow: 2px 2px 0px rgba(0,0,0,0.8), -1px -1px 0px rgba(0,0,0,0.8); }
          .card-back-royal-red { background-color: #450a0a; background-image: radial-gradient(circle at 50% 50%, #ffffff05 2px, transparent 2px), linear-gradient(45deg, #00000030 25%, transparent 25%, transparent 75%, #00000030 75%), linear-gradient(-45deg, #00000030 25%, transparent 25%, transparent 75%, #00000030 75%), repeating-linear-gradient(0deg, transparent, transparent 10px, rgba(255,255,255,0.02) 10px, rgba(255,255,255,0.02) 11px); background-size: 6px 6px, 16px 16px, 16px 16px, 100% 20px; box-shadow: inset 0 0 20px rgba(0,0,0,0.6); }
      `}</style>
    </div>
  );
};
